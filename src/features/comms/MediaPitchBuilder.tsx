...
export function MediaPitchBuilder({ orgId, projectId }: { orgId: string; projectId: string }) {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-amber-200 bg-white p-5">
      ...
      <textarea
        ...
        placeholder="The story angle — why this is news right now."
        value={angle}
        onChange={(e) => setAngle(e.target.value)}
      />
      <Button
        size="sm"
        onClick={() => draft.mutate({ orgId, projectId, reporterName: reporterName.trim(), outlet: outlet.trim(), beat: beat.trim(), angle: angle.trim() })}
        disabled={draft.isPending || !reporterName.trim() || !angle.trim()}
      >
