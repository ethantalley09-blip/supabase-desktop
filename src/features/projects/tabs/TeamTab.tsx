...
} from '@/features/staffing/useStaffing';
import type { ShiftStatus } from '@/features/staffing/staffingMath';
...
export function TeamTab({ orgId, projectId }: { orgId: string; projectId: string }) {
...
function StaffingSection({ orgId, projectId }: { orgId: string; projectId: string }) {
  ...
  const { data: members } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .eq('status', 'active');
      if (error) throw error;
      return data as unknown as { profile_id: string; profiles: { full_name: string | null; email: string } | null }[];
    },
    ...
    <div className="space-y-3 rounded-lg border border-neutral-200 bg-white p-4">
      ...
      {canManage.data && (
        <div className="flex flex-wrap items-end gap-2">
          ...
          <Button
            size="sm"
            disabled={!profileId || !shiftDate || createShift.isPending}
            onClick={() =>
              createShift.mutate(
                { projectId, profileId, shiftDate, teamName },
                ...
                <td className="px-3 py-1.5">
                  {canManage.data ? (
                    <select
                      ...
                      value={s.status}
                      onChange={(e) =>
                        updateStatus.mutate({ shiftId: s.id, projectId, status: e.target.value as ShiftStatus })
                      }
                    >
...
function LogisticsSection({ orgId, projectId }: { orgId: string; projectId: string }) {
  const canView = useHasPermission(orgId, 'hr.view');
  const canManage = useHasPermission(orgId, 'hr.manage');
  ...
  const submit = () => {
    ...
    createBooking.mutate(
      { projectId, hotelName, teamName, checkIn, checkOut, roomCount, nightlyRateCents: Math.round(nightlyRate * 100) },
      {
        onSuccess: () => {
          setHotelName('');
          setTeamName('');
  ...
  return (
    <div className="space-y-3 rounded-lg border border-neutral-200 bg-white p-4">
      ...
      <ul className="space-y-1 text-sm">
        {bookings?.map((b) => (
          <li key={b.id} className="rounded-md border border-neutral-100 bg-neutral-50 px-3 py-1.5 text-neutral-700">
            {b.hotel_name}
