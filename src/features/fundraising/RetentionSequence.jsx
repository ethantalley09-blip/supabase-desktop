...
export function RetentionSequence({ orgId, projectId, donors }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-rose-200 bg-white p-5">
      ...
      <div className="flex gap-2">
        ...
        <input type="number" className="h-9 w-24 rounded-md border border-neutral-300 px-2 text-sm" value={amount} onChange={(e) => setAmount(e.target.value)}/>
      ...
      {draft.data && (<div className="space-y-2">
          ...
          <SequenceStep label={`Impact update (+${draft.data.impact_update_delay_days}d)`} text={draft.data.impact_update}/>
          <SequenceStep label={`Second ask (+${draft.data.second_ask_delay_days}d)`} text={draft.data.second_ask}/>
        </div>)}
    </div>);
}
function SequenceStep({ label, text }) {
    return (<div className="rounded-md border border-rose-100 bg-rose-50 p-3">
