...
export type TerritoryVisit = {
  voter_id: string;
  outcome: VisitOutcome;
  persuadability_bucket: PersuadabilityBucket;
};

export type TerritoryDifficulty = {
  territoryId: string;
  name: string;
  attempts: number;
...
export function computeTerritoryDifficulty(
  voters: VoterRecord[],
  visits: TerritoryVisit[],
  territories: Territory[]
): TerritoryDifficulty[] {
  const voterTerritory = new Map(voters.map((v) => [v.id, v.territory_id]));
  const stats = new Map<string, { attempts: number; contacted: number; opposed: number; deadDoor: number }>();

  for (const visit of visits) {
    const territoryId = voterTerritory.get(visit.voter_id);
    if (!territoryId) continue; // door isn't assigned to a territory — nothing to attribute this visit to
    const s = stats.get(territoryId) ?? { attempts: 0, contacted: 0, opposed: 0, deadDoor: 0 };
    s.attempts += 1;
    if (visit.outcome === 'contacted') s.contacted += 1;
    if (visit.outcome === 'dead_door') s.deadDoor += 1;
    if (visit.persuadability_bucket === 'opposed') s.opposed += 1;
