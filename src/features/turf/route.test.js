...
import { applySegment, buildTurfSnapshot, dominantVoterLanguage, optimizeWalkOrder, splitIntoWalkLists, voterCity, voterLanguage, voterWard } from './route';
function voter(partial) {
    return {
        project_id: 'p1',
        data: {},
        full_name: null,
...
describe('splitIntoWalkLists', () => {
    ...
    it('partitions doors into k balanced lists covering every door exactly once', () => {
        ...
        const doors = [];
        for (let i = 0; i < 6; i++)
            doors.push(at(`w${i}`, -80 + i * 0.001, 40));
        for (let i = 0; i < 6; i++)
            doors.push(at(`e${i}`, -70 + i * 0.001, 40));
        ...
        for (const l of lists)
            expect(l.ordered.length).toBeLessThanOrEqual(4);
    });
