...
export function OutreachBooster({ project }) {
    ...
    if (!canUse.data)
        return null;
    if (!entitlement.data) {
        return (<div className="rounded-lg border border-dashed border-neutral-300 bg-neutral-50 p-4 text-sm text-neutral-500">
    ...
    const vcard = () => ['BEGIN:VCARD', 'VERSION:3.0', `FN:${project.name}`, `ORG:${project.name}`, `TEL;TYPE=CELL:${phone}`, 'END:VCARD'].join('\n');
    const downloadVcard = () => {
    ...
    const generate = async () => {
        const result = await assist.mutateAsync({
            ...
            tone,
            instructions: `The supporter is texting ${friendName || 'a friend'}. ` +
    ...
    return (<div className="space-y-3 rounded-lg border border-violet-200 bg-white p-5">
      ...
      <div className="space-y-1.5">
        ...
        <textarea id="issue" rows={2} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" value={issue} onChange={(e) => setIssue(e.target.value)} placeholder="Remind them to vote early this week and share the polling-place link."/>
      </div>
      ...
      {draft && (<div className="space-y-2 rounded-md border border-neutral-200 bg-neutral-50 p-3">
          ...
          <div className="flex gap-2">
            ...
            {phone && (<Button variant="outline" size="sm" onClick={() => copy(phone, 'number')}>
