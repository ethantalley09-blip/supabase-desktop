import { computeCanvasserLeaderboard } from '@/features/turf/canvasserStats';
import { computeTerritoryDifficulty } from '@/features/turf/territoryDifficulty';
import { computeBestTimeToKnock, detectPersuasionDrift } from '@/features/turf/visitHistory';
import { detectCanvasserFatigue } from '@/features/turf/canvasserFatigue';
import { detectSilentCanvassers } from '@/features/turf/canvasserSilence';
...
// ---------------------------------------------------------------------------
export function answerTopCanvasser(visits) {
...
function todaysVisits(visits, now) {
...
export function answerDoorsKnockedToday(visits, now = new Date()) {
...
export function answerTodaysContactRate(visits, now = new Date()) {
...
export function answerAvgDoorsPerCanvasserToday(visits, now = new Date()) {
...
export function answerTopCanvasserThisWeek(visits, now = new Date()) {
...
export function computeCanvasserMomentum(visits, now = new Date()) {
    ...
    for (const [canvasserId, { early, later, name }] of byCanvasser) {
        if (early.length < MIN_ATTEMPTS_PER_HALF_WEEK || later.length < MIN_ATTEMPTS_PER_HALF_WEEK)
            continue;
...
export function answerCanvasserMomentum(visits, now = new Date()) {
    ...
    const parts = [];
    if (improving.changePct > 0)
        parts.push(`${improving.name} improving most (+${improving.changePct}pt)`);
    if (declining.changePct < 0 && declining.canvasserId !== improving.canvasserId)
        parts.push(`${declining.name} declining most (${declining.changePct}pt)`);
    return {
...
export function computeTeamContactRates(visits, shifts) {
    const teamByCanvasserDate = new Map();
    for (const s of shifts) {
        if (s.team_name)
            teamByCanvasserDate.set(`${s.profile_id}|${s.shift_date}`, s.team_name);
    }
    const stats = new Map();
    for (const v of visits) {
...
export function answerTeamContactRate(visits, shifts) {
...
export function computeDayOfWeekPerformance(visits) {
    const byDay = new Map();
    for (const v of visits) {
...
export function answerBestDayOfWeek(visits) {
...
export function answerWeekendVsWeekday(visits) {
    ...
    const rate = (arr) => Math.round((arr.filter((v) => v.outcome === 'contacted').length / arr.length) * 100);
    return {
        id: 'weekend_vs_weekday',
        question,
...
// should assert.
export function answerCoachingPairs(visits) {
...
export function answerTodayVsBaseline(visits, now = new Date()) {
    ...
    const staffNote = Math.abs(staffChangePct) > 15
...
// ---------------------------------------------------------------------------
export function answerHardestTerritory(voters, visits, territories) {
...
export function rankTerritoriesByContactRate(ranked) {
    if (ranked.length === 0)
...
export function answerBestContactRateTerritory(voters, visits, territories) {
...
export function answerWorstContactRateTerritory(voters, visits, territories) {
...
export function answerNextTerritoryToCanvass(voters, territories, now = new Date()) {
    const question = 'Which turf should we canvass next?';
    const remaining = remainingDoorsToday(voters, now.getTime());
    const counts = new Map();
    for (const v of remaining) {
        if (v.territory_id)
            counts.set(v.territory_id, (counts.get(v.territory_id) ?? 0) + 1);
    }
...
export function computeTerritorySupportBreakdown(voters, visits, territories) {
    ...
    for (const v of visits) {
        ...
        if (v.persuadability_bucket === 'base_support')
            e.baseSupport += 1;
        else if (v.persuadability_bucket === 'persuadable')
            e.persuadable += 1;
        else if (v.persuadability_bucket === 'opposed')
            e.opposed += 1;
...
export function answerTerritorySupportBreakdown(voters, visits, territories) {
...
export function computeWalkbookSizes(voters, territories) {
    const counts = new Map();
...
export function answerWalkbookSizeOutliers(voters, territories) {
    ...
    if (tooLarge.length > 0)
        parts.push(`${tooLarge.map((t) => t.name).join(', ')} ${tooLarge.length === 1 ? 'is' : 'are'} oversized`);
    if (tooSmall.length > 0)
        parts.push(`${tooSmall.map((t) => t.name).join(', ')} ${tooSmall.length === 1 ? 'is' : 'are'} undersized`);
    return {
        id: 'walkbook_size_outliers',
        question,
...
export function answerUnattemptedDoors(voters, visits) {
...
export function answerRevisitCandidates(voters, visits) {
    const question = 'Which doors should be reattempted, and when?';
    const targets = voters.map((v) => ({ id: v.id, full_name: v.full_name }));
...
// ---------------------------------------------------------------------------
export function answerBestTimeToKnock(visits) {
...
// ---------------------------------------------------------------------------
export function answerPersuasionDrift(visits) {
...
// ---------------------------------------------------------------------------
export function answerCanvasserWellbeing(visits, now = new Date()) {
    ...
    const parts = [];
    if (fatigue.length > 0)
        parts.push(`${fatigue.length} showing a real pace drop today`);
    if (silent.length > 0)
        parts.push(`${silent.length} gone quiet for 90+ minutes after an active start`);
    return {
...
// ---------------------------------------------------------------------------
export function describeDataFreshnessAlert(visits, now = new Date()) {
    if (visits.length === 0)
        return null;
...
export function computeFundraisingPace(donations, now = new Date()) {
...
export function answerFundraisingPace(donations, now = new Date()) {
...
export function describeFundraisingMomentumAlert(pace) {
    if (pace.trendPct === null || Math.abs(pace.trendPct) < 25)
        return null;
...
export function answerTopFundraiser(donations) {
...
export function answerPaymentMethodBreakdown(donations) {
...
export function answerDonorGrowthVsAverageGift(donations, now = new Date()) {
...
export function answerDonorConcentration(donations) {
...
export function answerDonorRepeatShare(donations) {
    ...
    const countByDonor = new Map();
    for (const d of donations)
        countByDonor.set(d.donor_id, (countByDonor.get(d.donor_id) ?? 0) + 1);
    let repeatCents = 0;
    let firstTimeCents = 0;
    for (const d of donations) {
        if ((countByDonor.get(d.donor_id) ?? 0) > 1)
            repeatCents += d.amount_cents;
        else
            firstTimeCents += d.amount_cents;
    }
...
export function computeSocialPerformance(posts) {
    ...
    for (const p of posted)
        byPlatform.set(p.platform, (byPlatform.get(p.platform) ?? 0) + (p.impressions ?? 0));
...
export function answerSocialPerformance(posts) {
...
export function answerWeeklySocialReach(posts, now = new Date()) {
...
export function answerBestPerformingPost(posts) {
    ...
    const ranked = [...eligible].sort((a, b) => (b.engagement_count ?? 0) / (b.impressions || 1) - (a.engagement_count ?? 0) / (a.impressions || 1));
    const top = ranked[0];
...
export function answerPostingFrequency(posts, now = new Date()) {
...
export function computeDoorstepAttribution(donations) {
...
export function answerDoorstepAttribution(donations) {
...
// ---------------------------------------------------------------------------
export function answerTodaysStaffing(shifts, date) {
...
export function answerLodgingCost(bookings) {
...
// ---------------------------------------------------------------------------
export function answerCurrentDoorScript(entries) {
...
export function answerCurrentSurveyQuestions(entries) {
...
export function scenarioAddWorkers(currentDailyRate, perWorkerDailyRate, extraWorkers, remaining) {
...
export function computeAverageDailyDoors(visits, now = new Date(), windowDays = 7) {
...
export function answerSkipADayImpact(visits, now = new Date()) {
...
// ---------------------------------------------------------------------------
export function describeContactRateDropAlert(visits, now = new Date()) {
    const todayStart = new Date(now);
    todayStart.setHours(0, 0, 0, 0);
    ...
    if (todays.length < MIN_DAY_ATTEMPTS)
        return null;
    const priorWeek = visits.filter((v) => {
        const t = new Date(v.occurred_at).getTime();
        return t >= todayStartMs - 7 * DAY_MS && t < todayStartMs;
    });
    if (priorWeek.length < MIN_DAY_ATTEMPTS)
        return null;
