...
export function FilingGap({ orgId, projectId, aiEnabled }) {
    ...
    const gap = useMemo(() => (Number(theirs) > 0 || ours > 0 ? computeMoneyGap(ours, Math.round(Number(theirs || 0) * 100)) : null), [ours, theirs]);
    ...
    return (<div className="space-y-3 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="space-y-1">
          ...
          <Input type="number" className="w-36" placeholder={ourTotalCents ? String(Math.round(ourTotalCents / 100)) : 'enter ours'} value={ourOverride} onChange={(e) => setOurOverride(e.target.value)}/>
        </div>
      </div>

      {gap && Number(theirs) >= 0 && theirs !== '' && (<div className={`rounded-md border-l-4 p-3 text-sm ${gap.leader === 'us'
                ? 'border-emerald-500 bg-emerald-50 text-emerald-900'
                : gap.leader === 'them'
                    ? 'border-amber-500 bg-amber-50 text-amber-900'
                    : 'border-neutral-400 bg-neutral-50 text-neutral-800'}`}>
          {gap.leader === 'tied' ? (<span className="font-semibold">Dead even at {fmt(ours)}.</span>) : (<>
              ...
              {gap.leader === 'them' && (<> Try Funding Runway and the Emergency Ask on the Fundraising tab to close it.</>)}
            </>)}
          {read.data && (<p className="mt-2 flex items-start gap-1.5 border-t border-current/20 pt-2 text-xs opacity-90">
