...
import { useEventPlanningBriefing, type EventPlanningBriefing } from './useFundraisingAi';
...
export function HighDollarEventPlanner({
  orgId,
  projectId,
  donors,
  donations
  ...
  const [briefing, setBriefing] = useState<EventPlanningBriefing | null>(null);
  ...
  const getBriefing = () => {
    if (plan.invitees.length === 0) return;
    setBriefing(null);
    ai.mutate(
      {
        orgId,
  ...
  return (
    <div className="space-y-3 rounded-lg border border-rose-200 bg-white p-5">
      ...
      {plan.invitees.length === 0 ? (
        <p className="text-sm text-neutral-500">No prior real donors yet to build an invite list from.</p>
      ) : (
        <>
          ...
          {briefing && (
            <div className="space-y-1 rounded bg-rose-50 p-3 text-xs text-neutral-700">
              <p className="font-medium text-neutral-900">{briefing.headline}</p>
              <p className="text-rose-700">{briefing.gap_assessment}</p>
              {briefing.priority_calls.length > 0 && (
                <p>Call first: {briefing.priority_calls.join(', ')}</p>
              )}
            </div>
          )}
        </>
