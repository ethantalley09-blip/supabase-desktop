...
export function EmailCampaignStudio({ orgId, projectId }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-sky-200 bg-white p-5">
      ...
      <textarea rows={2} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" placeholder="What should the email say? Key points, tone, any real deadline." value={brief} onChange={(e) => setBrief(e.target.value)}/>
      <Button size="sm" onClick={() => draft.mutate({ orgId, projectId, goal: goal.trim(), audience: audience.trim(), brief: brief.trim() })} disabled={draft.isPending || !goal.trim() || !brief.trim()}>
      ...
      {draft.data && (<div className="space-y-2">
          <div className="grid gap-1.5 sm:grid-cols-3">
            ...
            ].map(([label, subject]) => (<button key={label} type="button" onClick={() => copy(`subject-${label}`, subject)} className="rounded-md border border-sky-100 bg-sky-50 p-2 text-left text-xs text-neutral-700 hover:bg-sky-100">
