...
export function IssueResponseEngine({ orgId, projectId, donations }) {
    const { data: events } = useIssueEvents(projectId);
    const draft = useDraftIssueResponse();
    const [eventType, setEventType] = useState('endorsement');
    ...
    return (<div className="space-y-3 rounded-lg border border-violet-200 bg-white p-5">
      ...
      <div className="flex flex-wrap gap-2">
        <select className="h-9 rounded-md border border-neutral-300 bg-white px-3 text-sm" value={eventType} onChange={(e) => setEventType(e.target.value)}>
          {EVENT_TYPES.map((t) => (<option key={t.value} value={t.value}>
              {t.label}
            </option>))}
        </select>
        <textarea rows={2} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" placeholder="What just happened? e.g. The Teachers Union endorsed us this morning." value={description} onChange={(e) => setDescription(e.target.value)}/>
      ...
      {events && events.length > 0 && (<div className="space-y-2">
          {events.map((ev) => (<div key={ev.id} className="rounded-md border border-neutral-200 p-3">
              <p className="text-sm font-medium text-neutral-900">
                {EVENT_TYPES.find((t) => t.value === ev.event_type)?.label ?? ev.event_type}: {ev.description}
              </p>
              {ev.responses && (<div className="mt-2 space-y-2">
                  {[ev.responses.email_a, ev.responses.email_b].map((em, i) => (<div key={i} className="rounded bg-neutral-50 p-2">
