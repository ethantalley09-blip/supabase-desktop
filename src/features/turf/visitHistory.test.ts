...
import { computeBestTimeToKnock, detectPersuasionDrift, type VisitForDrift, type VisitLike } from './visitHistory';
...
// timezone.
const atLocalHour = (hour: number, minute = 0): string =>
  new Date(2026, 6, 22, hour, minute, 0).toISOString();
...
describe('computeBestTimeToKnock', () => {
  ...
  it('picks the hour with the highest contact RATE, not the highest raw count', () => {
    const visits: VisitLike[] = [
      ...
      ...Array.from({ length: 7 }, () => ({ occurred_at: atLocalHour(18), outcome: 'no_answer' as const })),
      ...Array.from({ length: 3 }, () => ({ occurred_at: atLocalHour(18), outcome: 'contacted' as const })),
...
describe('detectPersuasionDrift', () => {
  const visit = (over: Partial<VisitForDrift>): VisitForDrift => ({
    voter_id: 'v1',
