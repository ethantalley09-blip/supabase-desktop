...
export function BallotChase({ orgId, projectId, voters, visibleVoters, canManage, canUseAi }) {
    ...
    const notesInView = useMemo(() => visibleVoters
        .map((v) => v.canvass_notes?.trim())
        .filter((n) => Boolean(n)), [visibleVoters]);
    const rows = visibleVoters.slice(0, MAX_ROWS);
    const digest = summarize.data?.text;
    const runSummary = () => summarize.mutate({
        orgId,
        projectId,
    ...
    return (<div className="space-y-3">
      ...
      {showAi && (<div className="space-y-2 rounded-lg border border-violet-200 bg-white p-4">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" onClick={runSummary} disabled={summarize.isPending || notesInView.length === 0}>
          ...
          {summarize.isError && (<p className="text-sm text-red-600">{summarize.error.message}</p>)}
          {digest && (<div className="whitespace-pre-wrap rounded-md border border-neutral-200 bg-neutral-50 p-3 text-sm text-neutral-800">
              {digest}
            </div>)}
        </div>)}
      ...
      <div className="overflow-hidden rounded-lg border border-neutral-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((v) => (<tr key={v.id} className="border-t border-neutral-100 align-top">
                <td className="px-4 py-2">
                  <div className="font-medium text-neutral-900">{v.full_name || '—'}</div>
                  <div className="text-xs text-neutral-400">{v.address_line || ''}</div>
                </td>
                <td className="px-4 py-2">
                  {canManage ? (<select className="h-8 rounded-md border border-neutral-300 bg-white px-2 text-sm" value={v.contact_status} disabled={updateStatus.isPending} onChange={(e) => updateStatus.mutate({
                    voterId: v.id,
                    projectId,
                    contact_status: e.target.value,
                    current: v
                })}>
                      {CONTACT_OPTIONS.map((o) => (<option key={o.value} value={o.value}>
                          {o.label}
                        </option>))}
                ...
                <td className="px-4 py-2">
                  {canManage ? (<select className="h-8 rounded-md border border-neutral-300 bg-white px-2 text-sm" value={v.ballot_status} disabled={updateStatus.isPending} onChange={(e) => updateStatus.mutate({
                    voterId: v.id,
                    projectId,
                    ballot_status: e.target.value,
                    current: v
                })}>
                      {BALLOT_OPTIONS.map((o) => (<option key={o.value} value={o.value}>
                          {o.label}
                        </option>))}
                    </select>) : (<span className="text-neutral-500">
        ...
        {visibleVoters.length > MAX_ROWS && (<p className="border-t border-neutral-100 px-4 py-2 text-xs text-neutral-400">
