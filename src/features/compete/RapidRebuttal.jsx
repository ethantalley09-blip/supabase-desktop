...
import { useRebuttal } from './useCompete';
...
export function RapidRebuttal({ orgId, projectId, records }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-red-200 bg-white p-5">
      ...
      <select className="h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm" value={recordId} onChange={(e) => setRecordId(e.target.value)}>
        <option value="">Which logged claim are you answering?</option>
        {records?.map((r) => (<option key={r.id} value={r.id}>
            {r.occurred_on} — {r.content.slice(0, 80)}
          </option>))}
      </select>
      <textarea rows={2} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" placeholder="The true facts, in your words. e.g. She voted FOR the levy in 2024 — it's in the county record." value={facts} onChange={(e) => setFacts(e.target.value)}/>
      <Button size="sm" onClick={() => record && rebut.mutate({ orgId, projectId, record, correctingFacts: facts.trim() })} disabled={rebut.isPending || !record || !facts.trim()}>
        <Sparkles className="h-4 w-4"/>
        {rebut.isPending ? 'Drafting…' : 'Draft rebuttal'}
      </Button>
      {rebut.isError && <p className="text-sm text-red-600">{rebut.error.message}</p>}

      {rebut.data && (<div className="space-y-2">
          {[
                ['Campaign statement', rebut.data.statement],
                ['Social', rebut.data.social],
                ['At the door', rebut.data.door_response]
            ].map(([label, text]) => (<div key={label} className="rounded-md border border-red-100 bg-red-50 p-3">
