...
export function OverviewTab({ project, onOpenTool }) {
    ...
    const stats = useMemo(() => {
        ...
        const lapsing = [...giftsByDonor.entries()].filter(([id, gifts]) => scoreLapse({ donorId: id, gifts }).lapseScore >= 40).length;
    ...
    const daysUntilShortfall = (dateStr) => Math.max(0, Math.round((new Date(dateStr).getTime() - Date.now()) / 86_400_000));
    const go = (toolId, tab) => onOpenTool?.({ tab, anchor: `tool-${toolId}` });
    const allActions = [
    ...
    return (<div className="space-y-4">
      ...
      {briefing.data && (<div className="flex items-start gap-2 rounded-lg border border-violet-200 bg-violet-50 p-3">
      ...
      <div className="rounded-lg border border-neutral-200 bg-white p-4">
        ...
        <svg viewBox="0 0 240 48" className="mt-2 h-12 w-full" preserveAspectRatio="none" role="img" aria-label="Donations per day, last 30 days">
          <polyline points={sparklinePoints(stats.last30)} fill="none" stroke="#059669" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round"/>
        </svg>
      </div>

      {/* Field progress */}
      <div className="grid gap-3 sm:grid-cols-2">
        <Progress label="Voters with a canvass note" value={pct(stats.contacted, stats.v.length)} detail={`${stats.contacted} of ${stats.v.length}`} color="bg-violet-500"/>
        <Progress label="Ballots returned" value={pct(stats.returned, stats.v.length)} detail={`${stats.returned} returned · ${stats.requested} requested`} color="bg-emerald-500"/>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <MiniCard icon={stats.momentum.direction === 'down' ? <TrendingDown className="h-4 w-4 text-red-500"/> : <TrendingUp className="h-4 w-4 text-emerald-500"/>} label="Momentum (15-day)" value={stats.momentum.direction === 'flat'
            ? 'Flat'
            : `${stats.momentum.direction === 'up' ? '+' : '-'}${stats.momentum.changePct}%`} detail={`$${Math.round(stats.momentum.last15Cents / 100).toLocaleString()} vs $${Math.round(stats.momentum.prev15Cents / 100).toLocaleString()} prior 15 days`}/>

        <MiniCard icon={<Wallet className="h-4 w-4 text-neutral-600"/>} label="Funding runway" value={!runwayPlan
            ? 'Not run yet'
            : runwayPlan.shortfall_date
                ? `${daysUntilShortfall(runwayPlan.shortfall_date)}d to shortfall`
                : 'Healthy'} detail={!runwayPlan
            ...
            : runwayPlan.shortfall_date
                ...
                : 'No shortfall projected in the next 90 days'} onClick={() => go('funding_runway', 'fundraising')}/>

        <MiniCard icon={<Newspaper className="h-4 w-4 text-indigo-600"/>} label="Opposition pulse" value={String(opponentRecords?.length ?? 0)} detail={opponentRecords && opponentRecords.length > 0
            ? `most recent: ${opponentRecords[0].record_type} on ${opponentRecords[0].occurred_on}`
            : 'log their public record to activate this'} onClick={() => go('opponent_log', 'compete')}/>

        <MiniCard icon={<Clock className="h-4 w-4 text-neutral-600"/>} label="Best time to reach supporters" value={stats.sendTime ? stats.sendTime.bestDay : 'Not enough data'} detail={stats.sendTime ? `around ${stats.sendTime.bestHourLabel}, from real gift timing` : 'activates after a few gifts come in'} onClick={() => go('send_time_insight', 'comms')}/>

        <MiniCard icon={<Trophy className="h-4 w-4 text-amber-500"/>} label="Top doorstep fundraisers" value={stats.topFundraisers[0]?.name ?? '—'} detail={stats.topFundraisers.length > 0
            ? stats.topFundraisers.map((f) => `${f.name} $${Math.round(f.totalCents / 100).toLocaleString()}`).join(' · ')
            : 'no gifts attributed to a canvasser yet'} onClick={() => go('doorstep_donations', 'turf')}/>

        <MiniCard icon={<Globe className="h-4 w-4 text-sky-600"/>} label="Language equity" value={stats.languages[0] ? `${stats.languages[0].language} ${stats.languages[0].pct}%` : 'English only on file'} detail={stats.languages[0]
            ? `${stats.languages[0].contacted} of ${stats.languages[0].total} ${stats.languages[0].language}-speaking voters contacted`
            : 'no non-English speakers logged yet'} onClick={() => go('field_coach', 'turf')}/>
      </div>

      {/* Where to push next — every line opens the tool that does it */}
      {actions.length > 0 && (<div className="rounded-lg border border-neutral-200 bg-white p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-neutral-500">
            Where to push next
          </p>
          <div className="mt-2 space-y-1">
            {actions.map((a) => (<button key={a.toolId} type="button" onClick={() => go(a.toolId, a.tab)} className="flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-sm text-neutral-700 hover:bg-neutral-50">
                <span>
...
function Progress({ label, value, detail, color }) {
    return (<div className="rounded-lg border border-neutral-200 bg-white p-4">
      <div className="flex items-baseline justify-between">
...
function MiniCard({ icon, label, value, detail, onClick }) {
    const Wrapper = onClick ? 'button' : 'div';
    return (<Wrapper type={onClick ? 'button' : undefined} onClick={onClick} className={`rounded-lg border border-neutral-200 bg-white p-4 text-left ${onClick ? 'hover:border-neutral-300 hover:bg-neutral-50' : ''}`}>
