...
export function DonorDedup({ orgId, projectId, donors }) {
    ...
    const donorName = (id) => donors?.find((d) => d.id === id)?.full_name ?? id;
    const run = async () => {
        if (!donorIdA || !donorIdB || donorIdA === donorIdB)
            return;
    ...
    return (<div className="space-y-3 rounded-lg border border-fuchsia-200 bg-white p-5">
      ...
      <div className="flex flex-wrap gap-2">
        <select className="h-9 flex-1 rounded-md border border-neutral-300 bg-white px-3 text-sm" value={donorIdA} onChange={(e) => setDonorIdA(e.target.value)}>
          <option value="">Record A…</option>
          {donors?.map((d) => (<option key={d.id} value={d.id}>
              {d.full_name}
            </option>))}
        </select>
        <select className="h-9 flex-1 rounded-md border border-neutral-300 bg-white px-3 text-sm" value={donorIdB} onChange={(e) => setDonorIdB(e.target.value)}>
      ...
      {assess.isError && <p className="text-sm text-red-600">{assess.error.message}</p>}

      {suggestions && suggestions.length > 0 && (<div className="space-y-2">
          {suggestions.map((s) => (<div key={s.id} className="space-y-1.5 rounded-md border border-fuchsia-100 bg-fuchsia-50 p-3">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={() => confirm.mutate({ suggestionId: s.id, donorIdA: s.donor_id_a, donorIdB: s.donor_id_b, orgId })} disabled={confirm.isPending}>
                  Merge into A
                </Button>
                <Button size="sm" variant="ghost" onClick={() => reject.mutate({ suggestionId: s.id, orgId })} disabled={reject.isPending}>
                  Not the same person
                </Button>
              </div>
            </div>))}
