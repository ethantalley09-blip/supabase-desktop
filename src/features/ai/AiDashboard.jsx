...
import { arrangeTools, reorderTools, TOOL_LOCATIONS } from '@/features/rbac/toolRegistry';
...
export function AiDashboard({ orgId, onOpenTool }) {
    ...
    const hide = (id) => persist(visible.map((t) => t.id).filter((v) => v !== id), [...(layout?.hidden ?? []), id]);
    const show = (id) => persist([...visible.map((t) => t.id), id], (layout?.hidden ?? []).filter((h) => h !== id));
    const open = (tool) => {
    ...
    if (tools.length === 0)
        return null;
    return (<div className="space-y-3 rounded-lg border border-neutral-200 bg-neutral-50 p-4">
      ...
      <div className="flex flex-wrap gap-1.5">
        <button type="button" onClick={() => setActiveCat('all')} className={`rounded-full px-3 py-1 text-xs font-medium ${activeCat === 'all' ? 'bg-neutral-900 text-white' : 'border border-neutral-300 bg-white text-neutral-600 hover:bg-neutral-100'}`}>
          All
        </button>
        {cats.map((c) => (<button key={c} type="button" onClick={() => setActiveCat(c)} className={`rounded-full px-3 py-1 text-xs font-medium ${activeCat === c ? 'bg-neutral-900 text-white' : 'border border-neutral-300 bg-white text-neutral-600 hover:bg-neutral-100'}`}>
            {CATEGORY_LABEL[c]}
          </button>))}
      </div>
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {shown.map((tool) => (<div key={tool.id} draggable onDragStart={() => setDragId(tool.id)} onDragOver={(e) => e.preventDefault()} onDrop={() => onDrop(tool.id)} onDragEnd={() => setDragId(null)} className={`group cursor-grab rounded-md border bg-white p-3 active:cursor-grabbing ${dragId === tool.id ? 'border-violet-400 opacity-60' : 'border-neutral-200 hover:border-neutral-300'}`}>
            <div className="flex items-start justify-between gap-1">
              ...
              <div className="flex shrink-0 items-center gap-1">
                <button type="button" title="Hide from my dashboard" onClick={() => hide(tool.id)} className="opacity-0 transition-opacity group-hover:opacity-100">
      ...
      {hiddenTools.length > 0 && (<div className="flex flex-wrap items-center gap-1.5">
          <span className="text-xs text-neutral-500">Hidden:</span>
          {hiddenTools.map((t) => (<button key={t.id} type="button" onClick={() => show(t.id)} className="inline-flex items-center gap-1 rounded-full border border-dashed border-neutral-300 px-2.5 py-0.5 text-xs text-neutral-500 hover:bg-white">
