...
// leads the list.
export function detectSilentCanvassers(visits, now = new Date()) {
    ...
    for (const v of visits) {
        ...
        if (t < todayStartMs || t > nowMs || t >= todayStartMs + DAY_MS)
            continue;
    ...
    for (const [canvasserId, list] of byCanvasser) {
        if (list.length < MIN_VISITS_TODAY)
            continue;
        const lastVisit = list.reduce((latest, v) => new Date(v.occurred_at).getTime() > new Date(latest.occurred_at).getTime() ? v : latest);
        const minutesSinceLastVisit = Math.round((nowMs - new Date(lastVisit.occurred_at).getTime()) / 60000);
        if (minutesSinceLastVisit < SILENCE_THRESHOLD_MINUTES)
            continue;
        alerts.push({
            canvasserId,
