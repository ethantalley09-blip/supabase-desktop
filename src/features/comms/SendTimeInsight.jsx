...
export function SendTimeInsight({ donations, orgId, projectId, aiEnabled }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      {!insight ? (<p className="text-sm text-neutral-500">
          ...
          <div className="flex h-16 items-end gap-0.5" role="img" aria-label="Donations by hour of day">
            {insight.hourlyCounts.map((c, h) => (<div key={h} title={`${h}:00 — ${c} gift${c === 1 ? '' : 's'}`} className={`flex-1 rounded-t ${h === insight.bestHour ? 'bg-neutral-900' : 'bg-neutral-200'}`} style={{ height: `${Math.max(4, (c / max) * 100)}%` }}/>))}
          </div>
          ...
          {tip.data && (<p className="flex items-start gap-1.5 border-t border-neutral-100 pt-2 text-xs text-violet-700">
