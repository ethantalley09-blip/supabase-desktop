...
import { useAddOpponentRecord, useArchiveOpponentRecord } from './useCompete';
const RECORD_TYPES = [
...
export function OpponentLog({ orgId, projectId, records }) {
    ...
    const submit = () => {
        if (!occurredOn || !content.trim())
            return;
        add.mutate({ orgId, projectId, recordType, occurredOn, source, content }, { onSuccess: () => { setContent(''); setSource(''); } });
    };
    return (<div className="space-y-3 rounded-lg border border-indigo-200 bg-white p-5">
      ...
      <div className="flex flex-wrap gap-2">
        <select className="h-9 rounded-md border border-neutral-300 bg-white px-3 text-sm" value={recordType} onChange={(e) => setRecordType(e.target.value)}>
          {RECORD_TYPES.map((t) => (<option key={t.value} value={t.value}>{t.label}</option>))}
        </select>
        <Input type="date" className="w-40" value={occurredOn} onChange={(e) => setOccurredOn(e.target.value)}/>
        <Input className="min-w-40 flex-1" placeholder="Source — outlet, URL, or event (e.g. Channel 4 debate)" value={source} onChange={(e) => setSource(e.target.value)}/>
      </div>
      <textarea rows={2} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" placeholder={'What did they say or do? e.g. "Said they would vote against the school levy renewal."'} value={content} onChange={(e) => setContent(e.target.value)}/>
      ...
      {records && records.length > 0 && (<div className="max-h-64 space-y-1.5 overflow-y-auto">
          {records.map((r) => (<div key={r.id} className="flex items-start justify-between gap-2 rounded-md bg-neutral-50 p-2.5">
              ...
              <button type="button" title="Archive" onClick={() => archive.mutate({ id: r.id, projectId })} className="shrink-0">
