...
type GrantEntitlementPayload = {
  ...
  key: string; // 'org_active' | 'fundraising_module' | 'comms_paid_tier' | ...
  granted: boolean;
  granted_reason: string; // 'webhook:stripe' | 'manual_admin' | ...
...
Deno.serve(async (req) => {
  ...
  const payload = (await req.json()) as GrantEntitlementPayload;
