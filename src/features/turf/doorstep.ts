...
export type DoorLike = {
  ...
  data: { city?: string; ward?: string; language?: string } | null;
};

export type WarmDoor = {
  voterId: string;
  name: string;
  address: string;
  score: number;      // 0-100
  reasons: string[];  // human-readable, shown to the walker
  language?: string;
};
...
const NOTE_SIGNALS: { phrase: string; points: number; reason: string }[] = [
...
export function scoreDoors(voters: DoorLike[], limit = 12): WarmDoor[] {
  const scored: WarmDoor[] = [];
  for (const v of voters) {
    if (v.contact_status === 'moved') continue;
    let score = 0;
    ...
    for (const s of NOTE_SIGNALS) {
      if (notes.includes(s.phrase)) {
        score += s.points;
        if (s.points > 0 && s.reason) reasons.push(s.reason);
      }
    }
...
export function canvasserLeaderboard(
  donations: { recorded_by?: string | null; amount_cents: number; recorder?: { full_name: string | null; email: string | null } | null }[]
): LeaderboardRow[] {
  const byRecorder = new Map<string, LeaderboardRow>();
  for (const d of donations) {
    if (!d.recorded_by) continue;
