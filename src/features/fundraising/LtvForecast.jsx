...
export function LtvForecast({ orgId, projectId, donors }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-teal-200 bg-white p-5">
      ...
      {forecast.isError && <p className="text-sm text-red-600">{forecast.error.message}</p>}

      {cultivateList && cultivateList.length > 0 && (<div className="rounded-md border border-teal-100 bg-teal-50 p-3">
          <p className="text-sm font-medium text-teal-900">
            {cultivateList.length} donor{cultivateList.length === 1 ? '' : 's'} flagged worth cultivating
          </p>
          <div className="mt-1 space-y-1">
            {cultivateList.slice(0, 5).map((c) => (<p key={c.id} className="text-xs text-teal-800">
