...
import { useOpponentDigest } from './useCompete';
...
export function OpponentDigest({ orgId, projectId, records }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      <Button size="sm" onClick={() => records && digest.mutate({ orgId, projectId, records })} disabled={digest.isPending || !enough}>
        <Sparkles className="h-4 w-4"/>
        {digest.isPending ? 'Analyzing…' : enough ? 'Analyze their messaging' : 'Log at least 3 entries first'}
      </Button>
      {digest.isError && <p className="text-sm text-red-600">{digest.error.message}</p>}

      {digest.data && (<div className="space-y-2">
          {digest.data.themes.map((t) => (<div key={t.theme} className="rounded-md bg-neutral-50 p-3">
