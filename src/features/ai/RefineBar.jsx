...
export function RefineBar({ orgId, projectId, text, onResult }) {
    const refine = useAiAssist();
    const [custom, setCustom] = useState('');
    const run = (instruction) => {
        if (!instruction.trim() || !text.trim())
            return;
        refine.mutate({ orgId, projectId, purpose: 'refine', context: text, instructions: instruction }, { onSuccess: (data) => onResult?.(data.text) });
    };
    return (<div className="space-y-2">
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {PRESETS.map((p) => (<button key={p.label} type="button" onClick={() => run(p.instruction)} disabled={refine.isPending} className="rounded-full border border-neutral-200 px-2.5 py-1 text-xs text-neutral-600 hover:bg-neutral-50 disabled:opacity-50">
            {p.label}
          </button>))}
      </div>
      <div className="flex gap-2">
        <Input placeholder="…or describe a change (e.g. add a call to volunteer Saturday)" value={custom} onChange={(e) => setCustom(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && run(custom)}/>
        <Button variant="outline" size="sm" onClick={() => run(custom)} disabled={refine.isPending || !custom.trim()}>
          {refine.isPending ? 'Refining…' : 'Refine'}
        </Button>
      </div>
      {refine.isError && <p className="text-sm text-red-600">{refine.error.message}</p>}
      {refine.data && !onResult && (<div className="whitespace-pre-wrap rounded-md border border-neutral-200 bg-neutral-50 p-3 text-sm text-neutral-800">
          {refine.data.text}
        </div>)}
    </div>);
}
