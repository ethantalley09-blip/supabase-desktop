...
export function RefundWatchdog({ orgId, projectId }: { orgId: string; projectId: string }) {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-slate-200 bg-white p-5">
      ...
      {logAndScan.isError && <p className="text-sm text-red-600">{(logAndScan.error as Error).message}</p>}

      {(logAndScan.data || latest) && (
        <div className={`rounded-md border p-3 ${RISK_STYLE[(logAndScan.data ?? latest)!.risk_level] ?? RISK_STYLE.normal}`}>
