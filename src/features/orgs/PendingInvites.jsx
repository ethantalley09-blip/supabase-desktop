...
export function PendingInvites() {
    ...
    return (<div className="mb-6 space-y-2">
      {invites.map((inv) => (<div key={inv.membership_id} className="flex items-center justify-between rounded-lg border border-blue-200 bg-blue-50 p-4">
          ...
          <div className="flex gap-2">
            <Button size="sm" disabled={resolve.isPending} onClick={() => resolve.mutate({ membershipId: inv.membership_id, accept: true })}>
              Accept
            </Button>
            <Button variant="outline" size="sm" disabled={resolve.isPending} onClick={() => resolve.mutate({ membershipId: inv.membership_id, accept: false })}>
              Decline
            </Button>
          </div>
        </div>))}
