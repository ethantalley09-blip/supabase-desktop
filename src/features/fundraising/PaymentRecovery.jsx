...
export function PaymentRecovery({ orgId, projectId, donors }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-cyan-200 bg-white p-5">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <select className="h-9 rounded-md border border-neutral-300 bg-white px-3 text-sm" value={failureType} onChange={(e) => setFailureType(e.target.value)}>
          {FAILURE_TYPES.map((f) => (<option key={f.value} value={f.value}>
              {f.label}
            </option>))}
        </select>
      ...
      {pending && pending.length > 0 && (<div className="rounded-md border border-cyan-100 bg-cyan-50 p-3">
