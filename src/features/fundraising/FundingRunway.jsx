...
import { useLatestRunwayPlan, useRunwayAnalysis } from './useGrowthAi';
...
export function FundingRunway({ orgId, projectId, donations }) {
    ...
    const runway = useMemo(() => computeRunway(Math.round(Number(cash || 0) * 100), Math.round(Number(burn || 0) * 100), dailyRaiseCents, expenses), [cash, burn, dailyRaiseCents, expenses]);
    const addExpense = () => {
        if (!expLabel.trim() || !expDate || !Number(expAmount))
            return;
    ...
    const strategies = analyze.data?.strategies ?? lastPlan?.strategies ?? null;
    return (<div className="space-y-3 rounded-lg border border-rose-200 bg-white p-5">
      ...
      <div className="space-y-2 rounded-md border border-neutral-200 p-3">
        ...
        {expenses.map((e, i) => (<div key={`${e.date}-${i}`} className="flex items-center justify-between text-xs text-neutral-600">
            <span>
      ...
      {runway.shortfallDate ? (<div className="rounded-md border-l-4 border-rose-600 bg-rose-50 p-3">
        ...
        </div>) : (<div className="rounded-md border-l-4 border-emerald-500 bg-emerald-50 p-3">
      ...
      {analyze.isError && <p className="text-sm text-red-600">{analyze.error.message}</p>}

      {strategies && (<div className="space-y-2">
          {[
                ['A', strategies.strategy_a],
                ['B', strategies.strategy_b]
            ].map(([label, s]) => (<div key={label} className="rounded-md border border-blue-100 bg-blue-50 p-3">
