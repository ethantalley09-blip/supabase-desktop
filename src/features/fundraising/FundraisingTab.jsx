...
import { DATE_RANGE_PRESETS, inRange, resolvePreset } from '@/lib/dates/dateRange';
import { COMPLIANCE_THRESHOLD_CENTS, formatUsd, useDonations, useDonationTotal, useOrgDonors, useRecordDonation } from './useFundraising';
...
export function FundraisingTab({ project }) {
    ...
    return (<div className="space-y-5">
      <div className="rounded-lg border border-neutral-200 bg-white p-5">
        <div className="flex items-baseline justify-between">
          ...
          {canManage.data && (<Button size="sm" onClick={() => setShowForm(!showForm)}>
        ...
        <div className="mt-4">
          ...
          <div className="mt-1 h-2 overflow-hidden rounded-full bg-neutral-100">
            <div className={thresholdMet ? 'h-full bg-emerald-500' : 'h-full bg-neutral-800'} style={{ width: `${pct}%` }}/>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <select className="h-8 rounded-md border border-neutral-300 bg-white px-2 text-sm" value={rangePreset} onChange={(e) => setRangePreset(e.target.value)}>
          {DATE_RANGE_PRESETS.map((p) => (<option key={p.id} value={p.id}>
              {p.label}
            </option>))}
        </select>
      ...
      {chartData.length > 0 && (<div className="rounded-lg border border-neutral-200 bg-white p-5">
      ...
      <div className="overflow-hidden rounded-lg border border-neutral-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
            {visibleDonations.map((d) => (<tr key={d.id} className="border-t border-neutral-100">
...
function DonorMessageStudio({ project }) {
    ...
    if (!canUseAi.data || !entitlement.data)
        return null;
    const generate = () => {
    ...
    return (<div className="space-y-3 rounded-lg border border-violet-200 bg-white p-5">
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="space-y-1.5">
          <Label>Type</Label>
          <select className="flex h-9 rounded-md border border-neutral-300 bg-white px-3 text-sm" value={kind} onChange={(e) => setKind(e.target.value)}>
            {DONOR_KINDS.map((k) => (<option key={k.value} value={k.value}>
                {k.label}
              </option>))}
          </select>
        </div>
        <div className="space-y-1.5">
          <Label>Tone</Label>
          <select className="flex h-9 rounded-md border border-neutral-300 bg-white px-3 text-sm" value={tone} onChange={(e) => setTone(e.target.value)}>
            {DONOR_TONES.map((t) => (<option key={t} value={t}>
                {t}
              </option>))}
          </select>
        </div>
      </div>
      <textarea rows={2} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" placeholder="Anything specific? e.g. Thank Maria for her $50 gift toward the field program." value={brief} onChange={(e) => setBrief(e.target.value)}/>
...
function DonationForm({ project, onDone }) {
    const recordDonation = useRecordDonation();
    const { data: donors } = useOrgDonors(project.org_id);
    const [existingDonorId, setExistingDonorId] = useState('');
    ...
    return (<form className="space-y-4 rounded-lg border border-neutral-200 bg-white p-5" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="space-y-1.5">
        <Label>Donor</Label>
        {donors && donors.length > 0 && (<select className="flex h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm" value={existingDonorId} onChange={(e) => setExistingDonorId(e.target.value)}>
      ...
      {!existingDonorId && (<div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
