// "Ask your data"'s fixed question bank, translated into deterministic
// data queries over aggregates the app already computes (TurfSnapshot,
// FundraisingSnapshot, GeocodeHealth) — so each one renders instantly with
// real project-specific evidence instead of waiting on an AI round trip.
// Free-text questions in AiCenterTab.tsx still go through the `data_qa` AI
// purpose unchanged; this module only covers the fixed bank. No Supabase
// import (pattern: route.ts, doorstep.ts) — see cannedAnswers.test.ts.
import type { FundraisingSnapshot } from '@/features/fundraising/fundraisingSnapshot';
import type { GeocodeHealth } from '@/features/turf/geocodeHealth';
...
export function answerBallotsOutstanding(turf: TurfSnapshot): CannedAnswer {
...
export function answerComplianceProgress(fundraising: FundraisingSnapshot | undefined): CannedAnswer {
...
export function answerTopCity(turf: TurfSnapshot): CannedAnswer {
...
export function answerGeocodingBacklog(geocode: GeocodeHealth): CannedAnswer {
...
export function answerVoterUniverse(turf: TurfSnapshot): CannedAnswer {
...
export function answerContactsLogged(turf: TurfSnapshot): CannedAnswer {
...
export function answerUnassignedTerritories(turf: TurfSnapshot): CannedAnswer {
...
export function answerFundraisingTotal(fundraising: FundraisingSnapshot | undefined): CannedAnswer {
...
export function answerAverageDonation(fundraising: FundraisingSnapshot | undefined): CannedAnswer {
...
export function buildCannedAnswers(input: {
  turf: TurfSnapshot;
  fundraising: FundraisingSnapshot | undefined;
  geocode: GeocodeHealth;
}): CannedAnswer[] {
  return [
