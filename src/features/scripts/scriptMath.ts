...
export type ScriptKind = 'door_script' | 'survey_question';

export type ScriptEntryLike = {
  kind: ScriptKind;
  content: string;
  sortOrder: number;
  active: boolean;
...
export function currentDoorScript(entries: ScriptEntryLike[]): string | null {
  const active = entries.filter((e) => e.kind === 'door_script' && e.active);
  return active.length > 0 ? active[active.length - 1].content : null;
}

export function currentSurveyQuestions(entries: ScriptEntryLike[]): string[] {
  return entries
