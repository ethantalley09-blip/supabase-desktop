...
export function RedTeam({ orgId, projectId }: { orgId: string; projectId: string }) {
  const red = useRedTeam();
  const [record, setRecord] = useState('');

  const badge = (likelihood: string) =>
    likelihood === 'high'
  ...
  return (
    <div className="space-y-3 rounded-lg border border-neutral-300 bg-white p-5">
      ...
      <textarea
        ...
        placeholder="e.g. Council member 6 years. Voted for the 2023 budget that raised parking fines. Missed 4 votes last spring (family illness). Small business owner, two late tax filings in 2019, both settled."
        value={record}
        onChange={(e) => setRecord(e.target.value)}
      />
      <Button
        size="sm"
        onClick={() => red.mutate({ orgId, projectId, ownRecord: record.trim() })}
        disabled={red.isPending || record.trim().length < 20}
      >
        <Sparkles className="h-4 w-4" />
        {red.isPending ? 'Attacking…' : 'Run the red team'}
      </Button>
      {red.isError && <p className="text-sm text-red-600">{(red.error as Error).message}</p>}

      {red.data && (
        <div className="space-y-2">
          {red.data.vulnerabilities.map((v, i) => (
