...
import { dominantVoterLanguage, optimizeWalkOrder, type WalkRoute } from './route';
...
import { DEFAULT_TURF_PREFERENCES, type HighlightMode, type TurfPreferences } from './turfPreferences';
import {
...
type Briefing = {
  headline: string;
  focus_areas: { area: string; why: string; action: string }[];
  watch_out: string;
};

type Objection = { acknowledge: string; bridge: string; pivot: string };
type DoorScript = { opener: string; key_points: string[]; sign_off: string };
type DoorExplain = { summary: string };
type LanguagePrep = { greeting: string; key_phrase: string; respectful_note: string };
type ShiftDebrief = { headline: string; wins: string[]; follow_up_tomorrow: string[] };
type TerritoryBriefing = { ranked: { territory: string; difficulty_note: string; staffing_advice: string }[] };
type RevisitStrategy = { verdict: string; suggestion: string };
type AskScript = { opener: string; ask: string; suggested_ask_dollars: number };
type PeakAskBriefing = { headline: string; staffing_advice: string };
type TerritoryRoiBriefing = { ranked: { territory: string; roi_note: string; staffing_advice: string }[] };
type GoldenHourPlan = { headline: string; advice: string };
type AskCoverageAlert = { alert: string; suggested_next_step: string };
type CoachingNote = { observation: string; coaching_tip: string };
type PriorityDoorBriefing = { headline: string; doors: { name: string; instruction: string }[] };
type CheckinPrompt = { checkin_message: string };
type TerritoryStaffingBriefing = { ranked: { suggestion: string; reason: string }[] };
type NeighborPitch = { pitch: string; if_yes: string; if_no: string; suggested_ask_dollars: number };
type DonationObjectionResult = { acknowledge: string; pivot: string; follow_up_text: string };
type RehearsalPrep = { confidence_opener: string; anticipated_questions: { question: string; answer: string }[] };
...
const DOOR_PRIORITY: Record<string, number> = { persuadable: 0, unknown: 1, base_support: 2, opposed: 3 };
...
export function TurfBriefing({
  ...
  canUseAi: boolean;
  onRebalance: (route: WalkRoute) => void;
  // Live Hotspot Caller reads whichever heatmap mode is active on the map
  // itself, so its words always agree with what's actually drawn.
  heatmapMode?: 'off' | HeatmapMode;
  // Click-to-Ask Door Popup: set the instant a captain clicks any pin on
  // the map, overriding Door Prep's own auto-picked top-priority door.
  mapSelectedVoterId?: string | null;
  ...
  const [moneyRouteNote, setMoneyRouteNote] = useState<string | null>(null);
  const [goalInput, setGoalInput] = useState('');
  const [objectionText, setObjectionText] = useState('');
  const [objectionResult, setObjectionResult] = useState<Objection | null>(null);
  const [doorScript, setDoorScript] = useState<DoorScript | null>(null);
  const [doorExplain, setDoorExplain] = useState<DoorExplain | null>(null);
  const [languagePrep, setLanguagePrep] = useState<LanguagePrep | null>(null);
  const [shiftDebrief, setShiftDebrief] = useState<ShiftDebrief | null>(null);
  const [territoryBriefing, setTerritoryBriefing] = useState<TerritoryBriefing | null>(null);
  const [revisitStrategy, setRevisitStrategy] = useState<RevisitStrategy | null>(null);
  const [momentumFor, setMomentumFor] = useState<string | null>(null);
  const [momentumScript, setMomentumScript] = useState<AskScript | null>(null);
  const [peakAskBriefing, setPeakAskBriefing] = useState<PeakAskBriefing | null>(null);
  const [territoryRoiBriefing, setTerritoryRoiBriefing] = useState<TerritoryRoiBriefing | null>(null);
  const [persistenceFor, setPersistenceFor] = useState<string | null>(null);
  const [persistenceScript, setPersistenceScript] = useState<AskScript | null>(null);
  const [goldenHourPlan, setGoldenHourPlan] = useState<GoldenHourPlan | null>(null);
  const [coverageGapFor, setCoverageGapFor] = useState<string | null>(null);
  const [coverageAlert, setCoverageAlert] = useState<AskCoverageAlert | null>(null);
  ...
  const [countdownScript, setCountdownScript] = useState<AskScript | null>(null);
  const [priorityDoorBriefing, setPriorityDoorBriefing] = useState<PriorityDoorBriefing | null>(null);
  ...
  const [territoryStaffingBriefing, setTerritoryStaffingBriefing] = useState<TerritoryStaffingBriefing | null>(null);
  const [silenceFor, setSilenceFor] = useState<string | null>(null);
  const [silencePrompt, setSilencePrompt] = useState<CheckinPrompt | null>(null);
  const [neighborProofFor, setNeighborProofFor] = useState<string | null>(null);
  const [neighborProofPitch, setNeighborProofPitch] = useState<NeighborPitch | null>(null);
  const [donationObjectionText, setDonationObjectionText] = useState('');
  const [donationObjectionResult, setDonationObjectionResult] = useState<DonationObjectionResult | null>(null);
  const [rehearsalPrep, setRehearsalPrep] = useState<RehearsalPrep | null>(null);
  const [customizeOpen, setCustomizeOpen] = useState(false);
  const [draft, setDraft] = useState<TurfPreferences>(DEFAULT_TURF_PREFERENCES);
  ...
  const snapshot = useMemo(
    () => buildBriefingSnapshot(voters, territories, undefined, { recentWindowMs, todayWindowMs }),
    [voters, territories, recentWindowMs, todayWindowMs]
  );
  const remainingDoors = useMemo(
    () => remainingDoorsToday(voters, undefined, { todayWindowMs }),
    [voters, todayWindowMs]
  );
  ...
  const neighborProof = useMemo(
    () => findNeighborhoodProof(warmDoors, voters, donations ?? []),
    [warmDoors, voters, donations]
  );
  ...
  const bestTime = useMemo(
    () =>
      computeBestTimeToKnock(bestTimeVisits, {
  ...
  const driftAlerts = useMemo(
    () =>
      detectPersuasionDrift(driftVisits)
        .filter((d) => (d.direction === 'warmed' ? prefs.persuasionDrift.showWarmed : prefs.persuasionDrift.showCooled))
        .slice(0, 5),
    [driftVisits, prefs.persuasionDrift.showWarmed, prefs.persuasionDrift.showCooled]
  );
  ...
  const momentumTargets = useMemo(
    () => findMomentumAskTargets(allDriftAlerts, donations ?? []).slice(0, 5),
    [allDriftAlerts, donations]
  );
  ...
  const daylightUrgent = Boolean(
    daylight && daylight.minutesOfDaylightLeft > 0 && daylight.minutesOfDaylightLeft <= prefs.daylight.warnMinutes
  );
  ...
  const goalProgress = useMemo(
    () => computeGoalProgress(donations ?? [], goalCents, undefined, daylight?.minutesOfDaylightLeft ?? null),
    [donations, goalCents, daylight]
  );
  ...
  const goldenHourTargets = useMemo(
    () => (daylightUrgent ? findGoldenHourTargets(warmDoors, remainingDoorIds, donations ?? []) : []),
    [daylightUrgent, warmDoors, remainingDoorIds, donations]
  );

  const territoryDifficulty = useMemo(
    () => computeTerritoryDifficulty(voters, visits ?? [], territories),
    [voters, visits, territories]
  );
  ...
  const territoryRoi = useMemo(
    () => computeTerritoryFundraisingRoi(voters, visits ?? [], donations ?? [], territories),
    [voters, visits, donations, territories]
  );
  ...
  const territoryStaffing = useMemo(
    () => computeTerritoryStaffing(voters, visits ?? [], territories),
    [voters, visits, territories]
  );
  ...
  const canvasserLeaderboard = useMemo(
    () => computeCanvasserLeaderboard(visits ?? [], { minAttempts: prefs.canvasserLeaderboard.minAttempts }),
    [visits, prefs.canvasserLeaderboard.minAttempts]
  );
  ...
  const askCoachStats = useMemo(
    () => computeCanvasserAskCoach(canvasserLeaderboard, donationLeaderboard).slice(0, 5),
    [canvasserLeaderboard, donationLeaderboard]
  );

  const allHouseholds = useMemo(() => groupIntoHouseholds(voters), [voters]);
  const overlapAlerts = useMemo(
    () =>
      detectCrossCanvasserOverlap(allHouseholds, visits ?? [], { lookbackDays: prefs.overlapGuard.lookbackDays }).slice(
        0,
        5
      ),
    [allHouseholds, visits, prefs.overlapGuard.lookbackDays]
  );
  ...
  const askCoverageGaps = useMemo(
    () =>
      findAskCoverageGaps(allHouseholds, visits ?? [], warmDoors, donations ?? [], {
        lookbackDays: prefs.overlapGuard.lookbackDays
      }).slice(0, 5),
    [allHouseholds, visits, warmDoors, donations, prefs.overlapGuard.lookbackDays]
  );
  const revisitQueue = useMemo(
    () => buildRevisitQueue(remainingDoors, visits ?? [], { minAttempts: prefs.revisitQueue.minAttempts }).slice(0, 5),
    [remainingDoors, visits, prefs.revisitQueue.minAttempts]
  );
  ...
  const persistenceTargets = useMemo(
    () => findPersistenceAskTargets(visits ?? [], donations ?? []).slice(0, 5),
    [visits, donations]
  );
  ...
  const countdownTargets = useMemo(
    () => (electionDate ? findCountdownAskTargets(warmDoors, donations ?? [], electionDate) : []),
    [electionDate, warmDoors, donations]
  );
  ...
  const priorityDoorInputs = useMemo(
    () =>
      remainingDoors.map((v) => ({
        id: v.id,
  ...
  const priorityDoors = useMemo(
    () => findPriorityDoors(priorityDoorInputs, revisitQueue, countdownDays),
    [priorityDoorInputs, revisitQueue, countdownDays]
  );
  ...
  const mapSelectedDoor = useMemo(
    () => (mapSelectedVoterId ? voters.find((v) => v.id === mapSelectedVoterId) ?? null : null),
    [mapSelectedVoterId, voters]
  );
  const topDoor = useMemo(() => {
    if (mapSelectedDoor) return mapSelectedDoor;
    const sorted = [...remainingDoors].sort(
      (a, b) => DOOR_PRIORITY[classifyPersuadability(a).bucket] - DOOR_PRIORITY[classifyPersuadability(b).bucket]
    );
    return sorted[0] ?? null;
  }, [mapSelectedDoor, remainingDoors]);
  const remainingLanguage = useMemo(() => dominantVoterLanguage(remainingDoors), [remainingDoors]);
  const topDoorVisits = useMemo(
    () => (visits ?? []).filter((v) => v.voter_id === topDoor?.id).slice(0, 5),
    [visits, topDoor]
  );
  ...
  const hotspot = useMemo(
    () => (heatmapMode === 'off' ? null : findHotspot(voters, heatmapMode)),
    [voters, heatmapMode]
  );
  ...
  const generateBriefing = () => {
    ...
    ai.mutate(
      ...
      { onSuccess: (res) => setBriefing(extractJson<Briefing>(res.text)) }
    );
  };
  ...
  const generateDebrief = () => {
    ...
    debriefAi.mutate(
      ...
      { onSuccess: (res) => setShiftDebrief(extractJson<ShiftDebrief>(res.text)) }
    );
  };
  ...
  const generateTerritoryBriefing = () => {
    ...
    territoryAi.mutate(
      { orgId, projectId, purpose: 'territory_difficulty_briefing', context: JSON.stringify(territoryDifficulty) },
      { onSuccess: (res) => setTerritoryBriefing(extractJson<TerritoryBriefing>(res.text)) }
    );
  };
  ...
  const getDoorScript = () => {
    ...
    scriptAi.mutate(
      ...
      { onSuccess: (res) => setDoorScript(extractJson<DoorScript>(res.text)) }
    );
  };
  ...
  const getDoorExplain = () => {
    ...
    explainAi.mutate(
      ...
      { onSuccess: (res) => setDoorExplain(extractJson<DoorExplain>(res.text)) }
    );
  };

  const getLanguagePrep = () => {
    if (!remainingLanguage) return;
    setLanguagePrep(null);
    languageAi.mutate(
      { orgId, projectId, purpose: 'door_language_prep', context: JSON.stringify({ language: remainingLanguage }) },
      { onSuccess: (res) => setLanguagePrep(extractJson<LanguagePrep>(res.text)) }
    );
  };

  const getRevisitStrategy = () => {
    if (!topRevisit) return;
    ...
    revisitAi.mutate(
      ...
      { onSuccess: (res) => setRevisitStrategy(extractJson<RevisitStrategy>(res.text)) }
    );
  };

  const getMomentumAsk = (target: (typeof momentumTargets)[number]) => {
    ...
    momentumAi.mutate(
      ...
      { onSuccess: (res) => setMomentumScript(extractJson<AskScript>(res.text)) }
    );
  };

  const getPeakAskBriefing = () => {
    if (!peakAskWindow) return;
    setPeakAskBriefing(null);
    peakAskAi.mutate(
      { orgId, projectId, purpose: 'peak_ask_briefing', context: JSON.stringify(peakAskWindow) },
      { onSuccess: (res) => setPeakAskBriefing(extractJson<PeakAskBriefing>(res.text)) }
    );
  };

  const getTerritoryRoiBriefing = () => {
    if (territoryRoi.length === 0) return;
    setTerritoryRoiBriefing(null);
    territoryRoiAi.mutate(
      { orgId, projectId, purpose: 'territory_roi_briefing', context: JSON.stringify(territoryRoi) },
      { onSuccess: (res) => setTerritoryRoiBriefing(extractJson<TerritoryRoiBriefing>(res.text)) }
    );
  };

  const getPersistenceAsk = (target: (typeof persistenceTargets)[number]) => {
    ...
    persistenceAi.mutate(
      ...
      { onSuccess: (res) => setPersistenceScript(extractJson<AskScript>(res.text)) }
    );
  };

  const getGoldenHourPlan = () => {
    if (goldenHourTargets.length === 0 || !daylight) return;
    ...
    goldenHourAi.mutate(
      ...
      { onSuccess: (res) => setGoldenHourPlan(extractJson<GoldenHourPlan>(res.text)) }
    );
  };

  const getCoverageAlert = (gap: (typeof askCoverageGaps)[number]) => {
    ...
    coverageGapAi.mutate(
      ...
      { onSuccess: (res) => setCoverageAlert(extractJson<AskCoverageAlert>(res.text)) }
    );
  };

  const getCoachingNote = (stat: (typeof askCoachStats)[number]) => {
    ...
    coachAi.mutate(
      ...
      { onSuccess: (res) => setCoachingNote(extractJson<CoachingNote>(res.text)) }
    );
  };

  const getCountdownAsk = (target: (typeof countdownTargets)[number]) => {
    ...
    countdownAi.mutate(
      ...
      { onSuccess: (res) => setCountdownScript(extractJson<AskScript>(res.text)) }
    );
  };

  const getPriorityDoorBriefing = () => {
    if (priorityDoors.length === 0) return;
    ...
    priorityDoorAi.mutate(
      ...
      { onSuccess: (res) => setPriorityDoorBriefing(extractJson<PriorityDoorBriefing>(res.text)) }
    );
  };

  const getCheckinPrompt = (alert: (typeof fatigueAlerts)[number]) => {
    ...
    checkinAi.mutate(
      ...
      { onSuccess: (res) => setCheckinPrompt(extractJson<CheckinPrompt>(res.text)) }
    );
  };
  ...
  const getTerritoryStaffingBriefing = () => {
    ...
    territoryStaffingAi.mutate(
      { orgId, projectId, purpose: 'territory_staffing_briefing', context: JSON.stringify(reallocations) },
      { onSuccess: (res) => setTerritoryStaffingBriefing(extractJson<TerritoryStaffingBriefing>(res.text)) }
    );
  };

  const getSilenceCheckin = (alert: (typeof silentCanvassers)[number]) => {
    ...
    silenceAi.mutate(
      ...
      { onSuccess: (res) => setSilencePrompt(extractJson<CheckinPrompt>(res.text)) }
    );
  };

  const getNeighborProofPitch = (door: (typeof neighborProof)[number]) => {
    ...
    neighborProofAi.mutate(
      ...
      { onSuccess: (res) => setNeighborProofPitch(extractJson<NeighborPitch>(res.text)) }
    );
  };

  const askDonationObjection = () => {
    if (!donationObjectionText.trim()) return;
    setDonationObjectionResult(null);
    donationObjectionAi.mutate(
      { orgId, projectId, purpose: 'donation_objection_handler', instructions: donationObjectionText.trim() },
      { onSuccess: (res) => setDonationObjectionResult(extractJson<DonationObjectionResult>(res.text)) }
    );
  };

  const getRehearsalPrep = () => {
    if (!askRehearsalSnapshot) return;
    setRehearsalPrep(null);
    rehearsalAi.mutate(
      { orgId, projectId, purpose: 'ask_rehearsal_prep', context: JSON.stringify(askRehearsalSnapshot) },
      { onSuccess: (res) => setRehearsalPrep(extractJson<RehearsalPrep>(res.text)) }
    );
  };
  ...
  const askObjection = () => {
    ...
    objectionAi.mutate(
      { orgId, projectId, purpose: 'door_objection_assist', instructions: objectionText.trim() },
      { onSuccess: (res) => setObjectionResult(extractJson<Objection>(res.text)) }
    );
  };
  ...
  const rebalance = () => {
    ...
    setRebalanceNote(
      remaining.length === remainingDoors.length
  ...
  return (
    <div className="space-y-3 rounded-lg border border-indigo-200 bg-white p-5">
      ...
      {snapshot.topRemainingTerritories.length > 0 && (
        <p className="text-xs text-neutral-500">
      ...
      {daylight && (
        <div
          className={`space-y-1 rounded-md border px-3 py-1.5 text-xs ${
            daylightUrgent ? 'border-rose-200 bg-rose-50 text-rose-800' : 'border-amber-200 bg-amber-50 text-amber-800'
          }`}
        >
      ...
      {hotspot && (
        <div className="space-y-1 rounded-md border border-orange-200 bg-orange-50 px-3 py-1.5 text-xs text-orange-900">
          <p className="flex items-center gap-1.5">
      ...
      <div className="space-y-2 rounded-md border border-emerald-300 bg-emerald-50/60 p-2.5">
        ...
        {goalCents > 0 ? (
          ...
          <p className="text-xs text-neutral-500">Set a real dollar goal for today's shift to track live progress.</p>
        )}
      </div>

      {goldenHourTargets.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-yellow-300 bg-yellow-50 p-2.5">
          <div className="flex items-center justify-between gap-2">
          ...
          <div className="space-y-1 text-xs text-neutral-700">
            {goldenHourTargets.map((d) => (
              <p key={d.voterId}>
                {d.name} · {d.address} — {d.reasons.join(' · ')}
              </p>
            ))}
          </div>
          {goldenHourAi.isError && <p className="text-sm text-red-600">{(goldenHourAi.error as Error).message}</p>}
          {goldenHourPlan && (
            <div className="space-y-1 rounded bg-white p-2.5 text-neutral-700">
      ...
      {bestTime && (
        <div className="space-y-1 rounded-md border border-sky-200 bg-sky-50 px-3 py-1.5 text-xs text-sky-800">
          <p className="flex items-center gap-1.5">
      ...
      {peakAskWindow && (
        <div className="space-y-2 rounded-md border border-green-200 bg-green-50 p-2.5 text-xs text-green-900">
          <div className="flex items-center justify-between gap-2">
            ...
            {canUseAi && (
              <Button size="sm" variant="outline" onClick={getPeakAskBriefing} disabled={peakAskAi.isPending}>
                <Sparkles className="h-3.5 w-3.5" />
                {peakAskAi.isPending ? '…' : 'Get staffing takeaway'}
              </Button>
            )}
          </div>
          {peakAskAi.isError && <p className="text-sm text-red-600">{(peakAskAi.error as Error).message}</p>}
          {peakAskBriefing && (
            <div className="space-y-1 rounded bg-white p-2.5 text-neutral-700">
      ...
      {driftAlerts.length > 0 && (
        <div className="space-y-1 rounded-md border border-neutral-200 p-2.5">
          <p className="text-xs font-semibold uppercase tracking-wide text-neutral-500">Persuasion drift</p>
          {driftAlerts.map((d) => (
            <div key={`${d.voterId}-${d.occurredAt}`} className="flex items-center gap-1.5 text-xs">
              {d.direction === 'warmed' ? (
                <ArrowUpRight className="h-3.5 w-3.5 shrink-0 text-emerald-600" />
              ) : (
                <ArrowDownRight className="h-3.5 w-3.5 shrink-0 text-rose-600" />
              )}
      ...
      {askRehearsalSnapshot && canUseAi && (
        <div className="space-y-2 rounded-md border border-fuchsia-200 bg-fuchsia-50 p-2.5">
          <div className="flex items-center justify-between gap-2">
          ...
          {rehearsalAi.isError && <p className="text-sm text-red-600">{(rehearsalAi.error as Error).message}</p>}
          {rehearsalPrep && (
            <div className="space-y-1.5 rounded bg-white p-2.5 text-xs text-neutral-700">
              <p className="font-medium text-fuchsia-900">{rehearsalPrep.confidence_opener}</p>
              {rehearsalPrep.anticipated_questions.map((q, i) => (
                <div key={i}>
      ...
      {momentumTargets.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-emerald-200 bg-emerald-50 p-2.5">
          ...
          {momentumTargets.map((t) => (
            <div key={t.voterId} className="space-y-1 rounded bg-white p-2 text-xs">
              ...
              {momentumScript && momentumFor === t.voterId && (
                <div className="space-y-1 rounded bg-emerald-50 p-2 text-neutral-700">
          ...
          {momentumAi.isError && <p className="text-sm text-red-600">{(momentumAi.error as Error).message}</p>}
        </div>
      )}

      {neighborProof.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-sky-200 bg-sky-50 p-2.5">
          ...
          {neighborProof.map((d) => (
            <div key={d.voterId} className="space-y-1 rounded bg-white p-2 text-xs">
              <div className="flex items-center justify-between gap-2">
                <span className="text-neutral-700">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => getNeighborProofPitch(d)}
                  disabled={neighborProofAi.isPending}
                >
                  <Sparkles className="h-3.5 w-3.5" />
                  {neighborProofAi.isPending && neighborProofFor === d.voterId ? '…' : 'Get pitch'}
                </Button>
              </div>
              {neighborProofPitch && neighborProofFor === d.voterId && (
                <div className="space-y-1 rounded bg-sky-50 p-2 text-neutral-700">
          ...
          {neighborProofAi.isError && <p className="text-sm text-red-600">{(neighborProofAi.error as Error).message}</p>}
        </div>
      )}

      {canvasserLeaderboard.length > 0 && (
        <div className="space-y-1 rounded-md border border-neutral-200 p-2.5">
          ...
          {canvasserLeaderboard.slice(0, 5).map((c, i) => (
            <div key={c.canvasserId} className="flex items-center justify-between text-xs text-neutral-700">
              <span>
                {i + 1}. {c.name}
              </span>
      ...
      {askCoachStats.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-cyan-200 bg-cyan-50 p-2.5">
          <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-cyan-900">
            <GraduationCap className="h-3.5 w-3.5" />
            Canvasser Ask Coach
          </p>
          {askCoachStats.map((s) => (
            <div key={s.canvasserId} className="space-y-1 rounded bg-white p-2 text-xs">
              <div className="flex items-center justify-between gap-2">
                <span className="text-neutral-700">
              ...
              {coachingNote && coachFor === s.canvasserId && (
                <div className="space-y-1 rounded bg-cyan-50 p-2 text-neutral-700">
          ...
          {coachAi.isError && <p className="text-sm text-red-600">{(coachAi.error as Error).message}</p>}
        </div>
      )}

      {fatigueAlerts.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-violet-200 bg-violet-50 p-2.5">
          ...
          {fatigueAlerts.map((f) => (
            <div key={f.canvasserId} className="space-y-1 rounded bg-white p-2 text-xs">
              <div className="flex items-center justify-between gap-2">
                <span className="text-neutral-700">
              ...
              {checkinPrompt && checkinFor === f.canvasserId && (
                <div className="space-y-1 rounded bg-violet-50 p-2 text-neutral-700">
                  <p>{checkinPrompt.checkin_message}</p>
                </div>
              )}
            </div>
          ))}
          {checkinAi.isError && <p className="text-sm text-red-600">{(checkinAi.error as Error).message}</p>}
        </div>
      )}

      {silentCanvassers.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-slate-300 bg-slate-100 p-2.5">
          ...
          {silentCanvassers.map((s) => (
            <div key={s.canvasserId} className="space-y-1 rounded bg-white p-2 text-xs">
              ...
              {silencePrompt && silenceFor === s.canvasserId && (
                <div className="space-y-1 rounded bg-slate-50 p-2 text-neutral-700">
                  <p>{silencePrompt.checkin_message}</p>
                </div>
              )}
            </div>
          ))}
          {silenceAi.isError && <p className="text-sm text-red-600">{(silenceAi.error as Error).message}</p>}
        </div>
      )}

      {overlapAlerts.length > 0 && (
        <div className="space-y-1 rounded-md border border-fuchsia-200 bg-fuchsia-50 p-2.5 text-xs text-fuchsia-900">
          <p className="flex items-center gap-1.5 font-semibold uppercase tracking-wide">
            <Users className="h-3.5 w-3.5" />
            Cross-canvasser overlap
          </p>
          {overlapAlerts.map((o) => (
            <p key={o.householdKey}>
      ...
      {askCoverageGaps.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-red-200 bg-red-50 p-2.5">
          ...
          {askCoverageGaps.map((g) => (
            <div key={g.householdKey} className="space-y-1 rounded bg-white p-2 text-xs">
              <div className="flex items-center justify-between gap-2">
                <span className="text-neutral-700">
              ...
              {coverageAlert && coverageGapFor === g.householdKey && (
                <div className="space-y-1 rounded bg-red-50 p-2 text-neutral-700">
          ...
          {coverageGapAi.isError && <p className="text-sm text-red-600">{(coverageGapAi.error as Error).message}</p>}
        </div>
      )}

      {priorityDoors.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-indigo-200 bg-indigo-50 p-2.5">
          <div className="flex items-center justify-between gap-2">
          ...
          {priorityDoorAi.isError && <p className="text-sm text-red-600">{(priorityDoorAi.error as Error).message}</p>}
          {priorityDoorBriefing && (
            <div className="space-y-1 rounded bg-white p-2.5 text-xs text-neutral-700">
              <p className="font-medium text-neutral-900">{priorityDoorBriefing.headline}</p>
              {priorityDoorBriefing.doors.map((d, i) => (
      ...
      {revisitQueue.length > 0 && (
        <div className="space-y-2 rounded-md border border-slate-200 bg-slate-50 p-2.5">
          <div className="flex items-center justify-between gap-2">
            ...
            {canUseAi && (
              <Button size="sm" variant="outline" onClick={getRevisitStrategy} disabled={revisitAi.isPending}>
                <Sparkles className="h-3.5 w-3.5" />
          ...
          {revisitAi.isError && <p className="text-sm text-red-600">{(revisitAi.error as Error).message}</p>}
          {revisitStrategy && (
            <div className="space-y-1 rounded bg-white p-2.5 text-xs text-neutral-700">
      ...
      {persistenceTargets.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-amber-200 bg-amber-50 p-2.5">
            ...
            <div key={t.voterId} className="space-y-1 rounded bg-white p-2 text-xs">
              <div className="flex items-center justify-between gap-2">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => getPersistenceAsk(t)}
                  disabled={persistenceAi.isPending}
                >
                  <Sparkles className="h-3.5 w-3.5" />
                  {persistenceAi.isPending && persistenceFor === t.voterId ? '…' : 'Get ask script'}
                </Button>
              </div>
              {persistenceScript && persistenceFor === t.voterId && (
                <div className="space-y-1 rounded bg-amber-50 p-2 text-neutral-700">
          ...
          {persistenceAi.isError && <p className="text-sm text-red-600">{(persistenceAi.error as Error).message}</p>}
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {canUseAi && (
          <Button size="sm" onClick={generateBriefing} disabled={ai.isPending}>
            <Sparkles className="h-3.5 w-3.5" />
            {ai.isPending ? 'Reading the field…' : "Generate today's briefing"}
          </Button>
        )}
        {canUseAi && (
          <Button variant="outline" size="sm" onClick={generateDebrief} disabled={debriefAi.isPending}>
      ...
      {briefing && (
        <div className="space-y-2 rounded-md border border-indigo-100 bg-indigo-50 p-3">
          ...
          {briefing.watch_out && (
            <p className="border-t border-indigo-100 pt-2 text-xs text-amber-700">⚠ {briefing.watch_out}</p>
          )}
        </div>
      )}

      {debriefAi.isError && <p className="text-sm text-red-600">{(debriefAi.error as Error).message}</p>}
      {shiftDebrief && (
        <div className="space-y-2 rounded-md border border-emerald-100 bg-emerald-50 p-3">
          ...
          {shiftDebrief.follow_up_tomorrow.length > 0 && (
      ...
      {topDoor && canUseAi && (
        <div className="space-y-2 rounded-md border border-teal-200 bg-teal-50 p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            {mapSelectedDoor && onClearMapSelection && (
              <Button size="sm" variant="outline" onClick={onClearMapSelection}>
                Clear map selection
              </Button>
            )}
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            {remainingLanguage && (
              <Button size="sm" variant="outline" onClick={getLanguagePrep} disabled={languageAi.isPending}>
                <Languages className="h-3.5 w-3.5" />
                {languageAi.isPending ? '…' : `${remainingLanguage} prep`}
              </Button>
            )}
          </div>
          {(scriptAi.isError || explainAi.isError || languageAi.isError) && (
            <p className="text-sm text-red-600">
              {((scriptAi.error || explainAi.error || languageAi.error) as Error).message}
            </p>
          )}
          {doorScript && (
            <div className="space-y-1 rounded bg-white p-2.5 text-xs text-neutral-700">
          ...
          {doorExplain && (
            <p className="rounded bg-white p-2.5 text-xs text-neutral-700">{doorExplain.summary}</p>
          )}
          {languagePrep && (
            <div className="space-y-1 rounded bg-white p-2.5 text-xs text-neutral-700">
      ...
      {territoryDifficulty.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-orange-200 bg-orange-50 p-3">
          ...
          <div className="space-y-1 text-xs text-neutral-700">
            {territoryDifficulty.slice(0, 5).map((t) => (
              <p key={t.territoryId}>
                {t.name}: {t.contactRatePct}% contact rate, {t.oppositionPct}% opposition, {t.remainingDoors} doors left
              </p>
            ))}
          </div>
          {territoryAi.isError && <p className="text-sm text-red-600">{(territoryAi.error as Error).message}</p>}
          {territoryBriefing && (
            <div className="space-y-1.5">
              {territoryBriefing.ranked.map((r, i) => (
                <div key={i} className="rounded bg-white p-2 text-xs">
      ...
      {territoryRoi.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-lime-200 bg-lime-50 p-3">
          <div className="flex items-center justify-between gap-2">
          ...
          {territoryRoiAi.isError && <p className="text-sm text-red-600">{(territoryRoiAi.error as Error).message}</p>}
          {territoryRoiBriefing && (
            <div className="space-y-1.5">
              {territoryRoiBriefing.ranked.map((r, i) => (
                <div key={i} className="rounded bg-white p-2 text-xs">
                  <p className="font-semibold text-neutral-800">{r.territory}</p>
      ...
      {reallocations.length > 0 && canUseAi && (
        <div className="space-y-2 rounded-md border border-teal-200 bg-teal-50 p-3">
          <div className="flex items-center justify-between gap-2">
          ...
          {territoryStaffingAi.isError && (
            <p className="text-sm text-red-600">{(territoryStaffingAi.error as Error).message}</p>
          )}
          {territoryStaffingBriefing && (
            <div className="space-y-1.5">
              {territoryStaffingBriefing.ranked.map((r, i) => (
                <div key={i} className="rounded bg-white p-2 text-xs">
      ...
      {canUseAi && (
        <div className="space-y-2 rounded-md border border-rose-200 bg-rose-50 p-3">
                ...
                <div key={t.voterId} className="space-y-1 rounded bg-white p-2 text-xs">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => getCountdownAsk(t)}
                      disabled={countdownAi.isPending}
                    >
                      <Sparkles className="h-3.5 w-3.5" />
                      {countdownAi.isPending && countdownFor === t.voterId ? '…' : 'Get ask script'}
                    </Button>
                  </div>
                  {countdownScript && countdownFor === t.voterId && (
                    <div className="space-y-1 rounded bg-rose-50 p-2 text-neutral-700">
          ...
          {countdownAi.isError && <p className="text-sm text-red-600">{(countdownAi.error as Error).message}</p>}
        </div>
      )}
      ...
      {canUseAi && (
        <div className="space-y-2 rounded-md border border-violet-200 bg-violet-50 p-3">
          ...
          <div className="flex gap-2">
            <Input
              placeholder="e.g. &quot;I'm worried about property taxes&quot;"
              value={objectionText}
              onChange={(e) => setObjectionText(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && askObjection()}
            />
          ...
          {objectionAi.isError && <p className="text-sm text-red-600">{(objectionAi.error as Error).message}</p>}
          {objectionResult && (
            <div className="space-y-1 rounded bg-white p-2.5 text-xs text-neutral-700">
      ...
      {canUseAi && (
        <div className="space-y-2 rounded-md border border-pink-200 bg-pink-50 p-3">
          <div className="flex items-center gap-1.5">
          ...
          <div className="flex gap-2">
            <Input
              placeholder="e.g. &quot;I don't have any cash on me right now&quot;"
              value={donationObjectionText}
              onChange={(e) => setDonationObjectionText(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && askDonationObjection()}
            />
            <Button
              size="sm"
              onClick={askDonationObjection}
              disabled={donationObjectionAi.isPending || !donationObjectionText.trim()}
            >
              <Sparkles className="h-3.5 w-3.5" />
              {donationObjectionAi.isPending ? '…' : 'Get response'}
            </Button>
          </div>
          {donationObjectionAi.isError && (
            <p className="text-sm text-red-600">{(donationObjectionAi.error as Error).message}</p>
          )}
          {donationObjectionResult && (
            <div className="space-y-1 rounded bg-white p-2.5 text-xs text-neutral-700">
              <p>
...
function QuickInsightLine({ insight, tone }: { insight: string | null | undefined; tone?: string }) {
  if (!insight) return null;
  return (
    <p className={`flex items-start gap-1.5 text-xs ${tone ?? 'text-violet-700'}`}>
  ...
  return (
    <label className="flex items-center justify-between gap-2 text-xs text-neutral-600">
      ...
      <input
        ...
        className="h-7 w-20 rounded-md border border-neutral-300 px-2 text-right text-xs"
      />
    </label>
  );
}
...
function CustomizePanel({
  ...
  draft: TurfPreferences;
  setDraft: (updater: (prev: TurfPreferences) => TurfPreferences) => void;
  onSave: () => void;
  onReset: () => void;
  saving: boolean;
}) {
    ...
    <div className="space-y-3 rounded-md border border-neutral-200 bg-neutral-50 p-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <fieldset className="space-y-1.5">
          ...
          <NumberField
            label={'"Recent" window (hours)'}
            value={draft.statWindows.recentHours}
            onChange={(n) => setDraft((p) => ({ ...p, statWindows: { ...p.statWindows, recentHours: n } }))}
          />
          <NumberField
            label={'"Today" window (hours)'}
            value={draft.statWindows.todayHours}
            onChange={(n) => setDraft((p) => ({ ...p, statWindows: { ...p.statWindows, todayHours: n } }))}
          />
        </fieldset>
        ...
        <fieldset className="space-y-1.5">
          ...
          <NumberField
            ...
            value={draft.bestTimeToKnock.minSample}
            onChange={(n) => setDraft((p) => ({ ...p, bestTimeToKnock: { ...p.bestTimeToKnock, minSample: n } }))}
          />
          <NumberField
            label="Min. attempts/hour"
            value={draft.bestTimeToKnock.minHourAttempts}
            onChange={(n) => setDraft((p) => ({ ...p, bestTimeToKnock: { ...p.bestTimeToKnock, minHourAttempts: n } }))}
          />
          <NumberField
            label="Lookback (days)"
            value={draft.bestTimeToKnock.lookbackDays}
            onChange={(n) => setDraft((p) => ({ ...p, bestTimeToKnock: { ...p.bestTimeToKnock, lookbackDays: n } }))}
          />
        </fieldset>
        ...
        <fieldset className="space-y-1.5">
          ...
          <NumberField
            label="Lookback (days)"
            value={draft.persuasionDrift.lookbackDays}
            onChange={(n) => setDraft((p) => ({ ...p, persuasionDrift: { ...p.persuasionDrift, lookbackDays: n } }))}
          />
          <CheckboxField
            label="Show warmed-up doors"
            checked={draft.persuasionDrift.showWarmed}
            onChange={(v) => setDraft((p) => ({ ...p, persuasionDrift: { ...p.persuasionDrift, showWarmed: v } }))}
          />
          <CheckboxField
            label="Show cooled-off doors"
            checked={draft.persuasionDrift.showCooled}
            onChange={(v) => setDraft((p) => ({ ...p, persuasionDrift: { ...p.persuasionDrift, showCooled: v } }))}
          />
        </fieldset>
        ...
        <fieldset className="space-y-1.5">
          ...
          <NumberField
            label="Household min. size"
            value={draft.household.minSize}
            onChange={(n) => setDraft((p) => ({ ...p, household: { minSize: n } }))}
          />
          <NumberField
            label="Heatmap noise floor"
            value={draft.heatmap.minWeightToShow}
            min={0}
            step={0.1}
            onChange={(n) => setDraft((p) => ({ ...p, heatmap: { minWeightToShow: n } }))}
          />
          <NumberField
            label="Daylight warning (min)"
            value={draft.daylight.warnMinutes}
            onChange={(n) => setDraft((p) => ({ ...p, daylight: { warnMinutes: n } }))}
          />
        </fieldset>
        ...
        <fieldset className="space-y-1.5">
          ...
          <NumberField
            label="Leaderboard min. attempts"
            value={draft.canvasserLeaderboard.minAttempts}
            onChange={(n) => setDraft((p) => ({ ...p, canvasserLeaderboard: { minAttempts: n } }))}
          />
          <NumberField
            label="Overlap lookback (days)"
            value={draft.overlapGuard.lookbackDays}
            onChange={(n) => setDraft((p) => ({ ...p, overlapGuard: { lookbackDays: n } }))}
          />
          <NumberField
            label="Revisit min. attempts"
            value={draft.revisitQueue.minAttempts}
            onChange={(n) => setDraft((p) => ({ ...p, revisitQueue: { minAttempts: n } }))}
          />
        </fieldset>
        ...
        <fieldset className="space-y-1.5 sm:col-span-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              value={draft.highlightFilter.mode}
              onChange={(e) =>
                setDraft((p) => ({ ...p, highlightFilter: { mode: e.target.value as HighlightMode, value: null } }))
              }
            >
            ...
            {draft.highlightFilter.mode === 'party' && (
              <select
                className="h-7 rounded-md border border-neutral-300 bg-white px-2 text-xs"
                value={draft.highlightFilter.value ?? ''}
                onChange={(e) => setDraft((p) => ({ ...p, highlightFilter: { ...p.highlightFilter, value: e.target.value } }))}
              >
                <option value="" disabled>Choose a party…</option>
                {Object.keys(PARTY_LABELS).map((k) => (
                  <option key={k} value={k}>{PARTY_LABELS[k as keyof typeof PARTY_LABELS]}</option>
                ))}
              </select>
            )}
            {draft.highlightFilter.mode === 'persuadability' && (
              <select
                className="h-7 rounded-md border border-neutral-300 bg-white px-2 text-xs"
                value={draft.highlightFilter.value ?? ''}
                onChange={(e) => setDraft((p) => ({ ...p, highlightFilter: { ...p.highlightFilter, value: e.target.value } }))}
              >
                ...
                {Object.keys(PERSUADABILITY_LABELS).map((k) => (
                  <option key={k} value={k}>{PERSUADABILITY_LABELS[k as keyof typeof PERSUADABILITY_LABELS]}</option>
                ))}
              </select>
            )}
          </div>
