...
export function useTopConnectors(orgId: string | undefined) {
  return useQuery({
...
export function useChurnRisk(orgId: string | undefined) {
  return useQuery({
...
export function useOptimizeAsk() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      projectId: string;
      donorId: string;
      donorHistory: string; // JSON string of donor's gift history
    }) => {
      ...
      if (!parsed) throw new Error('Could not parse ask optimization');
...
export function usePredictChurn() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      orgId: string;
      projectId: string;
      donorHistory: string; // JSON: {id, last_gift_at, avg_gift_cents, ...}
    }) => {
      const result = await ai.mutateAsync({
        orgId,
        projectId,
      ...
      const parsed = extractJson<{
        risk_score: number;
        reason: string;
        reactivation_ask_cents: number;
        win_back_message: string;
      }>(result.text);
      if (!parsed) throw new Error('Could not parse churn prediction');
      return parsed;
    }
  });
}
...
export function useScoreConnector() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      orgId: string;
      projectId: string;
      donorProfile: string; // JSON: {giving_history, velocity, engagement_signals, ...}
    }) => {
      const result = await ai.mutateAsync({
        orgId,
        projectId,
      ...
      if (!parsed) throw new Error('Could not parse connector score');
      return parsed;
    }
  });
}
...
export function useGenerateVariations() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      orgId: string;
      projectId: string;
      baseMessage: string; // the base fundraising message
      orgContext: string; // JSON: {deadline, has_matching_funds, ...}
    }) => {
      const result = await ai.mutateAsync({
        orgId,
        projectId,
...
export type MajorDonorEscalation = {
  id: string;
  donor_id: string;
...
export function useMajorDonorEscalations(orgId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('major_donor_escalations')
        ...
        .limit(20);
      if (error) throw error;
      return data as MajorDonorEscalation[];
    },
    enabled: Boolean(orgId)
  });
}
...
export function useAssessMajorDonor() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      projectId: string;
      donorId: string;
      signals: string; // JSON: {giving_trend, event_attendance, email_engagement}
    }) => {
      ...
      if (!parsed) throw new Error('Could not parse major-donor assessment');
...
export function useCheckFatigue() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      orgId: string;
      projectId: string;
      sendHistory: string; // JSON: {sends_last_30d, opens, clicks, unsubscribes}
    }) => {
      const result = await ai.mutateAsync({ orgId, projectId, purpose: 'fatigue_guard', context: sendHistory });
      const parsed = extractJson<{ fatigue_risk_score: number; recommendation: string }>(result.text);
      if (!parsed) throw new Error('Could not parse fatigue assessment');
      return parsed;
    }
  });
}
...
export function useBuildSprintPlan() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const parsed = extractJson<{
        daily_plan: { day: string; segment: string; ask_theme: string }[];
        pace_assessment: string;
      }>(result.text);
      if (!parsed) throw new Error('Could not parse sprint plan');
...
export function useLatestSprintPlan(projectId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('fec_sprint_plans')
        ...
        .maybeSingle();
      if (error) throw error;
      return data as FecSprintPlan | null;
    },
...
export function useDraftRetentionSequence() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      donorId: string;
      donationId?: string;
      donationContext: string; // JSON: {amount_cents, donor_history}
    }) => {
      ...
      const parsed = extractJson<{
        ...
        second_ask: string;
        impact_update_delay_days: number;
        second_ask_delay_days: number;
      }>(result.text);
      if (!parsed) throw new Error('Could not parse retention sequence');
...
export type PaymentRecoveryAlert = {
  id: string;
  donor_id: string;
  failure_type: string;
...
export function usePendingPaymentRecoveries(orgId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('payment_recovery_alerts')
        ...
        .limit(50);
      if (error) throw error;
      return data as PaymentRecoveryAlert[];
    },
    enabled: Boolean(orgId)
  });
}
...
export function useDraftPaymentRecovery() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const parsed = extractJson<{ message: string }>(result.text);
      if (!parsed) throw new Error('Could not parse recovery message');
...
export type OrgMember = { profile_id: string; full_name: string };
...
export function useOrgMembersForBridge(orgId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .eq('status', 'active');
      if (error) throw error;
      return (data as unknown as { profile_id: string; profiles: { email: string; full_name: string | null } | null }[]).map(
        (m) => ({ profile_id: m.profile_id, full_name: m.profiles?.full_name || m.profiles?.email || 'Unknown' })
      ) as OrgMember[];
    },
    enabled: Boolean(orgId)
  });
...
export function useDraftVolunteerAsk() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const parsed = extractJson<{ message: string }>(result.text);
      if (!parsed) throw new Error('Could not parse volunteer ask');
...
export function useVolunteerDonorAsks(orgId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('volunteer_donor_asks')
        ...
        .limit(20);
      if (error) throw error;
      return data as { id: string; profile_id: string; contribution_summary: string; ask_message: string }[];
    },
    enabled: Boolean(orgId)
  });
}
...
export function useDetectMomentum() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const parsed = extractJson<{ message: string; suggested_segment: string }>(result.text);
      if (!parsed) throw new Error('Could not parse momentum alert');
...
export function useDraftRecurringUpgrade() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const parsed = extractJson<{ suggested_monthly_cents: number; message: string }>(result.text);
      if (!parsed) throw new Error('Could not parse upgrade ask');
...
export function useForecastLtv() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      projectId: string;
      donorId: string;
      giftHistory: string; // JSON: {gifts: [...], tenure_months, trend}
    }) => {
      ...
      if (!parsed) throw new Error('Could not parse LTV forecast');
...
export function useCultivateList(orgId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('donor_ltv_forecasts')
        ...
        .limit(20);
      if (error) throw error;
      return data as LtvForecast[];
    },
    enabled: Boolean(orgId)
  });
}

export type DonorMergeSuggestion = {
  id: string;
  donor_id_a: string;
  donor_id_b: string;
...
export function useAssessDuplicate() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      donorIdA: string;
      donorIdB: string;
      recordsContext: string; // JSON: {a: {...}, b: {...}}
    }) => {
      const result = await ai.mutateAsync({ orgId, projectId, purpose: 'donor_dedup', context: recordsContext });
      const parsed = extractJson<{ same_person_likelihood: number; matched_fields: string[]; rationale: string }>(
        result.text
      );
      if (!parsed) throw new Error('Could not parse duplicate assessment');
...
export function usePendingMergeSuggestions(orgId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('donor_merge_suggestions')
        ...
        .limit(20);
      if (error) throw error;
      return data as DonorMergeSuggestion[];
    },
    enabled: Boolean(orgId)
  });
}
...
export function useRejectMerge() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ suggestionId, orgId }: { suggestionId: string; orgId: string }) => {
...
export function useLogRefundAndScan() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const parsed = extractJson<{ risk_level: string; analysis: string }>(result.text);
      if (!parsed) throw new Error('Could not parse refund risk scan');
...
export function useLatestRefundAlert(projectId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('refund_risk_alerts')
        ...
        .maybeSingle();
      if (error) throw error;
      return data as { risk_level: string; analysis: string; refund_count: number } | null;
    },
    enabled: Boolean(projectId)
  });
}
