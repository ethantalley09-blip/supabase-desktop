...
export function BundlerNetwork({ orgId, projectId, donors, donations }) {
    ...
    const getCultivationAsk = (cluster) => {
    ...
    return (<div className="space-y-3 rounded-lg border border-violet-200 bg-white p-5">
      ...
      {clusters.length === 0 ? (<p className="text-sm text-neutral-500">
          No employer cluster yet — appears once two or more real donors at the same employer have
          both given.
        </p>) : (<div className="space-y-2">
          {clusters.slice(0, 5).map((cluster) => (<div key={cluster.employer} className="rounded-md border border-violet-100 bg-violet-50 p-3 text-xs">
              ...
              {ask && askFor === cluster.employer && (<div className="mt-1.5 space-y-1 rounded bg-white p-2 text-neutral-700">
                  <p>{ask.ask_message}</p>
