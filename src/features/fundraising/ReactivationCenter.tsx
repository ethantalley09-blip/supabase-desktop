...
import type { Donation, Donor } from './useFundraising';
...
export function ReactivationCenter({
  orgId,
  projectId,
  donors,
  donations
  ...
  return (
    <div className="space-y-3 rounded-lg border border-orange-200 bg-white p-5">
      ...
      {lapsing.length === 0 ? (
        <p className="text-sm text-neutral-500">No donors are meaningfully off their giving rhythm right now.</p>
      ) : (
        <div className="space-y-2">
          {lapsing.map(({ donor, assessment, typicalGiftCents, giftCount }) => (
            <div key={donor.id} className="flex items-center justify-between rounded-md border border-orange-100 bg-orange-50 p-3">
              <div>
            ...
            <div key={r.id} className="rounded-md border border-neutral-200 p-3">
              ...
              {openDraft === r.id && (
                <div className="mt-2 space-y-2">
                  {(['impact', 'urgency', 'peer'] as const).map((angle) => (
                    <div key={angle} className="rounded bg-neutral-50 p-2">
