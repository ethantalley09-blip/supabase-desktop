...
export function OrgCreateWizard({ onCreated }) {
    const { user } = useAuth();
    const [submitted, setSubmitted] = useState(false);
    const createOrg = useCreateOrganization();
    const { register, handleSubmit, control, formState: { errors, isSubmitting } } = useForm({ resolver: zodResolver(orgSchema) });
    ...
    if (submitted) {
        return (<div className="mx-auto max-w-md rounded-lg border border-neutral-200 bg-white p-6 text-center shadow-sm">
    ...
    return (<form className="mx-auto max-w-lg space-y-5 rounded-lg border border-neutral-200 bg-white p-6 shadow-sm" onSubmit={handleSubmit(onSubmit)}>
      <div>
      ...
      <div className="space-y-1.5">
        ...
        <Controller name="org_type" control={control} render={({ field }) => (<Select value={field.value} onValueChange={field.onChange}>
              <SelectTrigger>
                <SelectValue placeholder="Select a type"/>
              </SelectTrigger>
              <SelectContent>
                {ORG_TYPES.map((t) => (<SelectItem key={t.value} value={t.value}>
                    {t.label}
                  </SelectItem>))}
              </SelectContent>
            </Select>)}/>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <Controller name="state_of_registration" control={control} render={({ field }) => (<Select value={field.value} onValueChange={field.onChange}>
                <SelectTrigger>
                  <SelectValue placeholder="Optional"/>
                </SelectTrigger>
                <SelectContent>
                  {US_STATES.map((s) => (<SelectItem key={s} value={s}>
                      {s}
                    </SelectItem>))}
                </SelectContent>
              </Select>)}/>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          {errors.billing_contact_email && (<p className="text-xs text-red-600">{errors.billing_contact_email.message}</p>)}
        </div>
      </div>

      {createOrg.isError && (<p className="text-sm text-red-600">{createOrg.error.message}</p>)}
