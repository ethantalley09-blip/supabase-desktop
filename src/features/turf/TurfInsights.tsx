...
import { buildTurfSnapshot, type Territory, type VoterRecord } from './useTurf';
...
export function TurfInsights({
  ...
  const snapshot = useMemo(
    () => JSON.stringify(buildTurfSnapshot(voters, territories)),
    [voters, territories]
  );

  // Gate: needs both the premium entitlement and the ai.use permission.
  if (!canUseAi || !entitlement.data) return null;
  ...
  const runCoach = () =>
    coach.mutate({ orgId, projectId, purpose: 'field_coach', context: snapshot });
  ...
  return (
    <div className="space-y-3 rounded-lg border border-violet-200 bg-white p-5">
      ...
      <div className="space-y-2">
        ...
        {coach.data && (
          <div className="whitespace-pre-wrap rounded-md border border-violet-200 bg-violet-50 p-3 text-sm text-neutral-800">
            {coach.data.text}
          </div>
        )}
      </div>
