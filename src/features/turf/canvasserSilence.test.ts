...
import { detectSilentCanvassers, type VisitForSilence } from './canvasserSilence';
...
function visit(id: string, name: string, minutes: number): VisitForSilence {
...
describe('detectSilentCanvassers', () => {
  it('flags a canvasser silent for over the threshold after a real earlier presence', () => {
    const visits: VisitForSilence[] = [visit('c1', 'Finn', 300), visit('c1', 'Finn', 250), visit('c1', 'Finn', 200)];
  ...
  it('does not flag a canvasser who is still actively knocking', () => {
    const visits: VisitForSilence[] = [visit('c1', 'Finn', 60), visit('c1', 'Finn', 10)];
  ...
  it('does not flag a canvasser with only one visit today, no matter how stale', () => {
    const visits: VisitForSilence[] = [visit('c1', 'Finn', 300)];
    expect(detectSilentCanvassers(visits, NOW)).toHaveLength(0);
  });

  it('ignores visits from a previous day', () => {
    const visits: VisitForSilence[] = [visit('c1', 'Finn', 60 * 30), visit('c1', 'Finn', 60 * 28)];
    expect(detectSilentCanvassers(visits, NOW)).toHaveLength(0);
  });

  it('ranks the longest silence first', () => {
    const visits: VisitForSilence[] = [
      visit('c1', 'Finn', 300),
      visit('c1', 'Finn', 200),
      visit('c2', 'Carol', 500),
      visit('c2', 'Carol', 400)
