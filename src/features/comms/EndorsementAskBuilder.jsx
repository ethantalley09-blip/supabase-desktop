...
export function EndorsementAskBuilder({ orgId, projectId }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-violet-200 bg-white p-5">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Input className="min-w-56 flex-1" placeholder="Why they're a fit, e.g. endorsed our housing platform publicly last year" value={whyFit} onChange={(e) => setWhyFit(e.target.value)}/>
        <Button size="sm" onClick={() => draft.mutate({ orgId, projectId, who: who.trim(), whyFit: whyFit.trim() })} disabled={draft.isPending || !who.trim() || !whyFit.trim()}>
