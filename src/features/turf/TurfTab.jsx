...
function colorForVoter(v, mode) {
    if (mode === 'party')
        return PARTY_COLORS[voterParty(v) ?? 'unknown'];
    if (mode === 'persuadability')
        return PERSUADABILITY_COLORS[classifyPersuadability(v).bucket];
...
function colorForHousehold(h, mode) {
...
function matchesHighlight(v, filter) {
    if (filter.mode === 'none' || !filter.value)
        return true;
    if (filter.mode === 'party')
        return (voterParty(v) ?? 'unknown') === filter.value;
    return classifyPersuadability(v).bucket === filter.value;
}
function opacityForHousehold(h, filter) {
    if (filter.mode === 'none' || !filter.value)
        return 1;
...
function weightForHousehold(h, mode) {
    if (mode === 'density')
        return Math.min(1, h.memberCount / 5);
    return Math.max(...h.members.map((v) => heatmapWeight(v, mode)));
}
export function TurfTab({ project }) {
    ...
    const [areaVoterCount, setAreaVoterCount] = useState(null);
    ...
    const [mapSelectedVoterId, setMapSelectedVoterId] = useState(null);
    ...
    const optimizeRoute = (territoryId, name) => {
    ...
    const splitTerritory = async (territoryId, name, k) => {
        const doors = territoryDoors(territoryId);
        if (doors.length < 2)
            return;
    ...
    const wards = useMemo(() => {
        const set = new Set();
        for (const v of voters ?? []) {
            if (cityFilter && voterCity(v) !== cityFilter)
                continue;
            const w = voterWard(v);
            if (w)
                set.add(w);
    ...
    const filteredVoters = useMemo(() => {
        return (voters ?? []).filter((v) => {
            if (cityFilter && voterCity(v) !== cityFilter)
                return false;
            if (wardFilter && voterWard(v) !== wardFilter)
                return false;
            return true;
        });
    ...
    const selectionName = () => [cityFilter, wardFilter && `Ward ${wardFilter}`].filter(Boolean).join(' — ') || 'Selection';
    const knockSelection = async () => {
        ...
        const skipNote = result.skippedCount > 0 ? ` (${result.skippedCount} already-assigned skipped)` : '';
        setLastAssignment(`"${result.territory.name}" walk list saved — ${result.assignedCount} voter${result.assignedCount === 1 ? '' : 's'} assigned${skipNote}.`);
    };
    ...
    useEffect(() => {
        ...
        map.on('click', (e) => {
            if (drawModeRef.current === 'off')
                return;
            const next = [...draftRingRef.current, [e.lngLat.lng, e.lngLat.lat]];
    ...
    useEffect(() => {
        if (!mapReady || !mapRef.current)
            return;
        const source = mapRef.current.getSource('voters');
        if (!source)
            return;
        ...
        const clampWeight = (raw) => (raw < prefs.heatmap.minWeightToShow ? 0 : raw);
        const features = groupBy === 'household'
    ...
    useEffect(() => {
        if (!mapReady || !mapRef.current)
            return;
        const source = mapRef.current.getSource('territories');
        if (!source)
            return;
        source.setData({
    ...
    useEffect(() => {
        ...
        if (!source)
            return;
        const features = draftRing.map((pos) => ({
            type: 'Feature',
    ...
    useEffect(() => {
        ...
        const coords = route.ordered.map((v) => [v.lng, v.lat]);
        const features = [];
        if (coords.length >= 2) {
    ...
    const getAreaBriefing = () => {
        ...
        setAreaBriefing(null);
        if (votersInArea.length === 0)
            return;
        const snapshot = buildBriefingSnapshot(votersInArea, territories ?? []);
        areaAi.mutate({ orgId: project.org_id, projectId: project.id, purpose: 'map_area_briefing', context: JSON.stringify(snapshot) }, { onSuccess: (res) => setAreaBriefing(extractJson(res.text)) });
    };
    const finishTerritory = async () => {
        if (draftRing.length < 3 || territoryName.trim().length < 2)
            return;
        ...
        setLastAssignment(`"${territoryName.trim()}" saved — ${result.assignedCount} voter${result.assignedCount === 1 ? '' : 's'} assigned by point-in-polygon.`);
        cancelDrawing();
    };
    ...
    return (<div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {canManage.data && (<div className="flex gap-2">
      ...
      <div id="tool-turf_briefing">
        <TurfBriefing orgId={project.org_id} projectId={project.id} 
    ...
    voters={filteredVoters} territories={territories ?? []} canUseAi={Boolean(canUseAi.data)} onRebalance={(r) => setRoute({ ...r, territoryId: 'live-rebalance', name: "Today's remaining doors" })} heatmapMode={heatmapMode} mapSelectedVoterId={mapSelectedVoterId} onClearMapSelection={() => setMapSelectedVoterId(null)}/>
      ...
      <div id="tool-doorstep_donations">
        <DoorstepDonations orgId={project.org_id} projectId={project.id} voters={voters ?? []} canUseAi={Boolean(canUseAi.data)}/>
      </div>
      ...
      <div id="tool-field_coach">
        <div id="tool-note_digest">
          <TurfInsights orgId={project.org_id} projectId={project.id} voters={voters ?? []} territories={territories ?? []} canUseAi={Boolean(canUseAi.data)}/>
        </div>
      </div>

      {canUseAi.data && (<div id="tool-volunteer_pipeline">
          <VolunteerPipeline orgId={project.org_id} projectId={project.id}/>
        </div>)}
      {canUseAi.data && (<div id="tool-gotv_sprint_plan">
      ...
      <div className="inline-flex gap-1 rounded-lg bg-neutral-100 p-1">
        <button type="button" onClick={() => setView('map')} className={`rounded-md px-3 py-1 text-sm ${view === 'map' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500'}`}>
          Map &amp; walk lists
        </button>
        <button type="button" onClick={() => setView('chase')} className={`rounded-md px-3 py-1 text-sm ${view === 'chase' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500'}`}>
          Ballot chase &amp; hygiene
        </button>
      </div>

      <div id="tool-import_mapping">
        {importing && (<ImportWizard projectId={project.id} orgId={project.org_id} onDone={() => setImporting(false)}/>)}
      </div>

      {hasGeography && (<div className="flex flex-wrap items-end gap-3 rounded-lg border border-neutral-200 bg-white p-4">
          <div className="space-y-1.5">
            <label className="block text-xs font-medium text-neutral-500">City</label>
            <select className="h-8 rounded-md border border-neutral-300 bg-white px-2 text-sm" value={cityFilter} onChange={(e) => {
          ...
          <div className="space-y-1.5">
            ...
            <select className="h-8 rounded-md border border-neutral-300 bg-white px-2 text-sm disabled:opacity-50" value={wardFilter} disabled={wards.length === 0} onChange={(e) => setWardFilter(e.target.value)}>
          ...
          {canManage.data && (<label className="flex items-center gap-1.5 text-xs text-neutral-600">
              <input type="checkbox" checked={skipAssigned} onChange={(e) => setSkipAssigned(e.target.checked)}/>
              Skip voters already in a territory
            </label>)}
          {hasSelection && (<Button variant="outline" size="sm" onClick={() => { setCityFilter(''); setWardFilter(''); }}>
              Clear
            </Button>)}
          {canManage.data && (<Button size="sm" onClick={knockSelection} disabled={!hasSelection || filteredAssignable === 0 || createFromSelection.isPending}>
      ...
      {view === 'chase' && (<BallotChase orgId={project.org_id} projectId={project.id} voters={voters ?? []} visibleVoters={filteredVoters} canManage={Boolean(canManage.data)} canUseAi={Boolean(canUseAi.data)}/>)}

      {view === 'map' && (<div className="space-y-2 rounded-lg border border-neutral-200 bg-white p-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            ].map(([mode, label]) => (<ModeButton key={mode} active={colorMode === mode} onClick={() => setColorMode(mode)}>
                  {label}
                </ModeButton>))}
            </div>
            <div className="flex items-center gap-1">
              ...
              {[
                ...
                ['staleness', 'Staleness']
            ].map(([mode, label]) => (<ModeButton key={mode} active={heatmapMode === mode} onClick={() => setHeatmapMode(mode)}>
                  {label}
                </ModeButton>))}
            </div>
            <div className="flex items-center gap-1">
              ...
              {[
                ...
                ['household', 'Household']
            ].map(([mode, label]) => (<ModeButton key={mode} active={groupBy === mode} onClick={() => setGroupBy(mode)}>
                  {label}
                </ModeButton>))}
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-3 text-xs text-neutral-600">
            ...
            {colorMode === 'party' &&
                Object.keys(PARTY_LABELS).map((k) => (<LegendChip key={k} color={PARTY_COLORS[k]} label={PARTY_LABELS[k]}/>))}
            {colorMode === 'persuadability' &&
                Object.keys(PERSUADABILITY_LABELS).map((k) => (<LegendChip key={k} color={PERSUADABILITY_COLORS[k]} label={PERSUADABILITY_LABELS[k]}/>))}
            {groupBy === 'household' && <LegendChip color={MIXED_HOUSEHOLD_COLOR} label="Mixed household"/>}
          </div>
          {groupBy === 'household' && (<p className="text-xs text-neutral-400">
      ...
      <div ref={containerRef} className={view === 'map'
            ? 'h-96 w-full overflow-hidden rounded-lg border border-neutral-200'
            : 'hidden'}/>

      {view === 'map' && (<>
      {drawMode === 'territory' && (<div className="flex items-end gap-3 rounded-lg border border-neutral-200 bg-white p-4">
          <div className="flex-1 space-y-1.5">
            <p className="text-xs text-neutral-500">
              Click the map to add vertices ({draftRing.length} so far — need at least 3), then name
              and save.
            </p>
            <Input placeholder="Territory name" value={territoryName} onChange={(e) => setTerritoryName(e.target.value)}/>
          </div>
          <Button size="sm" onClick={finishTerritory} disabled={draftRing.length < 3 || territoryName.trim().length < 2 || createTerritory.isPending}>
            Save territory
          </Button>
        </div>)}

      {drawMode === 'ask' && (<div className="space-y-2 rounded-lg border border-sky-200 bg-sky-50 p-4">
          ...
          {areaAi.isError && <p className="text-sm text-red-600">{areaAi.error.message}</p>}
          {areaVoterCount === 0 && (<p className="text-xs text-neutral-500">No mapped voters fall inside that shape.</p>)}
          {areaBriefing && (<div className="space-y-1.5 rounded bg-white p-3 text-xs text-neutral-700">
              <p className="font-medium text-neutral-900">{areaBriefing.headline}</p>
              {areaBriefing.focus_areas.map((f, i) => (<div key={i}>
      ...
      {createTerritory.isError && (<p className="text-sm text-red-600">{createTerritory.error.message}</p>)}

      {route && (<div className="overflow-hidden rounded-lg border border-pink-200 bg-white">
          ...
          <ol className="max-h-64 divide-y divide-neutral-100 overflow-y-auto text-sm">
            {route.ordered.map((v, i) => (<li key={v.id} className="flex gap-3 px-4 py-1.5">
      ...
      {(territories?.length ?? 0) > 0 && (<div className="overflow-hidden rounded-lg border border-neutral-200 bg-white">
          <table className="w-full text-sm">
            ...
            <tbody>
              {territories.map((t) => {
                    ...
                    return (<tr key={t.id} className="border-t border-neutral-100">
                    ...
                    <td className="px-4 py-2">
                      {canManage.data ? (<select className="h-8 rounded-md border border-neutral-300 bg-white px-2 text-sm" value={t.assigned_to ?? ''} onChange={(e) => assignTerritory.mutate({
                                territoryId: t.id,
                                projectId: project.id,
                        ...
                        </select>) : (<span className="text-neutral-500">
                          {t.profiles?.full_name || t.profiles?.email || 'Unassigned'}
                        </span>)}
                    </td>
                    <td className="px-4 py-2">
                      <div className="flex items-center gap-1.5">
                        <Button variant="outline" size="sm" disabled={count === 0} onClick={() => optimizeRoute(t.id, t.name)}>
                          Optimize
                        </Button>
                        {canManage.data && (<select aria-label={`Split ${t.name} into N walk lists`} className="h-8 rounded-md border border-neutral-300 bg-white px-2 text-sm disabled:opacity-50" value="" disabled={count < 2 || createFromSelection.isPending} onChange={(e) => {
                                const k = Number(e.target.value);
                                if (k >= 2)
                                    void splitTerritory(t.id, t.name, k);
                                e.target.value = '';
                            }}>
                            <option value="">Split…</option>
                            {[2, 3, 4, 5, 6].map((k) => (<option key={k} value={k}>
                                {k} lists
                              </option>))}
                          </select>)}
                      </div>
...
function ModeButton({ active, onClick, children }) {
    return (<button type="button" onClick={onClick} className={`rounded-md px-2 py-1 text-xs ${active ? 'bg-neutral-900 text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'}`}>
      {children}
    </button>);
}
