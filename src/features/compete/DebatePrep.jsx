...
import { useDebatePrep } from './useCompete';
...
export function DebatePrep({ orgId, projectId, records }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-amber-200 bg-white p-5">
      ...
      <textarea rows={2} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" placeholder="Your candidate's key positions, one per line. e.g. Renew the school levy / Fix the Main St corridor / No new city income tax" value={positions} onChange={(e) => setPositions(e.target.value)}/>
      <Button size="sm" onClick={() => records && prep.mutate({ orgId, projectId, records, ourPositions: positions.trim() })} disabled={prep.isPending || !positions.trim() || !(records?.length)}>
        <Sparkles className="h-4 w-4"/>
        {prep.isPending ? 'Preparing…' : 'Build prep sheet'}
      </Button>
      {prep.isError && <p className="text-sm text-red-600">{prep.error.message}</p>}

      {prep.data && (<div className="space-y-2">
          {prep.data.attacks.map((a, i) => (<div key={i} className="rounded-md border border-amber-100 bg-amber-50 p-3">
