...
export function SocialSchedulerPanel({ project }) {
    ...
    return (<div className="space-y-4 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      {showForm && (<div className="space-y-3 rounded-md border border-neutral-100 p-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label>Platform</Label>
              <select className="flex h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm" value={platform} onChange={(e) => setPlatform(e.target.value)}>
                {SOCIAL_PLATFORMS.map((p) => (<option key={p.platform} value={p.platform}>
                    {p.label}
                  </option>))}
              </select>
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea id="post-content" rows={2} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" value={content} onChange={(e) => setContent(e.target.value)}/>
          </div>
      ...
      <ul className="space-y-2">
        {posts?.map((p) => (<SocialPostRow key={p.id} post={p} canManage={Boolean(canManage.data)} onRecordMetrics={(impressions, engagement) => recordMetrics.mutate({ id: p.id, impressions, engagement })}/>))}
        {posts?.length === 0 && <li className="text-sm text-neutral-400">No posts yet.</li>}
      </ul>
    </div>);
}
function SocialPostRow({ post, canManage, onRecordMetrics }) {
    ...
    return (<li className="rounded-md border border-neutral-100 p-3 text-sm">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-neutral-400">
          ...
          {canManage && (<button className="underline-offset-2 hover:underline" onClick={() => setEditing(!editing)}>
              {editing ? 'cancel' : 'record metrics'}
            </button>)}
        </div>
      </div>
      <p className="mt-1 text-neutral-600">{post.content}</p>
      {editing && (<div className="mt-2 flex items-end gap-2">
