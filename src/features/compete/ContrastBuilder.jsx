...
import { useContrast } from './useCompete';
...
export function ContrastBuilder({ orgId, projectId, records }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-indigo-200 bg-white p-5">
      ...
      <textarea rows={2} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" placeholder="Our position, in your words. e.g. We support renewing the school levy — it funds 40 classroom aides." value={position} onChange={(e) => setPosition(e.target.value)}/>
      <Button size="sm" onClick={() => record && contrast.mutate({ orgId, projectId, record, ourPosition: position.trim() })} disabled={contrast.isPending || !record || !position.trim()}>
        <Sparkles className="h-4 w-4"/>
        {contrast.isPending ? 'Building…' : 'Build contrast'}
      </Button>
      {contrast.isError && <p className="text-sm text-red-600">{contrast.error.message}</p>}

      {contrast.data && (<div className="space-y-2">
          ...
          <div className="rounded-md border border-indigo-100 bg-indigo-50 p-3">
            ...
            <ul className="mt-1 list-disc space-y-0.5 pl-4 text-xs text-neutral-700">
              {contrast.data.talking_points.map((tp, i) => (<li key={i}>{tp}</li>))}
            </ul>
          </div>
        </div>)}
    </div>);
