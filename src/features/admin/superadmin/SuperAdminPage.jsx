...
import { useActivateOrg, useAllOrganizations, useOrgEntitlements, useSetEntitlement } from './useAdminData';
...
export function SuperAdminPage() {
    ...
    return (<div className="mx-auto max-w-5xl space-y-8 px-4 py-8">
      <div>
      ...
      <section>
        ...
        {pending.length === 0 && (<p className="rounded-md border border-dashed border-neutral-300 p-4 text-sm text-neutral-400">
            No organizations awaiting activation.
          </p>)}
        <div className="space-y-3">
          {pending.map((org) => (<PendingOrgCard key={org.id} org={org}/>))}
        </div>
      </section>
      ...
      <section>
        ...
        <div className="overflow-hidden rounded-lg border border-neutral-200 bg-white">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rest.map((org) => (<tr key={org.id} className="border-t border-neutral-100">
                  ...
                  <td className="px-4 py-2">
                    <span className={cn('rounded-full px-2 py-0.5 text-xs font-medium', org.status === 'active'
                ...
                : 'bg-neutral-100 text-neutral-600')}>
                  ...
                  <td className="px-4 py-2 text-right">
                    <Button variant="ghost" size="sm" onClick={() => setSelectedOrgId(org.id === selectedOrgId ? null : org.id)}>
...
function PendingOrgCard({ org }) {
    const activateOrg = useActivateOrg();
    return (<div className="flex items-center justify-between rounded-lg border border-amber-200 bg-amber-50 p-4">
      <div>
      ...
      {activateOrg.isError && (<p className="text-xs text-red-600">{activateOrg.error.message}</p>)}
    </div>);
}
function EntitlementManager({ org }) {
    ...
    return (<section className="rounded-lg border border-neutral-200 bg-white p-5">
      ...
      <div className="mt-4 space-y-2">
        {ORG_ENTITLEMENT_KEYS.map((key) => {
            ...
            return (<div key={key} className="flex items-center justify-between rounded-md border border-neutral-100 px-3 py-2">
              <div>
                <code className="text-sm text-neutral-800">{key}</code>
                {existing && (<span className="ml-2 text-xs text-neutral-400">({existing.granted_reason})</span>)}
              </div>
              <Button variant={granted ? 'destructive' : 'default'} size="sm" disabled={setEntitlement.isPending} onClick={() => setEntitlement.mutate({
                    orgId: org.id,
                    key,
                    granted: !granted,
      ...
      {projectLevel.length > 0 && (<div className="mt-4">
          <h3 className="text-xs font-semibold uppercase tracking-wide text-neutral-500">
            Project-scoped
          </h3>
          <ul className="mt-2 space-y-1 text-sm text-neutral-600">
            {projectLevel.map((e) => (<li key={e.id}>
      ...
      {setEntitlement.isError && (<p className="mt-3 text-sm text-red-600">{setEntitlement.error.message}</p>)}
    </section>);
}
