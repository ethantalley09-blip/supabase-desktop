...
import { formatUsd, type Donation } from '@/features/fundraising/useFundraising';
...
export function CampaignCommandCenter({ voters, territories, donations, totalCents }: { voters: VoterRecord[]; territories: Territory[]; donations: Donation[]; totalCents: number }) {
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-md bg-neutral-50 px-2 py-2"><div className="font-semibold text-neutral-900">{value}</div><div className="mt-0.5 text-[11px] text-neutral-500">{label}</div></div>; }
