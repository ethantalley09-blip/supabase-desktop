...
import { guessFieldMapping, isBlankRow, parseVoterFile } from './parseFile';
const CHUNK_SIZE = 500;
export function ImportWizard({ projectId, orgId, onDone }) {
    const { user } = useAuth();
    ...
    const suggestMapping = async () => {
        ...
        const pick = (v) => (typeof v === 'string' && sheet.columns.includes(v) ? v : '');
        setMapping({
    ...
    return (<div className="space-y-4 rounded-lg border border-neutral-200 bg-white p-5">
      <h3 className="text-sm font-semibold text-neutral-900">Import voter list</h3>

      {!sheet && (<div>
          <label onDragOver={onDragOver} onDragEnter={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} className={`flex cursor-pointer flex-col items-center gap-2 rounded-lg border-2 border-dashed px-6 py-10 text-center transition-colors ${dragActive ? 'border-neutral-900 bg-neutral-50' : 'border-neutral-300 hover:border-neutral-400'}`}>
            ...
            <input type="file" accept=".csv,.xlsx,.xls,.xlsm,.xlsb,.ods,.tsv,.txt" className="sr-only" onChange={(e) => e.target.files?.[0] && onFile(e.target.files[0])}/>
          </label>
      ...
      {sheet && (<>
          <div className="flex items-start justify-between gap-3">
            ...
            {showAiSuggest && (<Button variant="outline" size="sm" className="shrink-0" onClick={suggestMapping} disabled={suggest.isPending}>
                <Sparkles className="h-4 w-4"/>
                {suggest.isPending ? 'Reading…' : 'Suggest with AI'}
              </Button>)}
          </div>
          {suggest.isError && (<p className="text-sm text-red-600">{suggest.error.message}</p>)}
          {aiNotes && (<p className="rounded-md border border-violet-200 bg-violet-50 px-3 py-2 text-sm text-violet-900">
              {aiNotes}
            </p>)}
          {sheet.parseWarnings.length > 0 && (<p className="flex items-start gap-1.5 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
                ...
                ['lat', 'Latitude'],
                ['lng', 'Longitude']
            ].map(([key, label]) => (<div key={key} className="space-y-1.5">
                <Label>{label}</Label>
                <select className="flex h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm" value={mapping[key]} onChange={(e) => setMapping({ ...mapping, [key]: e.target.value })}>
                  <option value="">Not in file</option>
                  {sheet.columns.map((c) => (<option key={c} value={c}>
                      {c}
                    </option>))}
                </select>
              </div>))}
          ...
          {importRows.isError && (<p className="text-sm text-red-600">{importRows.error.message}</p>)}
          {progress && importRows.isPending && (<div className="space-y-1">
              <div className="h-1.5 w-full overflow-hidden rounded-full bg-neutral-100">
                <div className="h-full rounded-full bg-neutral-900 transition-all" style={{ width: `${Math.round((progress.done / progress.total) * 100)}%` }}/>
              </div>
              <p className="text-xs text-neutral-400">
                Importing {progress.done.toLocaleString()} of {progress.total.toLocaleString()}…
              </p>
            </div>)}
          {importRows.data && (<div className={`space-y-1 rounded-md border p-3 text-xs ${importRows.data.failed === 0
                    ? 'border-emerald-200 bg-emerald-50 text-emerald-800'
                    : 'border-amber-200 bg-amber-50 text-amber-800'}`}>
              <p className="flex items-center gap-1.5 font-medium">
                {importRows.data.failed === 0 ? (<CheckCircle2 className="h-3.5 w-3.5"/>) : (<AlertTriangle className="h-3.5 w-3.5"/>)}
              ...
              {importRows.data.chunkErrors.map((msg, i) => (<p key={i} className="text-amber-700">{msg}</p>))}
              {importRows.data.failed > 0 && (<Button size="sm" variant="outline" className="mt-1" onClick={onDone}>
                  Done
                </Button>)}
            </div>)}
