...
export function ComplianceTab({ project }) {
    ...
    return (<div className="space-y-4">
      ...
      {status?.threshold_met_at && (<p className="text-sm text-neutral-500">
          Compliance tools unlocked {format(new Date(status.threshold_met_at), 'PPp')} — this project
          crossed $1,000 in lifetime donations.
        </p>)}

      <PeriodReport project={project} itemizationThresholdCents={typeof federal?.ruleset?.['itemization_threshold_cents'] === 'number'
            ? federal.ruleset['itemization_threshold_cents']
            : null}/>

      <RulesetCard title="Federal ruleset" ruleset={federal ?? null} emptyNote="No federal ruleset seeded for this organization type."/>
      <RulesetCard title={project.state ? `State ruleset — ${project.state}` : 'State ruleset'} ruleset={state ?? null} emptyNote={project.state
...
function RulesetCard({ title, ruleset, emptyNote }) {
    return (<div className="rounded-lg border border-neutral-200 bg-white p-5">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-neutral-900">{title}</h3>
        {ruleset && (<span className={ruleset.reviewed_by_counsel
                ? 'rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700'
                : 'rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700'}>
            {ruleset.reviewed_by_counsel ? 'Counsel reviewed' : 'Awaiting counsel review'}
          </span>)}
      </div>
      {ruleset ? (<dl className="mt-3 space-y-1.5 text-sm">
          {Object.entries(ruleset.ruleset).map(([key, value]) => (<div key={key} className="flex gap-3">
              <dt className="w-64 shrink-0 text-neutral-400">{key}</dt>
              <dd className="text-neutral-800">
                {Array.isArray(value) ? value.join(', ') : String(value)}
              </dd>
            </div>))}
        </dl>) : (<p className="mt-3 text-sm text-neutral-400">{emptyNote}</p>)}
    </div>);
}
