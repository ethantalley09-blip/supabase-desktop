...
const MIN_SAMPLE = 3;
export function computePeakAskWindow(donations) {
    const linked = donations.filter((d) => d.voter_id);
    if (linked.length < MIN_SAMPLE)
        return null;
    const hourlyCents = new Array(24).fill(0);
    ...
    let bestHour = 0;
    for (let h = 1; h < 24; h++) {
        if (hourlyCents[h] > hourlyCents[bestHour])
            bestHour = h;
    }
    if (hourlyCents[bestHour] === 0)
        return null;
    return {
        bestHour,
