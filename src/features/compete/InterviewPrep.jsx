...
export function InterviewPrep({ orgId, projectId }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      <div className="flex flex-wrap gap-2">
        <Input className="w-56" placeholder="Format, e.g. WKRT morning radio, 10 min" value={format} onChange={(e) => setFormat(e.target.value)}/>
        <Input className="min-w-56 flex-1" placeholder="Likely topics, e.g. housing plan, recent endorsement, budget vote" value={topics} onChange={(e) => setTopics(e.target.value)}/>
        <Button size="sm" onClick={() => draft.mutate({ orgId, projectId, format: format.trim(), topics: topics.trim() })} disabled={draft.isPending || !format.trim() || !topics.trim()}>
      ...
      {draft.data && (<div className="space-y-3">
          ...
          <div>
            ...
            <ul className="mt-1 space-y-1">
              {draft.data.bridge_phrases.map((p, i) => (<li key={i} className="text-xs text-neutral-600">— {p}</li>))}
            </ul>
          </div>
          <div className="space-y-1.5">
            <p className="text-xs font-semibold text-neutral-700">Likely questions</p>
            {draft.data.likely_questions.map((q, i) => (<div key={i} className="rounded-md border border-neutral-200 bg-neutral-50 p-2.5 text-xs">
