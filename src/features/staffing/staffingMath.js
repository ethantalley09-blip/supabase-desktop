...
export function summarizeShiftsForDate(shifts, date) {
    const todays = shifts.filter((s) => s.shift_date === date);
    const teams = [...new Set(todays.map((s) => s.team_name).filter((t) => Boolean(t)))].sort();
    return {
        date,
...
export function summarizeLogistics(bookings) {
