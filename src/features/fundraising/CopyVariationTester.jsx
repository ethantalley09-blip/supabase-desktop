...
export function CopyVariationTester({ orgId, projectId }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-emerald-200 bg-white p-5">
      ...
      <textarea rows={3} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" placeholder="Base fundraising message…" value={baseMessage} onChange={(e) => setBaseMessage(e.target.value)}/>

      <div>
        <label className="text-xs font-semibold text-neutral-600">Org Context (JSON)</label>
        <textarea rows={2} className="mt-1 w-full rounded-md border border-neutral-300 px-3 py-2 text-xs font-mono" value={orgContext} onChange={(e) => setOrgContext(e.target.value)}/>
      </div>
      ...
      {generateVariations.isError && (<p className="text-sm text-red-600">{generateVariations.error.message}</p>)}

      {result && (<div className="space-y-2">
          <p className="text-xs text-neutral-600 italic">{result.notes}</p>

          {['a', 'b', 'c'].map((variant) => (<div key={variant} className="space-y-1 rounded-md border border-neutral-200 bg-neutral-50 p-3">
              <p className="text-xs font-semibold uppercase text-neutral-500">Variant {variant.toUpperCase()}</p>
              <p className="text-sm text-neutral-800">
                {result[`variant_${variant}`]}
              </p>
