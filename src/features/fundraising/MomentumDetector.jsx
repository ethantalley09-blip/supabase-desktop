...
// moment, news hit).
export function MomentumDetector({ orgId, projectId, donations }) {
    ...
    const stats = useMemo(() => {
        ...
        const baselineAvg = olderWindows.length > 0
    ...
    return (<div className="space-y-3 rounded-lg border border-red-200 bg-white p-5">
      ...
      {stats && (<p className="text-xs text-neutral-600">
      ...
      <div className="flex gap-2">
        <Input placeholder="What's happening right now? (optional, e.g. 'just went viral after the debate')" value={trigger} onChange={(e) => setTrigger(e.target.value)}/>
        <Button size="sm" onClick={run} disabled={detect.isPending || !stats}>
          <Sparkles className="h-4 w-4"/>
          {detect.isPending ? 'Checking…' : 'Check Momentum'}
        </Button>
      </div>
      {detect.isError && <p className="text-sm text-red-600">{detect.error.message}</p>}

      {detect.data && (<div className={`rounded-md border p-3 ${detect.data.spikeMultiplier >= 2 ? 'border-red-200 bg-red-50' : 'border-neutral-200 bg-neutral-50'}`}>
