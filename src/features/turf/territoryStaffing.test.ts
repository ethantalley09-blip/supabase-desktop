...
import { computeTerritoryStaffing, suggestReallocations, type VisitForStaffing } from './territoryStaffing';
...
function territory(id: string, name: string): Territory {
  return { id, name } as Territory;
}

function voter(id: string, territoryId: string): VoterRecord {
  return {
    id,
    project_id: 'p1',
    data: {},
...
function visit(voterId: string, canvasserId: string, days: number): VisitForStaffing {
