...
import { canvasserLeaderboard, scoreDoors } from './doorstep';
...
export function DoorstepDonations({ orgId, projectId, voters, canUseAi }) {
    ...
    const [cascadeFor, setCascadeFor] = useState(null);
    const [cascadeAsk, setCascadeAsk] = useState(null);
    const [recurringFor, setRecurringFor] = useState(null);
    const [recurringAsk, setRecurringAsk] = useState(null);
    const [recordingFor, setRecordingFor] = useState(null);
    const [recordAmount, setRecordAmount] = useState('');
    const [referralFor, setReferralFor] = useState(null);
    const [referralAsk, setReferralAsk] = useState(null);
    ...
    const cascadeTargets = useMemo(() => findHouseholdCascadeTargets(households, donations ?? []).slice(0, 5), [households, donations]);
    const recurringTargets = useMemo(() => findDoorstepRecurringTargets(donations ?? [], visits ?? []).slice(0, 5), [donations, visits]);
    ...
    const getPitch = (d) => {
        ...
        ai.mutate({
            ...
            onSuccess: (res) => setPitch(extractJson(res.text))
        });
    };
    const getCascadeAsk = (target, askTarget) => {
        ...
        }, { onSuccess: (res) => setCascadeAsk(extractJson(res.text)) });
    };
    const getRecurringAsk = (target) => {
        ...
        }, { onSuccess: (res) => setRecurringAsk(extractJson(res.text)) });
    };
    const startRecording = (d) => {
        ...
        recordDonation.mutate({
            ...
            onSuccess: () => {
                ...
                if (canUseAi) {
                    ...
                    }, { onSuccess: (res) => setReferralAsk(extractJson(res.text)) });
                }
            }
        });
    };
    return (<div className="space-y-3 rounded-lg border border-emerald-200 bg-white p-5">
      ...
      {warmDoors.length === 0 ? (<p className="text-sm text-neutral-500">
          ...
          {warmDoors.map((d) => (<div key={d.voterId} className="rounded-md border border-emerald-100 bg-emerald-50 p-2.5">
              ...
              <div className="mt-1.5 flex flex-wrap gap-2">
                {canUseAi && (<Button size="sm" variant="outline" onClick={() => getPitch(d)} disabled={ai.isPending}>
                    <Sparkles className="h-3.5 w-3.5"/>
                    {ai.isPending && pitchFor === d.voterId ? 'Writing…' : '20-second ask'}
                  </Button>)}
                {canRecordGift && recordingFor !== d.voterId && (<Button size="sm" variant="outline" onClick={() => startRecording(d)}>
                    <HandCoins className="h-3.5 w-3.5"/>
                    Record gift
                  </Button>)}
              </div>
              {pitch && pitchFor === d.voterId && (<div className="mt-2 space-y-1 rounded bg-white p-2.5 text-xs text-neutral-700">
              ...
              {recordingFor === d.voterId && (<div className="mt-2 flex items-center gap-2 rounded bg-white p-2.5">
                  <span className="text-xs text-neutral-500">$</span>
                  <Input type="number" min="1" step="1" className="h-7 w-24" value={recordAmount} onChange={(e) => setRecordAmount(e.target.value)} autoFocus/>
              ...
              {referralFor === d.voterId && (referralAi.isPending || referralAsk || referralAi.isError) && (<div className="mt-2 space-y-1 rounded border border-amber-200 bg-amber-50 p-2.5 text-xs text-neutral-700">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-amber-700">
                    Doorstep Referral Ask
                  </p>
                  {referralAi.isPending && <p className="text-neutral-500">Drafting…</p>}
                  {referralAi.isError && <p className="text-red-600">{referralAi.error.message}</p>}
                  {referralAsk && (<>
      ...
      {recordDonation.isError && (<p className="text-sm text-red-600">{recordDonation.error.message}</p>)}

      {cascadeTargets.length > 0 && canUseAi && (<div className="space-y-2 rounded-md border border-indigo-200 bg-indigo-50 p-3">
          ...
          <div className="space-y-2">
            {cascadeTargets.map((t) => (<div key={t.householdKey} className="rounded bg-white p-2 text-xs">
                <p className="text-neutral-600">
                  {t.address}: {t.givingMemberName} already gave {formatUsd(t.givingMemberAmountCents)}
                </p>
                {t.askTargets.map((a) => (<div key={a.voterId} className="mt-1 space-y-1">
                    <div className="flex items-center justify-between gap-2">
                    ...
                    {cascadeAsk && cascadeFor === a.voterId && (<div className="space-y-1 rounded bg-indigo-50 p-2 text-neutral-700">
          ...
          {cascadeAi.isError && <p className="text-sm text-red-600">{cascadeAi.error.message}</p>}
        </div>)}

      {recurringTargets.length > 0 && canUseAi && (<div className="space-y-2 rounded-md border border-teal-200 bg-teal-50 p-3">
          ...
          <div className="space-y-2">
            {recurringTargets.map((t) => (<div key={t.voterId} className="space-y-1 rounded bg-white p-2 text-xs">
                <div className="flex items-center justify-between gap-2">
                ...
                {recurringAsk && recurringFor === t.voterId && (<div className="space-y-1 rounded bg-teal-50 p-2 text-neutral-700">
          ...
          {recurringAi.isError && <p className="text-sm text-red-600">{recurringAi.error.message}</p>}
        </div>)}

      {leaderboard.length > 0 && (<div className="rounded-md border border-neutral-200 p-3">
          ...
          <div className="mt-1.5 space-y-1">
            {leaderboard.map((r, i) => (<div key={r.recorderId} className="flex items-center justify-between text-xs">
                <span className="text-neutral-700">
                  {i + 1}. {r.name}
                </span>
          ...
          {shoutout.data && (<p className="mt-2 flex items-start gap-1.5 border-t border-neutral-100 pt-2 text-xs text-violet-700">
