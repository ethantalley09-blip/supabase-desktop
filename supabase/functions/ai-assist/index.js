...
function systemFor(purpose) {
    if (purpose === 'email_campaign')
        return EMAIL_CAMPAIGN_SYSTEM;
    if (purpose === 'press_release')
        return PRESS_RELEASE_SYSTEM;
    if (purpose === 'media_pitch')
        return MEDIA_PITCH_SYSTEM;
    if (purpose === 'direct_mail')
        return DIRECT_MAIL_SYSTEM;
    if (purpose === 'phone_script')
        return PHONE_SCRIPT_SYSTEM;
    if (purpose === 'volunteer_pipeline')
    ...
    if (purpose === 'door_objection_assist')
        return DOOR_OBJECTION_SYSTEM;
    if (purpose === 'door_script_personalize')
        return DOOR_SCRIPT_SYSTEM;
    if (purpose === 'door_explainer')
        return DOOR_EXPLAINER_SYSTEM;
    if (purpose === 'door_language_prep')
        return DOOR_LANGUAGE_SYSTEM;
    if (purpose === 'shift_debrief')
        return SHIFT_DEBRIEF_SYSTEM;
    if (purpose === 'territory_difficulty_briefing')
        return TERRITORY_DIFFICULTY_SYSTEM;
    if (purpose === 'revisit_strategy')
        return REVISIT_STRATEGY_SYSTEM;
    if (purpose === 'momentum_ask_script')
        return MOMENTUM_ASK_SYSTEM;
    if (purpose === 'household_cascade_ask')
        return HOUSEHOLD_CASCADE_SYSTEM;
    if (purpose === 'peak_ask_briefing')
        return PEAK_ASK_SYSTEM;
    if (purpose === 'territory_roi_briefing')
        return TERRITORY_ROI_SYSTEM;
    if (purpose === 'persistence_ask_script')
        return PERSISTENCE_ASK_SYSTEM;
    if (purpose === 'golden_hour_ask_plan')
        return GOLDEN_HOUR_SYSTEM;
    if (purpose === 'ask_coverage_alert')
        return ASK_COVERAGE_SYSTEM;
    if (purpose === 'canvasser_ask_coaching')
        return CANVASSER_COACH_SYSTEM;
    if (purpose === 'election_countdown_ask')
        return ELECTION_COUNTDOWN_SYSTEM;
    if (purpose === 'priority_door_briefing')
        return PRIORITY_DOOR_SYSTEM;
    if (purpose === 'canvasser_checkin_prompt')
        return CANVASSER_CHECKIN_SYSTEM;
    if (purpose === 'territory_staffing_briefing')
        return TERRITORY_STAFFING_SYSTEM;
    if (purpose === 'map_area_briefing')
        return MAP_AREA_BRIEFING_SYSTEM;
    if (purpose === 'canvasser_silence_checkin')
        return CANVASSER_SILENCE_SYSTEM;
    if (purpose === 'neighborhood_proof_ask')
        return NEIGHBORHOOD_PROOF_SYSTEM;
    if (purpose === 'donation_objection_handler')
        return DONATION_OBJECTION_SYSTEM;
    if (purpose === 'ask_rehearsal_prep')
        return ASK_REHEARSAL_SYSTEM;
    if (purpose === 'doorstep_referral_ask')
        return DOORSTEP_REFERRAL_SYSTEM;
    if (purpose === 'bundler_cultivation_ask')
        return BUNDLER_CULTIVATION_SYSTEM;
    if (purpose === 'event_planning_briefing')
        return EVENT_PLANNING_SYSTEM;
    if (purpose === 'doorstep_recurring_ask')
        return DOORSTEP_RECURRING_SYSTEM;
    if (purpose === 'mistake_response')
        return MISTAKE_RESPONSE_SYSTEM;
    ...
    if (purpose === 'doorstep_pitch')
        return DOORSTEP_PITCH_SYSTEM;
    if (purpose === 'contrast_message')
        return CONTRAST_SYSTEM;
    if (purpose === 'rebuttal')
        return REBUTTAL_SYSTEM;
    if (purpose === 'debate_prep')
        return DEBATE_PREP_SYSTEM;
    if (purpose === 'self_opposition')
        return RED_TEAM_SYSTEM;
    if (purpose === 'opponent_digest')
        return OPPONENT_DIGEST_SYSTEM;
    if (purpose === 'emergency_ask')
        return EMERGENCY_ASK_SYSTEM;
    if (purpose === 'funding_runway')
        return FUNDING_RUNWAY_SYSTEM;
    if (purpose === 'network_ask')
        return NETWORK_ASK_SYSTEM;
    if (purpose === 'reactivation_sequence')
        return REACTIVATION_SYSTEM;
    if (purpose === 'issue_response')
        return ISSUE_RESPONSE_SYSTEM;
    if (purpose === 'refine')
        return REFINE_SYSTEM;
    if (purpose === 'major_donor_escalation')
        return MAJOR_DONOR_SYSTEM;
    if (purpose === 'fatigue_guard')
        return FATIGUE_GUARD_SYSTEM;
    if (purpose === 'fec_sprint_plan')
        return FEC_SPRINT_SYSTEM;
    if (purpose === 'retention_sequence')
        return RETENTION_SYSTEM;
    if (purpose === 'payment_recovery')
        return PAYMENT_RECOVERY_SYSTEM;
    if (purpose === 'volunteer_donor_bridge')
        return VOLUNTEER_BRIDGE_SYSTEM;
    if (purpose === 'momentum_alert')
        return MOMENTUM_SYSTEM;
    if (purpose === 'recurring_upgrade')
        return RECURRING_UPGRADE_SYSTEM;
    if (purpose === 'ltv_forecast')
        return LTV_FORECAST_SYSTEM;
    if (purpose === 'donor_dedup')
        return DEDUP_SYSTEM;
    if (purpose === 'refund_risk_scan')
        return REFUND_RISK_SYSTEM;
    if (purpose === 'ask_optimization')
        return ASK_OPTIMIZATION_SYSTEM;
    if (purpose === 'churn_prediction')
        return CHURN_PREDICTION_SYSTEM;
    if (purpose === 'connector_scoring')
        return CONNECTOR_SCORING_SYSTEM;
    if (purpose === 'compliant_variation')
        return COMPLIANT_VARIATION_SYSTEM;
    if (purpose === 'note_summary')
        return ANALYSIS_SYSTEM;
    if (purpose === 'data_qa')
        return DATA_SYSTEM;
    if (purpose === 'geocode_strategy')
        return GEOCODE_SYSTEM;
    if (purpose === 'field_coach')
        return COACH_SYSTEM;
    if (purpose === 'import_mapping')
        return IMPORT_SYSTEM;
    if (purpose === 'translate')
        return TRANSLATE_SYSTEM;
    if (purpose === 'donor_message')
        return DONOR_SYSTEM;
    if (purpose === 'segment_filter')
        return SEGMENT_SYSTEM;
    if (purpose === 'content_pack')
        return CONTENT_PACK_SYSTEM;
...
function buildPrompt(b) {
    if (b.purpose === 'note_summary') {
        ...
        if (notes.length === 0)
            return 'No canvass notes were provided.';
    ...
    if (b.purpose === 'donation_objection_handler') {
        ...
        if (b.context?.trim())
            lines.push(`Door context (JSON): ${b.context}`);
    ...
    if (b.purpose === 'door_objection_assist') {
        ...
        if (b.context?.trim())
            lines.push(`Our real position on this, as staff typed it: ${b.context}`);
    ...
    if (b.purpose === 'broadcast') {
        ...
        lines.push('Draft a short, warm personal text message a supporter can send to someone they personally know.', 'Because modern phones filter texts from unknown numbers, the message must FIRST politely ask the recipient to save the sender/campaign number, THEN make the ask. Keep it personal, not mass-market. Under 60 words.');
    }
    if (b.audience)
        lines.push(`Audience: ${b.audience}.`);
    if (b.tone)
...
Deno.serve(async (req) => {
    ...
    if (!apiKey)
        return json({ error: 'AI is not configured on this server.' }, 503);
    let body;
    try {
        body = (await req.json());
    }
    ...
    if (body.purpose === 'note_summary') {
        ...
        if (!body.context?.trim())
            return json({ error: 'context (data snapshot) is required' }, 400);
        ...
        if (!body.instructions?.trim())
            return json({ error: 'text to translate is required' }, 400);
        if (!body.language?.trim())
            return json({ error: 'a target language is required' }, 400);
    }
    else if (body.purpose === 'refine') {
        if (!body.context?.trim())
            return json({ error: 'the message to refine is required' }, 400);
        if (!body.instructions?.trim())
            return json({ error: 'a refine instruction is required' }, 400);
    }
    else if (body.purpose === 'ask_optimization' || body.purpose === 'churn_prediction' || body.purpose === 'connector_scoring') {
        if (!body.context?.trim())
            return json({ error: 'donor history/profile (context) is required' }, 400);
    }
    else if (body.purpose === 'compliant_variation') {
        if (!body.instructions?.trim())
            return json({ error: 'the base message is required' }, 400);
        if (!body.context?.trim())
            return json({ error: 'org context (calendar/budget) is required' }, 400);
    }
    else if (body.purpose === 'major_donor_escalation' ||
    ...
    if (entErr)
        return json({ error: entErr.message }, 500);
    if (!entitled) {
    ...
    try {
        ...
        const text = message.content
            ...
            .trim();
        if (!text)
            return json({ error: 'The model returned no text (possibly a refusal).' }, 502);
        return json({ text });
    }
    catch (e) {
        return json({ error: `AI request failed: ${e.message}` }, 502);
    }
});
