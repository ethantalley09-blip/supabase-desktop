...
export function AskOptimizer({
  ...
  return (
    <div className="space-y-3 rounded-lg border border-violet-200 bg-white p-5">
      ...
      {optimizeAsk.isError && <p className="text-sm text-red-600">{(optimizeAsk.error as Error).message}</p>}

      {result && (
        <div className="space-y-2 rounded-md border border-violet-100 bg-violet-50 p-3">
