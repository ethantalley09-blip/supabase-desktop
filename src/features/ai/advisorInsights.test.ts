...
import {
  answerAvgDoorsPerCanvasserToday,
  answerBestContactRateTerritory,
  answerBestDayOfWeek,
  answerBestPerformingPost,
  answerBestTimeToKnock,
  answerSkipADayImpact,
  answerCanvasserMomentum,
  answerCanvasserWellbeing,
  answerCoachingPairs,
  answerCurrentDoorScript,
  answerCurrentSurveyQuestions,
  answerDonorConcentration,
  answerDonorGrowthVsAverageGift,
  answerDonorRepeatShare,
  answerDoorsKnockedToday,
  answerDoorstepAttribution,
  answerFundraisingPace,
  answerHardestTerritory,
  answerLodgingCost,
  answerNextTerritoryToCanvass,
  answerPaymentMethodBreakdown,
  answerPersuasionDrift,
  answerPostingFrequency,
  answerRevisitCandidates,
  answerSocialPerformance,
  answerTeamContactRate,
  answerTerritorySupportBreakdown,
  answerTodaysContactRate,
  answerTodaysStaffing,
  answerTodayVsBaseline,
  answerTopCanvasser,
  answerTopCanvasserThisWeek,
  answerTopFundraiser,
  answerUnattemptedDoors,
  answerWalkbookSizeOutliers,
  answerWeekendVsWeekday,
  answerWeeklySocialReach,
  answerWorstContactRateTerritory,
  computeAverageDailyDoors,
  computeCanvasserMomentum,
  computeDayOfWeekPerformance,
  computeDoorstepAttribution,
  computeFundraisingPace,
  computeSocialPerformance,
  computeTeamContactRates,
  computeTerritorySupportBreakdown,
  computeWalkbookSizes,
  describeContactRateDropAlert,
  describeDataFreshnessAlert,
  describeFundraisingMomentumAlert,
  rankTerritoriesByContactRate,
  scenarioAddWorkers,
  scenarioAdjustPace
} from './advisorInsights';
import type { AdvisorSocialPost } from './advisorData';
import type { CanvasserVisitLike } from '@/features/turf/canvasserStats';
import type { TerritoryDifficulty, TerritoryVisit } from '@/features/turf/territoryDifficulty';
import type { Territory, VoterRecord } from '@/features/turf/useTurf';
import type { VisitLike } from '@/features/turf/visitHistory';
import type { VisitForRevisit } from '@/features/turf/revisitQueue';
import type { Donation } from '@/features/fundraising/useFundraising';

describe('answerTopCanvasser', () => {
  const visits: CanvasserVisitLike[] = [
...
const daysAgoIso = (n: number, hour = 12) => {
...
describe('answerDoorsKnockedToday / answerTodaysContactRate / answerAvgDoorsPerCanvasserToday', () => {
  ...
  it('count only real visits from today, ignoring other days', () => {
    const visits = [
      { occurred_at: daysAgoIso(0), outcome: 'contacted' as const, canvasser_id: 'a' },
      { occurred_at: daysAgoIso(0), outcome: 'no_answer' as const, canvasser_id: 'b' },
      { occurred_at: daysAgoIso(1), outcome: 'contacted' as const, canvasser_id: 'a' }
    ];
...
describe('answerTopCanvasserThisWeek', () => {
  it('only counts real visits from the trailing 7 days', () => {
    const visits = [
      { canvasser_id: 'a', canvasser_name: 'Ana', outcome: 'contacted' as const, occurred_at: daysAgoIso(2) },
      { canvasser_id: 'a', canvasser_name: 'Ana', outcome: 'contacted' as const, occurred_at: daysAgoIso(2) },
      { canvasser_id: 'a', canvasser_name: 'Ana', outcome: 'no_answer' as const, occurred_at: daysAgoIso(2) },
      { canvasser_id: 'b', canvasser_name: 'Ben', outcome: 'contacted' as const, occurred_at: daysAgoIso(10) },
      { canvasser_id: 'b', canvasser_name: 'Ben', outcome: 'contacted' as const, occurred_at: daysAgoIso(10) },
      { canvasser_id: 'b', canvasser_name: 'Ben', outcome: 'contacted' as const, occurred_at: daysAgoIso(10) }
    ];
...
describe('computeTeamContactRates / answerTeamContactRate', () => {
  ...
  it('attributes real visits to a team only when the canvasser+date matches a team-tagged shift', () => {
    const visits = [
      { canvasser_id: 'a', occurred_at: '2026-01-15T10:00:00Z', outcome: 'contacted' as const },
      { canvasser_id: 'a', occurred_at: '2026-01-15T10:30:00Z', outcome: 'no_answer' as const },
      { canvasser_id: 'b', occurred_at: '2026-01-15T10:00:00Z', outcome: 'contacted' as const },
      { canvasser_id: 'c', occurred_at: '2026-01-15T10:00:00Z', outcome: 'contacted' as const } // no shift -> unattributed
    ];
...
describe('computeDayOfWeekPerformance / answerBestDayOfWeek / answerWeekendVsWeekday', () => {
  ...
  it('compares real weekend vs weekday contact rates', () => {
    ...
    const weekday = Array.from({ length: 6 }, () => ({ occurred_at: '2026-01-15T10:00:00Z', outcome: 'no_answer' as const }));
...
describe('answerCoachingPairs', () => {
  it('pairs the strongest canvasser with the one who has the most room to grow', () => {
    const visits: CanvasserVisitLike[] = [
      ...Array.from({ length: 4 }, () => ({ canvasser_id: 'a', canvasser_name: 'Ana', outcome: 'contacted' as const })),
      ...Array.from({ length: 4 }, () => ({ canvasser_id: 'b', canvasser_name: 'Ben', outcome: 'no_answer' as const }))
    ];
...
describe('answerHardestTerritory / rankTerritoriesByContactRate', () => {
  const voters: VoterRecord[] = [
    { id: 'v1', territory_id: 't1' } as VoterRecord,
    { id: 'v2', territory_id: 't1' } as VoterRecord,
    { id: 'v3', territory_id: 't1' } as VoterRecord,
    { id: 'v4', territory_id: 't2' } as VoterRecord,
    { id: 'v5', territory_id: 't2' } as VoterRecord,
    { id: 'v6', territory_id: 't2' } as VoterRecord
  ];
  const territories: Territory[] = [
    { id: 't1', name: 'North' } as Territory,
    { id: 't2', name: 'South' } as Territory
  ];
  const visits: TerritoryVisit[] = [
...
describe('answerNextTerritoryToCanvass', () => {
  ...
  it('picks the real territory with the most remaining doors, even with zero visits logged', () => {
    const voters: VoterRecord[] = [
      { id: 'v1', territory_id: 't1', contact_status: 'active', lat: 1, lng: 1, last_contacted_at: null } as VoterRecord,
      { id: 'v2', territory_id: 't1', contact_status: 'active', lat: 1, lng: 1, last_contacted_at: null } as VoterRecord,
      { id: 'v3', territory_id: 't2', contact_status: 'active', lat: 1, lng: 1, last_contacted_at: null } as VoterRecord
    ];
    const territories: Territory[] = [{ id: 't1', name: 'North' } as Territory, { id: 't2', name: 'South' } as Territory];
...
describe('computeTerritorySupportBreakdown / answerTerritorySupportBreakdown', () => {
  const voters: VoterRecord[] = [
    { id: 'v1', territory_id: 't1' } as VoterRecord,
    { id: 'v2', territory_id: 't2' } as VoterRecord
  ];
  const territories: Territory[] = [{ id: 't1', name: 'North' } as Territory, { id: 't2', name: 'South' } as Territory];
...
describe('computeWalkbookSizes / answerWalkbookSizeOutliers', () => {
  ...
  it('flags a real oversized walkbook relative to the average', () => {
    const territories: Territory[] = [
      { id: 't1', name: 'Big' } as Territory,
      { id: 't2', name: 'Normal A' } as Territory,
      { id: 't3', name: 'Normal B' } as Territory
    ];
    const voters: VoterRecord[] = [
      ...Array.from({ length: 30 }, (_, i) => ({ id: `big${i}`, territory_id: 't1' }) as VoterRecord),
      ...Array.from({ length: 10 }, (_, i) => ({ id: `a${i}`, territory_id: 't2' }) as VoterRecord),
      ...Array.from({ length: 10 }, (_, i) => ({ id: `b${i}`, territory_id: 't3' }) as VoterRecord)
    ];
...
describe('answerUnattemptedDoors', () => {
  ...
  it('counts real active mapped voters with zero logged visits', () => {
    const voters: VoterRecord[] = [
      { id: 'v1', contact_status: 'active', lat: 1, lng: 1 } as VoterRecord,
      { id: 'v2', contact_status: 'active', lat: 1, lng: 1 } as VoterRecord,
      { id: 'v3', contact_status: 'moved', lat: 1, lng: 1 } as VoterRecord
    ];
...
describe('answerRevisitCandidates', () => {
  ...
  it('flags a real door with repeated no-answer attempts', () => {
    const voters: VoterRecord[] = [{ id: 'v1', full_name: 'Jamie' } as VoterRecord];
    const visits: VisitForRevisit[] = [
...
describe('answerPersuasionDrift', () => {
  ...
  it('reports a real warmed-up shift between two visits', () => {
    const visits = [
      { voter_id: 'v1', voter_name: 'Jamie', occurred_at: '2026-01-01T10:00:00Z', persuadability_bucket: 'opposed' as const },
      { voter_id: 'v1', voter_name: 'Jamie', occurred_at: '2026-01-05T10:00:00Z', persuadability_bucket: 'base_support' as const }
    ];
...
describe('answerCanvasserWellbeing', () => {
  ...
  const today = (h: number, m = 0) => new Date(Date.UTC(2026, 0, 15, h, m)).toISOString();
  ...
  it('flags a real gone-quiet canvasser', () => {
    const visits = [
      { canvasser_id: 'a', canvasser_name: 'Sam', occurred_at: today(9), outcome: 'contacted' as const },
      { canvasser_id: 'a', canvasser_name: 'Sam', occurred_at: today(9, 15), outcome: 'contacted' as const }
    ];
...
describe('answerTodaysStaffing', () => {
  ...
  it('reports the real headcount and team breakdown for today', () => {
    const shifts = [
      { profile_id: 'a', profile_name: 'Ana', shift_date: '2026-01-15', status: 'scheduled' as const, team_name: 'North' },
      { profile_id: 'b', profile_name: 'Ben', shift_date: '2026-01-15', status: 'worked' as const, team_name: 'South' },
      { profile_id: 'c', profile_name: 'Cam', shift_date: '2026-01-14', status: 'scheduled' as const, team_name: 'North' }
    ];
...
describe('answerCurrentSurveyQuestions', () => {
  ...
  it('lists the real active questions in order', () => {
    const entries = [
      { kind: 'survey_question' as const, content: 'Are you registered?', sortOrder: 0, active: true },
      { kind: 'survey_question' as const, content: 'Who will you support?', sortOrder: 1, active: true }
    ];
...
describe('answerPostingFrequency', () => {
  ...
  it('reports a real posts-per-week average without asserting a target', () => {
    const posts: AdvisorSocialPost[] = Array.from({ length: 8 }, (_, i) => ({
      platform: 'x',
      content: `Post ${i}`,
      status: 'posted',
      impressions: 100,
...
describe('describeContactRateDropAlert', () => {
  ...
  const priorWeekAt = (contactedCount: number) =>
    Array.from({ length: 7 }, (_, i) => ({
      occurred_at: new Date(todayStart.getTime() - (i + 1) * 86_400_000).toISOString(),
      outcome: i < contactedCount ? ('contacted' as const) : ('no_answer' as const)
    }));
  ...
  it('stays silent on a normal contact rate', () => {
    const today = Array.from({ length: 10 }, (_, i) => ({
      ...
      outcome: i < 3 ? ('contacted' as const) : ('no_answer' as const) // 30% today
    }));
  ...
  it('flags a genuine contact-rate drop vs. the trailing week', () => {
    const today = Array.from({ length: 10 }, (_, i) => ({
      occurred_at: new Date(todayStart.getTime() + i * 60_000).toISOString(),
      outcome: 'no_answer' as const // 0% today
    }));
