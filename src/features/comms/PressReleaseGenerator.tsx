...
export function PressReleaseGenerator({ orgId, projectId }: { orgId: string; projectId: string }) {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-neutral-300 bg-white p-5">
      ...
      <textarea
        ...
        placeholder="What's the announcement? e.g. Campaign announces endorsement from the county Teachers Union, effective today."
        value={announcement}
      ...
      <textarea
        rows={2}
        className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm"
        placeholder="Optional: a real quote to include, attributed to whoever said it"
        value={quote}
      ...
      <Button
        size="sm"
        onClick={() =>
          draft.mutate({ orgId, projectId, announcement: announcement.trim(), quote: quote.trim(), location: location.trim(), contact: contact.trim() })
        }
        disabled={draft.isPending || !announcement.trim()}
      >
      ...
      {draft.data && (
        <div className="rounded-md border border-neutral-200 bg-neutral-50 p-3 text-xs text-neutral-800">
