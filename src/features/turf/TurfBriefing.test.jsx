...
vi.mock('@/features/fundraising/useFundraising', async () => {
    const actual = await vi.importActual('@/features/fundraising/useFundraising');
    return { ...actual, useDonations: () => ({ data: mockDonations }) };
});
let mockVisits = [];
const saveMutate = vi.fn();
vi.mock('./useTurf', async () => {
    const actual = await vi.importActual('./useTurf');
    return {
        ...actual,
...
function renderBriefing(voters, territories = []) {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    const wrapper = ({ children }) => (<QueryClientProvider client={client}>{children}</QueryClientProvider>);
    return render(<TurfBriefing orgId="org1" projectId="proj1" voters={voters} territories={territories} canUseAi={true} onRebalance={vi.fn()}/>, { wrapper });
}
describe('TurfBriefing', () => {
    ...
    it('lists a stubborn door in the Revisit Queue and requests a strategy on click', async () => {
        ...
        expect(aiMutate).toHaveBeenCalledWith(expect.objectContaining({ purpose: 'revisit_strategy' }), expect.anything());
    });
    it('calls onRebalance with a route built from the remaining doors', async () => {
        const onRebalance = vi.fn();
        const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
        render(<QueryClientProvider client={client}>
        <TurfBriefing orgId="org1" projectId="proj1" voters={[voter({ id: 'v1' })]} territories={[]} canUseAi={true} onRebalance={onRebalance}/>
      </QueryClientProvider>);
