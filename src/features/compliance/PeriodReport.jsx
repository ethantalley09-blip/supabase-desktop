...
export function PeriodReport({ project, itemizationThresholdCents }) {
    ...
    return (<div className="rounded-lg border border-neutral-200 bg-white p-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select className="h-8 rounded-md border border-neutral-300 bg-white px-2 text-sm" value={preset} onChange={(e) => setPreset(e.target.value)}>
          ...
          <Button variant="outline" size="sm" onClick={() => exportPdf(`${filePrefix}.pdf`, `${project.name} — donors, ${periodLabel}`, exportRows())}>
            PDF
          </Button>
        </div>
      </div>
      ...
      <table className="mt-4 w-full text-sm">
        ...
        <tbody>
          {rows.map((r) => (<tr key={r.donor} className="border-t border-neutral-100">
              ...
              <td className="py-1.5">
                {r.itemized ? (<span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">
                    Itemize
                  </span>) : (<span className="text-xs text-neutral-400">below threshold</span>)}
              </td>
            </tr>))}
          {rows.length === 0 && (<tr>
