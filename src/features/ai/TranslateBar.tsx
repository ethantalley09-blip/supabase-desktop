...
export function TranslateBar({
  ...
  const initial =
    defaultLanguage && TRANSLATION_LANGUAGES.includes(defaultLanguage as (typeof TRANSLATION_LANGUAGES)[number])
      ? defaultLanguage
      : TRANSLATION_LANGUAGES[0];
  const [language, setLanguage] = useState(initial);

  const run = () =>
    translate.mutate({ orgId, projectId, purpose: 'translate', instructions: text, language });
  ...
  return (
    <div className="space-y-2">
      ...
      {translate.isError && <p className="text-sm text-red-600">{(translate.error as Error).message}</p>}
      {translate.data && (
