...
import { useAcknowledge, useCreateBroadcast, useOrgRoles, useProjectThreads, useReply, useThreadMessages } from './useComms';
export function CommsTab({ project }) {
    ...
    return (<div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {canBroadcast.data && (<Button size="sm" onClick={() => setComposing(!composing)}>
      ...
      <div className="space-y-3">
        {threads?.map((t) => (<ThreadCard key={t.id} thread={t} orgId={project.org_id} projectId={project.id} showAi={showAi}/>))}
        {threads?.length === 0 && (<p className="rounded-md border border-dashed border-neutral-300 p-4 text-sm text-neutral-400">
            No broadcasts yet.
          </p>)}
      </div>
      ...
      {paidTier.data ? (<div className="space-y-4">
          ...
          <div id="tool-send_time_insight">
            <SendTimeInsight donations={donations} orgId={project.org_id} projectId={project.id} aiEnabled={showAi}/>
          </div>
        ...
        </div>) : (<div className="rounded-lg border border-dashed border-neutral-300 bg-neutral-50 p-4 text-sm text-neutral-500">
...
function BroadcastComposer({ project, onDone }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <select className="flex h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm" value={targetRoleId} onChange={(e) => setTargetRoleId(e.target.value)}>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          ...
          {aiEnabled.data && canUseAi.data && (<Button type="button" variant="outline" size="sm" onClick={draftWithAi} disabled={assist.isPending}>
              {assist.isPending ? 'Drafting…' : 'Draft with AI'}
            </Button>)}
        </div>
        <textarea id="body" rows={3} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" value={body} onChange={(e) => setBody(e.target.value)}/>
      </div>
      {error && <p className="text-sm text-red-600">{error}</p>}
      {assist.isError && <p className="text-sm text-red-600">{assist.error.message}</p>}
      {createBroadcast.isError && (<p className="text-sm text-red-600">{createBroadcast.error.message}</p>)}
      <Button size="sm" onClick={submit} disabled={createBroadcast.isPending}>
        Send broadcast
      </Button>
    </div>);
}
function ThreadCard({ thread, orgId, projectId, showAi }) {
...
function ThreadView({ threadId, orgId, projectId, showAi }) {
    const { user } = useAuth();
    ...
    return (<div className="space-y-3 border-t border-neutral-100 px-4 py-3">
      {messages?.map((m) => {
            const acked = m.message_acknowledgements.some((a) => a.profile_id === user?.id);
            return (<div key={m.id} className="rounded-md bg-neutral-50 p-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {m.sender_id !== user?.id && !acked && (<Button variant="outline" size="sm" onClick={() => acknowledge.mutate({ messageId: m.id, threadId })}>
      ...
      <div className="flex gap-2">
        <Input placeholder="Reply…" value={replyBody} onChange={(e) => setReplyBody(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendReply()}/>
        {showAi && (<Button variant="outline" size="sm" onClick={draftWithAi} disabled={draftReply.isPending}>
      ...
      {draftReply.isError && (<p className="text-sm text-red-600">{draftReply.error.message}</p>)}
    </div>);
}
