// Pure, dependency-free field helpers: city/ward extraction from imported
// voter rows, walk-order optimization, turf splitting, and the aggregate
// snapshot the AI features summarize. Kept out of useTurf.ts (which pulls in
// the Supabase client) so it stays unit-testable without env/config.
...
export function voterCity(v: VoterRecord): string | null {
  return pickField(v.data, ['city', 'city name', 'city_name']);
}

export function voterWard(v: VoterRecord): string | null {
...
export function voterLanguage(v: VoterRecord): string | null {
...
// present.
export function dominantVoterLanguage(voters: VoterRecord[]): string | null {
...
export type WalkRoute = { ordered: VoterRecord[]; meters: number };
...
function pathMeters(order: number[], coords: [number, number][]): number {
  let total = 0;
...
function twoOpt(order: number[], coords: [number, number][]): number[] {
  const d = (a: number, b: number) => haversineMeters(coords[a], coords[b]);
  ...
  while (improved) {
    ...
    for (let i = 1; i < route.length - 1; i++) {
      for (let k = i + 1; k < route.length; k++) {
        ...
        if (next !== -1) delta += d(b, next) - d(c, next);
        if (delta < -1e-6) {
          let lo = i;
          let hi = k;
          while (lo < hi) {
...
export function optimizeWalkOrder(voters: VoterRecord[]): WalkRoute {
  const pts = voters.filter((v) => v.lat !== null && v.lng !== null);
  const coords = pts.map((v) => [v.lng!, v.lat!] as [number, number]);
  const n = pts.length;
  if (n <= 2) return { ordered: pts, meters: pathMeters([...pts.keys()], coords) };
...
export function splitIntoWalkLists(voters: VoterRecord[], k: number): WalkRoute[] {
  const pts = voters.filter((v) => v.lat !== null && v.lng !== null);
  const n = pts.length;
  if (n === 0) return [];
  if (k <= 1 || n <= 1) return [optimizeWalkOrder(pts)];
  if (n <= k) return pts.map((p) => optimizeWalkOrder([p]));

  const coords = pts.map((v) => [v.lng!, v.lat!] as [number, number]);

  // Deterministic seeding: sweep west-to-east and pick k evenly spaced doors.
  const sweep = [...coords.keys()].sort(
    (a, b) => coords[a][0] - coords[b][0] || coords[a][1] - coords[b][1]
  );
  ...
  const byFit = [...Array(n).keys()].sort(
    (a, b) =>
      haversineMeters(coords[a], centroids[assign[a]]) -
  ...
  for (const i of byFit) {
    ...
    if (clusters[c].length >= cap) {
      ...
      for (let cc = 0; cc < k; cc++) {
        if (clusters[cc].length >= cap) continue;
...
// A whitelisted filter DSL for "Smart Segments" — the AI turns a plain-English
// description into one of these, and we apply it here. Keeping the schema fixed
// and the matching pure means the model can never reach the database or run
// arbitrary logic; unknown keys/values are simply ignored.
export type SegmentFilter = {
  ...
  mapped?: boolean;
  hasNotes?: boolean;
  assignedToTerritory?: boolean;
};

const CONTACT_VALUES = new Set<ContactStatus>(['active', 'moved', 'bad_address', 'deceased', 'do_not_contact']);
const BALLOT_VALUES = new Set(['none', 'requested', 'returned']);

export function applySegment(voters: VoterRecord[], filter: SegmentFilter): VoterRecord[] {
  const eq = (a: string | null, b: string) => (a ?? '').trim().toLowerCase() === b.trim().toLowerCase();
  return voters.filter((v) => {
    if (filter.contact_status && CONTACT_VALUES.has(filter.contact_status as ContactStatus) && v.contact_status !== filter.contact_status)
      return false;
    if (filter.ballot_status && BALLOT_VALUES.has(filter.ballot_status) && v.ballot_status !== filter.ballot_status)
      return false;
    if (filter.city && !eq(voterCity(v), filter.city)) return false;
    if (filter.ward && !eq(voterWard(v), filter.ward)) return false;
    if (filter.language) {
      const lang = voterLanguage(v);
      if (!lang || !lang.toLowerCase().includes(filter.language.trim().toLowerCase())) return false;
    }
    if (typeof filter.mapped === 'boolean' && (v.lat !== null && v.lng !== null) !== filter.mapped) return false;
    if (typeof filter.hasNotes === 'boolean' && Boolean(v.canvass_notes?.trim()) !== filter.hasNotes) return false;
...
export type TurfSnapshot = {
  ...
  mapped: number;
  unmapped: number;
  geocodableBacklog: number; // unmapped but has an address we could geocode
  contactStatus: Record<ContactStatus, number>;
  ballots: { requested: number; returned: number; outstanding: number; returnRatePct: number };
  territories: { total: number; unassigned: number };
  votersAssignedToTerritory: number;
  distinctCities: number;
  distinctWards: number;
  topCities: { city: string; count: number }[];
  topLanguages: { language: string; count: number }[]; // preferred-language mix, if the file has it
  notesLogged: number;
};
...
export function buildTurfSnapshot(voters: VoterRecord[], territories: Territory[]): TurfSnapshot {
  const contactStatus: Record<ContactStatus, number> = {
    active: 0,
    moved: 0,
    bad_address: 0,
    deceased: 0,
  ...
  for (const v of voters) {
    ...
    if (v.lat !== null && v.lng !== null) mapped += 1;
    else if (v.address_line) geocodableBacklog += 1;
    if (v.ballot_status === 'requested') requested += 1;
    else if (v.ballot_status === 'returned') returned += 1;
    if (v.territory_id) votersAssignedToTerritory += 1;
    if (v.canvass_notes?.trim()) notesLogged += 1;
  ...
  const topN = (counts: Map<string, number>) =>
