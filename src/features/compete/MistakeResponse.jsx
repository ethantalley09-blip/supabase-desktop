...
export function MistakeResponse({ orgId, projectId }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-neutral-300 bg-white p-5">
      ...
      <textarea rows={3} className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm" placeholder="e.g. At last night's forum, the candidate misstated the district's unemployment rate as 12% — it's actually 5.4%. A reporter caught it on video." value={whatHappened} onChange={(e) => setWhatHappened(e.target.value)}/>
      <Button size="sm" onClick={() => draft.mutate({ orgId, projectId, whatHappened: whatHappened.trim() })} disabled={draft.isPending || whatHappened.trim().length < 20}>
