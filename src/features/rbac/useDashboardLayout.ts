...
import type { DashboardLayout } from './toolRegistry';
...
export function useDashboardLayout(orgId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return ((data?.layout as DashboardLayout | null) ?? {}) as DashboardLayout;
    },
...
export function useSaveDashboardLayout() {
  ...
  return useMutation({
    mutationFn: async ({ orgId, layout }: { orgId: string; layout: DashboardLayout }) => {
      const { data: auth } = await supabase.auth.getUser();
      if (!auth.user) throw new Error('Not signed in');
      const { error } = await supabase
        .from('dashboard_layouts')
        .upsert(
          { profile_id: auth.user.id, org_id: orgId, layout, updated_at: new Date().toISOString() },
