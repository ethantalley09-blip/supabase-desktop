...
export function ProjectCreateForm({ orgId, onDone }) {
    const createProject = useCreateProject();
    const [fundraisingAddon, setFundraisingAddon] = useState(false);
    const { register, handleSubmit, control, formState: { errors, isSubmitting } } = useForm({ resolver: zodResolver(projectSchema) });
    ...
    return (<form className="space-y-4 rounded-lg border border-neutral-200 bg-white p-5 shadow-sm" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          <Label>State</Label>
          <Controller name="state" control={control} render={({ field }) => (<Select value={field.value} onValueChange={field.onChange}>
                <SelectTrigger>
      ...
      <div className="rounded-md border border-neutral-200 bg-neutral-50 p-3">
        ...
        <label className="mt-2 flex items-start gap-2 text-sm">
          <input type="checkbox" className="mt-0.5" checked={fundraisingAddon} onChange={(e) => setFundraisingAddon(e.target.checked)}/>
          <span>
