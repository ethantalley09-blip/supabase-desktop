...
export function buildGeocodeAiSnapshot(voters) {
...
export function computeGeocodeHealth(voters) {
    let mapped = 0;
    let noAddress = 0;
    let unattempted = 0;
    let ambiguous = 0;
...
export function needsManualFix(voters) {
    return voters.filter((v) => v.geocode_status === 'ambiguous' || (v.lat === null && (v.geocode_status === 'no_match' || v.geocode_status === 'error')));
}
