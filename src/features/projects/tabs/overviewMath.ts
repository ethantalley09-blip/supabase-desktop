...
export type Momentum = {
  ...
  changePct: number;   // 0-100+, always positive; direction carries the sign
  last15Cents: number;
  prev15Cents: number;
};
...
export type LanguageCoverage = { language: string; total: number; contacted: number; pct: number };
...
export function computeLanguageCoverage(
  voters: { canvass_notes: string | null; data: { language?: string } | null }[]
): LanguageCoverage[] {
  const byLang = new Map<string, { total: number; contacted: number }>();
  for (const v of voters) {
    const lang = v.data?.language;
    if (!lang || lang === 'English') continue;
    const row = byLang.get(lang) ?? { total: 0, contacted: 0 };
    row.total += 1;
    if (v.canvass_notes) row.contacted += 1;
    byLang.set(lang, row);
  }
