...
export type VisitForStaffing = {
  voter_id: string;
  canvasser_id: string;
  occurred_at: string;
};

export type TerritoryStaffing = {
  territoryId: string;
  name: string;
  remainingDoors: number;
  activeCanvassers: number;
  // remainingDoors / max(activeCanvassers, 1) — a zero-canvasser territory's
  // ratio equals its raw remaining count, which is the point: it reads as
  // maximally under-staffed rather than dividing by zero.
  doorsPerCanvasser: number;
};

export type ReallocationSuggestion = {
  fromTerritory: string;
  toTerritory: string;
  fromDoorsPerCanvasser: number;
  toDoorsPerCanvasser: number;
};
...
export function computeTerritoryStaffing(
  voters: VoterRecord[],
  visits: VisitForStaffing[],
  ...
  const canvassersByTerritory = new Map<string, Set<string>>();
  for (const v of visits) {
    if (new Date(v.occurred_at).getTime() < cutoff) continue;
    const territoryId = voterTerritory.get(v.voter_id);
    if (!territoryId) continue;
    const set = canvassersByTerritory.get(territoryId) ?? new Set<string>();
...
export function suggestReallocations(staffing: TerritoryStaffing[], limit = 3): ReallocationSuggestion[] {
  ...
  for (const under of understaffed) {
    const over = overstaffed.find(
      (o) => o.territoryId !== under.territoryId && under.doorsPerCanvasser >= o.doorsPerCanvasser * REALLOCATION_RATIO
    );
    if (!over) continue;
    suggestions.push({
