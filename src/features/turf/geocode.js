...
async function fetchCensus(query) {
    if (window.__TAURI__) {
        const { fetch: tauriFetch, ResponseType } = await import('@tauri-apps/api/http');
        const res = await tauriFetch(`${CENSUS_ORIGIN}${CENSUS_PATH}?${query}`, {
            method: 'GET',
...
export async function geocodeAddressDetailed(address) {
...
function oneLineAddress(voter) {
    const city = (voter.data?.['City'] ?? voter.data?.['city'] ?? '');
    const state = (voter.data?.['State'] ?? voter.data?.['state'] ?? '');
    return [voter.address_line, city, state].filter(Boolean).join(', ');
}
async function sleep(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}
async function geocodeBatch(voters, onProgress) {
    let resolved = 0;
    let ambiguous = 0;
    let noMatch = 0;
    let errored = 0;
    for (let i = 0; i < voters.length; i++) {
        ...
        const update = result.status === 'matched'
            ? {
                lat: result.lat,
                lng: result.lng,
        ...
        if (error)
            errored += 1;
        else if (result.status === 'matched' && !result.ambiguous)
            resolved += 1;
        else if (result.status === 'matched' && result.ambiguous)
            ambiguous += 1;
