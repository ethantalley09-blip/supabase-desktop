...
export type VisitForCadence = {
  canvasser_id: string;
  canvasser_name: string | null;
  occurred_at: string;
};

export type CadencePattern = 'lapsing' | 'accelerating';

export type CadenceSignal = {
  canvasserId: string;
  name: string;
  pattern: CadencePattern;
  // A real, computed description — usable directly as VolunteerPipeline's
  // free-text "situation" field, never a fabricated one.
  situation: string;
};
...
function weekIndex(dateStr: string, now: Date): number {
...
export function detectCanvasserCadence(visits: VisitForCadence[], now: Date = new Date()): CadenceSignal[] {
  const byCanvasser = new Map<string, { name: string | null; recentWeeks: Set<number>; priorWeeks: Set<number> }>();

  for (const v of visits) {
    const wIdx = weekIndex(v.occurred_at, now);
    if (wIdx < 0 || wIdx >= RECENT_WEEKS + PRIOR_WEEKS) continue;

    const entry = byCanvasser.get(v.canvasser_id) ?? {
      name: v.canvasser_name,
      recentWeeks: new Set<number>(),
      priorWeeks: new Set<number>()
    };
    if (wIdx < RECENT_WEEKS) entry.recentWeeks.add(wIdx);
    else entry.priorWeeks.add(wIdx - RECENT_WEEKS);
    if (!entry.name && v.canvasser_name) entry.name = v.canvasser_name;
    byCanvasser.set(v.canvasser_id, entry);
  }

  const lapsing: CadenceSignal[] = [];
  const accelerating: CadenceSignal[] = [];
