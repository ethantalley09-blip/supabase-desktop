...
type GotvPlan = { daily_plan: { day: string; focus: string; volunteer_allocation: string }[]; priority_call: string };

function useGotvSprintPlan() {
  const ai = useAiAssist();
  return useMutation({
    mutationFn: async (input: { orgId: string; projectId: string; context: Record<string, unknown> }) => {
      ...
      const parsed = extractJson<GotvPlan>(result.text);
      if (!parsed) throw new Error('Could not parse the GOTV plan');
      return parsed;
    }
  });
}
...
export function GotvSprintPlan({ orgId, projectId, voters }: { orgId: string; projectId: string; voters: VoterRecord[] }) {
  ...
  const run = () => {
    if (!electionDate || days === null) return;
    plan.mutate({
      orgId,
      projectId,
      context: {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      {days !== null && days > 30 && (
        <p className="text-xs text-amber-600">Plans cover up to 30 days out — check back closer to election day.</p>
      )}
      {plan.isError && <p className="text-sm text-red-600">{(plan.error as Error).message}</p>}
