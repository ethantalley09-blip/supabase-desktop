...
export function AiCenterTab({
  ...
  const [thread, setThread] = useState<(ConversationTurn & { id: string })[]>([]);
  ...
  const fundraisingSnapshot = useMemo(
    () =>
      fundraisingEnt.data
        ? buildFundraisingSnapshot(donations ?? [], donationTotal ?? 0, COMPLIANCE_THRESHOLD_CENTS)
        : undefined,
    [donations, donationTotal, fundraisingEnt.data]
  );
  const snapshot = useMemo(
    () => JSON.stringify({ turf: turfSnapshot, fundraising: fundraisingSnapshot }),
    [turfSnapshot, fundraisingSnapshot]
  );
  ...
  const advisorFundraising = useMemo(
    () =>
      fundraisingEnt.data
        ? [
  ...
  const scriptEntries = useMemo(
    () => (campaignScripts ?? []).map((s) => ({ kind: s.kind, content: s.content, sortOrder: s.sort_order, active: s.active })),
    [campaignScripts]
  );
  const advisorScripts = useMemo(
    () => [answerCurrentDoorScript(scriptEntries), answerCurrentSurveyQuestions(scriptEntries)],
    [scriptEntries]
  );
  ...
  const scenario = useMemo(
    () =>
      scenarioAdjustPace(
        fundraisingPace.dailyAverageCents,
        Math.max(0, scenarioGoalDollars * 100 - (donationTotal ?? 0)),
        scenarioPaceChangePct
      ),
    [fundraisingPace, scenarioGoalDollars, scenarioPaceChangePct, donationTotal]
  );
  ...
  const doorsScenario = useMemo(
    () => scenarioAdjustPace(avgDailyDoors, scenarioDoorsRemaining, scenarioDoorsPaceChangePct),
    [avgDailyDoors, scenarioDoorsRemaining, scenarioDoorsPaceChangePct]
  );
  ...
  const runAsk = (q: string) => {
    ...
    ask.mutate(
      { orgId: project.org_id, projectId: project.id, purpose: 'data_qa', instructions, context: snapshot },
      {
        onSuccess: (data) =>
          setThread((t) => [...t, { id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, question: text, answer: data.text }])
      }
    );
  };
    ...
    <div className="space-y-4">
      ...
      <Card icon={<Search className="h-4 w-4 text-violet-600" />} title="Ask your data">
          ...
          <div className="flex flex-wrap gap-1.5">
            {quickPrompts.map((prompt) => (
              <button
                ...
                className="rounded-full border border-neutral-200 bg-white px-2.5 py-1 text-xs text-neutral-600 hover:bg-neutral-50 disabled:opacity-50"
              >
                {prompt}
              </button>
            ))}
        ...
        <div className="space-y-2 border-t border-neutral-100 pt-3">
          ...
          {cannedAnswers.map((a) => (
            <div key={a.id} className="rounded-md border border-neutral-200 bg-neutral-50 p-2.5">
              <p className="text-xs font-medium text-neutral-900">{a.question}</p>
              <p className="text-xs text-neutral-700">{a.answer}</p>
              {a.evidence.length > 0 && (
                <p className="text-xs text-neutral-400">{a.evidence.join(' · ')}</p>
              )}
              <button
                type="button"
                onClick={() => runAsk(a.question)}
                disabled={ask.isPending}
                className="mt-1 text-xs text-violet-600 hover:underline disabled:opacity-50"
              >
                Ask AI to elaborate →
              </button>
            </div>
      ...
      {has('advisor_insights') && (
      <div id="tool-advisor_insights">
      <Card icon={<TrendingUp className="h-4 w-4 text-violet-600" />} title="Advisor Insights">
        ...
        {momentumAlert && (
          <p className="rounded-md border border-amber-200 bg-amber-50 p-2 text-xs text-amber-800">{momentumAlert}</p>
        )}
        {dataFreshnessAlert && (
          <p className="rounded-md border border-amber-200 bg-amber-50 p-2 text-xs text-amber-800">{dataFreshnessAlert}</p>
        )}
        {contactRateDropAlert && (
          <p className="rounded-md border border-amber-200 bg-amber-50 p-2 text-xs text-amber-800">{contactRateDropAlert}</p>
        )}
        <div className="space-y-2">
          {[...advisorCanvassing, ...advisorTurf, ...advisorSocial, ...advisorFundraising, ...advisorStaffing, ...advisorScripts].map((a) => (
            <div key={a.id} className="rounded-md border border-neutral-200 bg-neutral-50 p-2.5">
              <p className="text-xs font-medium text-neutral-900">{a.question}</p>
              <p className="text-xs text-neutral-700">{a.answer}</p>
            ...
            <div className="flex flex-wrap items-end gap-3">
              <label className="text-xs text-neutral-500">
                ...
                <input
                  type="number"
                  min="0"
                  value={scenarioGoalDollars}
                  onChange={(e) => setScenarioGoalDollars(Number(e.target.value) || 0)}
                  className="mt-1 h-9 w-28 rounded-md border border-neutral-300 px-2 text-sm text-neutral-900"
                />
              </label>
              <label className="text-xs text-neutral-500">
                Pace change (%)
                <input
                  type="number"
                  value={scenarioPaceChangePct}
                  onChange={(e) => setScenarioPaceChangePct(Number(e.target.value) || 0)}
                  className="mt-1 h-9 w-24 rounded-md border border-neutral-300 px-2 text-sm text-neutral-900"
                />
              </label>
            </div>
          ...
          <div className="flex flex-wrap items-end gap-3">
            <label className="text-xs text-neutral-500">
              ...
              <input
                type="number"
                min="0"
                value={scenarioDoorsRemaining}
                onChange={(e) => setScenarioDoorsRemaining(Number(e.target.value) || 0)}
                className="mt-1 h-9 w-28 rounded-md border border-neutral-300 px-2 text-sm text-neutral-900"
              />
            </label>
            <label className="text-xs text-neutral-500">
              ...
              <input
                type="number"
                value={scenarioDoorsPaceChangePct}
                onChange={(e) => setScenarioDoorsPaceChangePct(Number(e.target.value) || 0)}
                className="mt-1 h-9 w-24 rounded-md border border-neutral-300 px-2 text-sm text-neutral-900"
              />
            </label>
          </div>
          ...
          <div className="space-y-2 border-t border-neutral-100 pt-3">
            ...
            <div className="flex gap-2">
              <textarea
                ...
                placeholder="Paste the current door-knock opening script…"
                value={newDoorScript}
                onChange={(e) => setNewDoorScript(e.target.value)}
              />
              <Button
                size="sm"
                disabled={!newDoorScript.trim() || createScript.isPending}
                onClick={() =>
                  createScript.mutate(
                    { projectId: project.id, kind: 'door_script', content: newDoorScript },
                    { onSuccess: () => setNewDoorScript('') }
                  )
                }
              >
                Save
            ...
            <div className="flex gap-2">
              <Input
                ...
                onKeyDown={(e) => e.key === 'Enter' && newSurveyQuestion.trim() && createScript.mutate(
                  { projectId: project.id, kind: 'survey_question', content: newSurveyQuestion, sortOrder: scriptEntries.length },
                  { onSuccess: () => setNewSurveyQuestion('') }
                )}
              />
              <Button
                size="sm"
                variant="outline"
                disabled={!newSurveyQuestion.trim() || createScript.isPending}
                onClick={() =>
                  createScript.mutate(
                    { projectId: project.id, kind: 'survey_question', content: newSurveyQuestion, sortOrder: scriptEntries.length },
                    { onSuccess: () => setNewSurveyQuestion('') }
