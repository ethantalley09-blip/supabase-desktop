...
const voter = (id) => ({ id, project_id: 'p1', full_name: `Voter ${id}`, address_line: '1 Main St' });
...
describe('detectCrossCanvasserOverlap', () => {
    it('flags a household visited by two different canvassers', () => {
        const alerts = detectCrossCanvasserOverlap([household('h1', ['v1'])], [
    ...
    it('does not flag the same canvasser revisiting their own door', () => {
        const alerts = detectCrossCanvasserOverlap([household('h1', ['v1'])], [visit({ voter_id: 'v1', canvasser_id: 'c1' }), visit({ voter_id: 'v1', canvasser_id: 'c1' })]);
        expect(alerts).toEqual([]);
    });
    it('counts visits across different members of the same household', () => {
        const alerts = detectCrossCanvasserOverlap([household('h1', ['v1', 'v2'])], [
            visit({ voter_id: 'v1', canvasser_id: 'c1', canvasser_name: 'Finn' }),
    ...
    it('sorts by visit count descending', () => {
        const alerts = detectCrossCanvasserOverlap([household('h1', ['v1']), household('h2', ['v2'])], [
