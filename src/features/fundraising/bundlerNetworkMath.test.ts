...
import { findBundlerClusters, type DonationForBundler, type DonorForBundler } from './bundlerNetworkMath';

function donor(id: string, name: string, employer: string | null): DonorForBundler {
...
describe('findBundlerClusters', () => {
  it('forms a cluster when two real donors at the same employer have both given', () => {
    ...
    const donations: DonationForBundler[] = [
  ...
  it('excludes an employer with only one real giver', () => {
    const donors = [donor('d1', 'Alice', 'Acme Corp'), donor('d2', 'Bob', 'Acme Corp')];
    const donations: DonationForBundler[] = [{ donor_id: 'd1', amount_cents: 5000 }];
