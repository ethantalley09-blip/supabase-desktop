...
export function ContributionLimitGuardian({ donors, donations }) {
    ...
    const employerAlerts = useMemo(() => findEmployerClustersNearLimit(donors ?? [], donations ?? [], limitCents), [donors, donations, limitCents]);
    return (<div className="space-y-3 rounded-lg border border-slate-200 bg-white p-5">
      ...
      <div className="flex items-center gap-2">
        ...
        <Input type="number" min="1" step="1" className="h-9 w-32" placeholder="e.g. 3300" value={limitInput} onChange={(e) => setLimitInput(e.target.value)}/>
      </div>

      {limitCents <= 0 ? (<p className="text-sm text-neutral-500">Enter a limit to see real donors/clusters approaching it.</p>) : (<>
          <div className="space-y-1 text-xs text-neutral-700">
            <p className="font-semibold uppercase tracking-wide text-slate-500">Individual donors</p>
            {donorAlerts.length === 0 ? (<p className="text-neutral-500">No real donor is within 80% of this limit yet.</p>) : (donorAlerts.map((a) => (<p key={a.donorId} className={a.pctOfLimit >= 100 ? 'font-medium text-red-700' : ''}>
          ...
          <div className="space-y-1 text-xs text-neutral-700">
            ...
            {employerAlerts.length === 0 ? (<p className="text-neutral-500">No real employer cluster is within 80% of this limit yet.</p>) : (employerAlerts.map((a) => (<p key={a.employer} className={a.pctOfLimit >= 100 ? 'font-medium text-red-700' : ''}>
