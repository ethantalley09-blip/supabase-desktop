...
const target = (id) => ({ id, full_name: `Voter ${id}` });
const visit = (over) => ({
    voter_id: 'v1',
...
describe('buildRevisitQueue', () => {
    ...
    it('excludes a door that was ever successfully contacted', () => {
        const queue = buildRevisitQueue([target('v1')], [visit({ voter_id: 'v1' }), visit({ voter_id: 'v1' }), visit({ voter_id: 'v1', outcome: 'contacted' })]);
        expect(queue).toEqual([]);
    });
    ...
    it('ranks by attempt count descending', () => {
        const queue = buildRevisitQueue([target('v1'), target('v2')], [
            visit({ voter_id: 'v1' }),
            visit({ voter_id: 'v1' }),
            visit({ voter_id: 'v2' }),
            visit({ voter_id: 'v2' }),
