...
export function GeocodeAdvanced({ projectId, orgId, voters, canUseAi }) {
    ...
    return (<div className="space-y-4 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      {canUseAi && entitlement.data && (<div className="rounded-md border border-violet-200 bg-violet-50 p-3">
          ...
          {aiCoach.isError && <p className="mt-2 text-sm text-red-600">{aiCoach.error.message}</p>}
      ...
      {remaining > 0 && (<div className="flex items-center gap-3">
          ...
          {progress && geocodeAll.isPending && (<span className="text-xs text-neutral-400">
              {progress.done} of {progress.total}
            </span>)}
        </div>)}
      {geocodeAll.data && !geocodeAll.isPending && (<p className="text-sm text-emerald-600">
      ...
      {fixQueue.length > 0 && (<div className="space-y-2 border-t border-neutral-100 pt-3">
          <div className="flex items-center gap-2">
          ...
          <div className="space-y-2">
            {fixQueue.map((v) => (<div key={v.id} className="rounded-md border border-amber-100 bg-amber-50 p-3 text-sm">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {editing !== v.id && (<Button variant="outline" size="sm" onClick={() => startEdit(v)}>
                      Fix
                    </Button>)}
                </div>
                {editing === v.id && (<div className="mt-2 space-y-2">
                    <div className="flex gap-2">
                      <Input className="flex-1" value={editAddress} onChange={(e) => setEditAddress(e.target.value)} placeholder="Corrected address"/>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => saveManualPin(v.id)} disabled={manualFix.isPending || !manualLat || !manualLng}>
                        Save pin
                      </Button>
      ...
      {manualFix.isError && <p className="text-sm text-red-600">{manualFix.error.message}</p>}
    </div>);
}
function Stat({ label, value, sub, warn }) {
    return (<div className="rounded-md border border-neutral-100 p-2.5">
