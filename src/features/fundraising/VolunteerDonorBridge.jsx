...
export function VolunteerDonorBridge({ orgId, projectId }) {
    ...
    return (<div className="space-y-3 rounded-lg border border-indigo-200 bg-white p-5">
      ...
      <div className="flex flex-wrap gap-2">
        <select className="h-9 flex-1 rounded-md border border-neutral-300 bg-white px-3 text-sm" value={profileId} onChange={(e) => setProfileId(e.target.value)}>
          <option value="">Select volunteer…</option>
          {members?.map((m) => (<option key={m.profile_id} value={m.profile_id}>
              {m.full_name}
            </option>))}
        </select>
        <Input className="flex-1" placeholder="Their contribution, e.g. knocked 120 doors across 3 shifts" value={contribution} onChange={(e) => setContribution(e.target.value)}/>
      ...
      {recentAsks && recentAsks.length > 0 && (<div className="space-y-2">
          {recentAsks.slice(0, 3).map((a) => (<div key={a.id} className="rounded-md border border-indigo-100 bg-indigo-50 p-3">
