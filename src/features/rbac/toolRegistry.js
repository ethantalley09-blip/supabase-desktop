...
export function filterTools(permissions, hasAiModule) {
    if (!hasAiModule || !permissions['ai.use'])
        return [];
    return TOOL_REGISTRY.filter((tool) => tool.requires.every((key) => permissions[key]));
}
export function isToolVisible(toolId, permissions, hasAiModule) {
...
export function arrangeTools(tools, layout) {
...
export function reorderTools(visibleIds, dragId, targetId) {
