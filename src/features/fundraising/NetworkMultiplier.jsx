...
export function NetworkMultiplier({ orgId, projectId, donors }) {
    ...
    const run = () => {
        ...
        if (!donor || !relationship.trim() || !reason.trim())
            return;
        draft.mutate({
            orgId,
            projectId,
    ...
    return (<div className="space-y-3 rounded-lg border border-sky-200 bg-white p-5">
      ...
      <div className="space-y-2">
        <select className="h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm" value={donorId} onChange={(e) => setDonorId(e.target.value)}>
          <option value="">Which donor is sharing?</option>
          {donors?.map((d) => (<option key={d.id} value={d.id}>
              {d.full_name}
            </option>))}
        </select>
        <Input placeholder="Who's it for? e.g. my coworkers at the clinic" value={relationship} onChange={(e) => setRelationship(e.target.value)}/>
        <Input placeholder="Why did they give? e.g. school funding matters to my kids" value={reason} onChange={(e) => setReason(e.target.value)}/>
      ...
      {asks && asks.length > 0 && (<div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-neutral-500">Ready to forward</p>
          {asks.map((a) => (<div key={a.id} className="rounded-md border border-sky-100 bg-sky-50 p-3">
