...
function knockable(v) {
    return v.contact_status === 'active';
}
...
export function classifyPersuadability(v) {
    // Dead, hostile-by-status, or already-excluded doors carry no directional
    // signal worth acting on.
    if (v.contact_status !== 'active')
        return { bucket: 'unknown', score: 0, reasons: [] };
    ...
    if (v.ballot_status === 'returned' && support > 0)
        support += 10;
    const top = Math.max(support, persuadable, opposed);
    if (top < PERSUADABLE_THRESHOLD)
        return { bucket: 'unknown', score: 0, reasons: [] };
    if (opposed === top)
        return { bucket: 'opposed', score: opposed, reasons };
    if (persuadable === top)
        return { bucket: 'persuadable', score: persuadable, reasons };
...
// a time.
export const PERSUADABILITY_COLORS = {
    base_support: '#16a34a',
    persuadable: '#d97706',
    opposed: '#991b1b',
    unknown: '#d4d4d4'
};
export const PERSUADABILITY_LABELS = {
...
function fundraisingWeight(v) {
    if (v.contact_status !== 'active')
        return 0;
    const notes = (v.canvass_notes ?? '').toLowerCase();
    const hits = FUNDRAISING_PHRASES.filter((p) => notes.includes(p)).length;
    if (hits === 0)
        return v.ballot_status === 'returned' ? 0.15 : 0;
    return Math.min(1, 0.4 + hits * 0.2);
}
const STALENESS_WINDOW_MS = 14 * 24 * 60 * 60 * 1000; // 14 days
function stalenessWeight(v, nowMs) {
    if (!v.last_contacted_at)
        return 1; // never contacted is the hottest
...
export function heatmapWeight(v, mode, nowMs = Date.now()) {
    switch (mode) {
        case 'density':
            return 1;
        case 'persuadability': {
            const { bucket } = classifyPersuadability(v);
            if (bucket === 'persuadable')
                return 1;
            if (bucket === 'base_support')
                return 0.4;
            if (bucket === 'opposed')
                return 0.1;
            return 0.2;
...
export function remainingDoorsToday(voters, nowMs = Date.now(), opts) {
    const todayWindowMs = opts?.todayWindowMs ?? RECENT_WINDOW_MS;
    return voters.filter((v) => {
        if (!knockable(v) || v.lat === null || v.lng === null)
            return false;
...
export function buildBriefingSnapshot(voters, territories, nowMs = Date.now(), opts) {
    ...
    const activeVoters = voters.filter((v) => v.contact_status === 'active');
    for (const v of activeVoters) {
        const contactedMs = v.last_contacted_at ? new Date(v.last_contacted_at).getTime() : null;
        if (contactedMs !== null && nowMs - contactedMs <= recentWindowMs)
            contactedLast4h += 1;
        if (contactedMs !== null && nowMs - contactedMs <= todayWindowMs)
            contactedLast24h += 1;
        const { bucket } = classifyPersuadability(v);
        if (bucket === 'persuadable')
            persuadable += 1;
        else if (bucket === 'base_support')
            baseSupport += 1;
        else if (bucket === 'opposed')
            opposed += 1;
        else
            directionUnknown += 1;
    }
