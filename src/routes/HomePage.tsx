      ...
      <header className="flex items-center justify-between border-b border-neutral-200 bg-white px-6 py-3">
        ...
        <div className="flex items-center gap-3">
          {profile?.is_super_admin && (
            <Link to="/admin" className="text-sm text-neutral-600 underline-offset-2 hover:underline">
              SuperAdmin
            </Link>
          )}
...
function OrgProjects({ org }: { org: Organization }) {
  ...
  return (
    <div className="mt-4 space-y-3">
      ...
      {creating ? (
        <ProjectCreateForm orgId={org.id} onDone={() => setCreating(false)} />
      ) : (
        canManageProjects.data && (
